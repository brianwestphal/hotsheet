...
const DEMO_META: Record<number, DemoMeta> = {
  ...
  3: { verb: 'focus', heroSelector: '.sidebar, #sidebar', caption: 'Slice your work — custom views & filters', focusScale: 1.5 },
  ...
  8: { verb: 'focus', heroSelector: '.dashboard-chart-card', caption: 'Throughput, flow & cycle time', captionPosition: 'top-center', focusScale: 1.42 },
...
const INTERACTIONS: Record<number, InteractionSpec> = {
  ...
  5: {
    ...
    frames: (url) => [
      ...
      // Right-click a selected row → context menu. The menu items have no stable
      // selectors, so after dispatching the (positioned) contextmenu we TAG the
      // Priority item + its High option with ids and force the submenu visible —
      // then the cursor can visibly move to them via hover/click actions.
      {
        continue: true,
        actions: [
      ...
      {
        continue: true,
        actions: [{ type: 'click', selector: '#demoHighItem' }, { type: 'wait', ms: 500 }],
        duration: 1700,
        transition: { type: 'crossfade', duration: 200 },
      },
    ],
