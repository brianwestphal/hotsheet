...
/// navigation fails with `net::ERR_ABORTED` and the webview silently stays on
/// `about:blank`, which is why the 0.20.0 Windows .exe/.msi installs opened an
/// empty window instead of the welcome screen (HS-9738; the same bug as
/// Glassbox GitHub #57). Reproduced on a Windows 11 VM against the shipped
/// 0.20.0 installer via CDP: the page sat at `about:blank`, and navigating the
/// same process to `http://tauri.localhost/welcome.html` rendered the welcome
/// screen. `app.windows[].useHttpsScheme` is not set in `tauri.conf.json`, so
/// the Windows scheme is `http`.
///
/// Only the release-only no-args / restore-failed branches call this (dev mode
/// always runs the server and navigates to it), hence the debug-build
/// dead-code allowance.
                    ...
                    if let Some(ref data_dir) = restored_dir {
                        ...
                        tauri::async_runtime::spawn(async move {
                            if let Err(e) = spawn_sidecar_and_navigate(&handle, &dir, extra).await {
                                ...
                                // Navigate to welcome screen as fallback — platform-
                                // specific asset URL, see `app_asset_url` (HS-9738).
...
#[cfg(test)]
mod app_asset_url_tests {
    //! HS-9738 — the welcome screen is reached by navigating to a bundled asset,
    //! and the scheme Tauri serves it from differs per platform. Windows
    //! (WebView2) resolves only `http://tauri.localhost`; a `tauri://localhost`
    //! URL there aborts and leaves the webview on about:blank — the empty-window
    //! bug in the 0.20.0 Windows installers (same class as Glassbox GitHub #57).
    use super::{app_asset_url, app_asset_url_for};

    #[test]
    fn uses_http_host_on_windows_style_platforms() {
        assert_eq!(
    ...
    #[test]
    fn uses_custom_scheme_elsewhere() {
        assert_eq!(
    ...
    #[test]
    fn matches_host_platform_and_parses() {
