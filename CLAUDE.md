...
- **Don't write new `xxx.innerHTML = yyy` assignments in client code** — `toElement` (now routed through `kerfjs::toElement` per HS-8241 / §62) handles SVG-namespace + entity + custom-attr correctness; direct `innerHTML` assignments bypass that and reintroduce the bug class §62 closed. Use `el.replaceChildren(toElement(<jsx />))` instead, or `el.replaceChildren(toElement(<span>{raw(htmlString)}</span>))` for raw-HTML escape hatches. The `no-restricted-syntax` ESLint rule (HS-8243) flags new violations outside an explicit file-path allowlist (~35 files with existing usage — see `eslint.config.mjs`); when you touch one of those files for unrelated reasons, opportunistically migrate the existing `innerHTML` callsite and remove the file from the allowlist so it gets full protection going forward. Test files (`**/*.test.{ts,tsx}`) are exempt — `document.body.innerHTML = '<...>'` is the standard happy-dom setup pattern.
