...
114. `114-acp-channel.md` — the ACP (Agent Client Protocol) *agent-drive* layer for Tier-A CLI agents (docs/113 §113.2). Design (HS-9310), gated on the HS-8008 spike. An **ACP client** (`src/acp/client.ts`, greenfield) parallels the Claude channel: `session/prompt` = play button, `session/update` = busy, `stopReason` = done, `session/request_permission` = the §47 overlay (which becomes **option-driven** — the agent supplies its own `PermissionOption[]`, the key surface change). MCP (`hotsheet_*`) rides ACP unchanged via `session/new.mcpServers`. Claude stays on its MCP-notification channel. Open decisions §114.9
