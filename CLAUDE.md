...
104. `104-worker-worktree-auto-approve.md` — pre-approve a worker worktree's MCP server + skills by writing the worktree's `settings.local.json` at create time; fixes the `registerChannelAt` allow-rule gap; residual workspace-trust prompt documented (design only, HS-9058)
