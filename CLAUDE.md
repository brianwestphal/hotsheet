...
- **Reading order** — documents are numbered for linear reading, from high-level overview to specific subsystems:
  ...
  69. `69-telemetry-dashboard.md` — Cross-project telemetry dashboard (HS-8153 design): new full-window view accessible from the left sidebar, conditional visibility (only shown when at least one project has `telemetry_enabled === true`). Sections: Today / Week / Month dollar-amount chips, cost-by-project sortable table with click-through to the per-project drawer Telemetry tab, cost-by-model SVG donut + legend, 7×24 day-of-week × hour activity heatmap inline-SVG, top-10 most-expensive-prompts list with click-through to the §67.10.3 drilldown. Single bundled `GET /api/telemetry/dashboard?window=...` route; live queries against `otel_metrics` + `otel_events`, no precomputed rollup tables (per §67.6). Implementation phased: HS-8479 (sidebar entry), HS-8480 (backend queries + endpoint), HS-8481 (shell + chips + empty-state), HS-8482 (project table + model donut), HS-8483 (heatmap + top-10). **Superseded by §70 (HS-8503 reshape — Phase 3 / HS-8507 shipped).**
