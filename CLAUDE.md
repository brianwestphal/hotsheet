...
### Filesystem access on a user-configured path (`backupDir`, `dataDir`)
...
**`dataDir` counts too, and the dangerous calls are narrower than they look (HS-9568 / HS-9570).** `dataDir` is user-chosen, so a project kept in iCloud / Dropbox / a network share puts **attachment** reads and writes on the same footing as the backup dir — on a REQUEST path. `src/routes/attachments.ts` is therefore covered by the same ESLint selector, and its serve path **streams** rather than buffering (which also stops an 80 MB attachment costing 80 MB of `external` per concurrent serve).

But the hazard is specifically **a synchronous call that must fetch or flush BYTES**, not sync `fs` in general. Measured: `readdir` plus a per-entry `existsSync` against a Google Drive File Provider root took **0.6 ms**, and 10 000 local entries took 44 ms — File Provider caches directory *metadata* locally. HS-9527's 686 ms was a *134 KB content read*, which has to be materialized over the network. Metadata calls stay banned in these modules anyway (cheap to avoid, easy to misjudge at the call site), but when weighing risk elsewhere, ask whether the call moves bytes.
...
89. `89-git-worktrees.md` — **RETIRED (HS-9686).** git worktrees + per-worktree AI agents. **KEPT: the follower `.hotsheet` redirect** (Phase A — `resolveAuthoritativeDataDir` + `deriveChannelActor`) so an agent in any worktree claims from the owner's backlog. **REMOVED:** worktree create/list/remove + pool wiring (Claude/Codex do worktrees now). Adopt-worktree follow-up = HS-9688.
90. `90-distributed-execution.md` — claim/lease + distributed execution. **KEPT — the tool-agnostic claim/lease coordination core:** `claimed_by`/lease columns, atomic `claim-next` (SKIP LOCKED), the flat `blocked_by` gate, `deriveChannelActor`, and the `hotsheet_claim_next`/`_renew_lease`/`_release`/`_set_blocked_by` MCP tools. **RETIRED (HS-9686): the worker pool it fed** (launch/dispatch/pool MCP tools).
91. `91-worker-pool-scaling.md` — **RETIRED (HS-9686).** worker-pool dynamic scaling design (historical).
92. `92-coordinator-dispatch.md` — **RETIRED (HS-9686).** coordinator-dispatch UX design (historical).
...
98. `98-worker-batching-policy.md` — **RETIRED (HS-9686).** worker freshness/batching policy (historical).
99. `99-worker-worktree-refresh.md` — **RETIRED (HS-9686).** worker-side `refreshWorktree` helper (historical).
100. `100-server-driven-worker-launch.md` — **RETIRED (HS-9686).** server-driven worker-pool launch choreography (historical).
101. `101-prompt-based-worker-management.md` — **RETIRED (HS-9686).** worker-pool prompt box + partition/dispatch (historical).
102. `102-per-worker-git-state-and-review.md` — **RETIRED (HS-9686).** per-worker git state on pool tiles + branch review (historical).
103. `103-command-button-target-picker.md` — **RETIRED (HS-9686).** command-button "Run on → worker/all-workers" picker (historical).
104. `104-worker-worktree-auto-approve.md` — **RETIRED (HS-9686).** pre-approve a worker worktree's MCP server + skills (historical).
105. `105-worktree-node-modules-provisioning.md` — **RETIRED (HS-9686).** provision `node_modules` into worker worktrees (historical).
106. `106-integration-helpers-ready-signal-and-gates.md` — **RETIRED (HS-9686).** per-worker branch-ready signal + in-helper gates (historical).
...
134. `134-server-supervision.md` — **server process supervision** (HS-9563, design + one shipped half): how the Tauri shell notices the server process has DIED and what it does about it. Organizing distinction — the server stops serving two ways and they need different machinery: a **wedge** (loop pinned, process alive) is the docs/45 watchdog's job, an **EXIT** (process gone) is the shell's, and a wedge becomes an exit because the watchdog SIGKILLs it. §134.3 SHIPPED (HS-9558, dev only): `describe_child_exit` + a permanent non-dismissible "Server Stopped" overlay that REPLACES the connection-error popup, because the shell reaped the child so "the server is gone" is a fact, not the guess a failed fetch makes. **§134.4 + §134.5 are open BUGS, not decisions — production is worse off than dev:** the sidecar's exit after a successful navigate is handled by literally nothing, and its stderr is `eprintln!`-only (lost on a GUI launch, which is how the shipped app always starts) — the same hole HS-9557 closed for dev, and the only place a WASM/OOM abort is ever recorded since those never reach JS. §134.6 is the actual decision: **A** never restart / **B** a "Restart Server" button / **C** bounded auto-restart; recommendation **B**, since it is a strict subset of C, cannot spin on a crash-on-startup bug, and there is currently ONE observed death to design from. Any of B/C needs the spawn extracted out of `.setup()` (the launch path — HS-8704 territory), the `--replace`-vs-lock race verified rather than assumed, and a renavigate. **DECIDED 2026-08-04 (HS-9563): A — never restart.** The maintainer's reasoning reframes the area: a restart is failure ABSORPTION (cheaper deaths, not rarer ones), and the real requirement is that the server SURVIVES long uptime on a machine periodically under heavy memory/CPU pressure — tracked as HS-9566 / §134.10, where the prevention work lives (docs/128 ceiling validation via HS-9562, docs/131 pressure response, and — the investigation's actual finding, **HS-9567** — that **`server-wake` has NEVER fired on macOS**: `isSuspendGap` keys off wall-vs-monotonic clock divergence, but libuv's monotonic clock ADVANCES during sleep there, so every suspend is logged as a multi-minute "event-loop blocked" and the suspend branch — the only place `wakeListeners` fire — is unreachable, meaning the HS-8726 post-wake stagger has never engaged despite HS-9553 naming a post-sleep stampede as the 2026-08-01 trigger. The fix signal already exists: `freezeAnalysis::looksLikeSuspend` uses CPU ratio, wired only into post-hoc analysis. The earlier "can the watchdog SIGKILL a healthy but CPU-starved server?" worry is **dropped, not backlog** — measured blocks are p50 133 ms / p99 664 ms, ~100× under the 60 s threshold, and both stack captures show a genuinely pinned frame). B/C are recorded as considered-and-declined so the question isn't reopened from scratch. §134.9 has no open decisions.
135. `135-empty-cluster-surfacing.md` — **empty-cluster surfacing** (HS-9576, SHIPPED): tells the user when a project's database came up EMPTY over data it used to hold — the state HS-9573's guard already refuses to write over, but which until now only reached stderr. **The guard buys time; §135 spends it** — the artifacts survive only until someone notices, and an empty project looks exactly like a working one (why the 2026-08-04 incident was invisible for a day). REUSES the HS-7899 surface rather than inventing one (`.db-recovery-marker.json` + `GET /api/db/recovery-status` + the banner + Dismiss), keyed off a new optional `kind` (`corrupt-open` | `empty-cluster`, defaulting to `corrupt-open` on read so an older server's marker still parses) plus `priorTicketCount`; `corruptPath` is deliberately `''` because no cluster was renamed aside IN THIS PROCESS. Copy must say three things the corrupt-open wording does not — nothing failed to load, the existing copies are PAUSED/protected (not merely available), and check Repair Database FIRST since a preserved `db-corrupt-*` folder may be newer than the newest backup (HS-9575 made those selectable). Noise control: once per project per session (a blocked project trips the guard every backup + snapshot tick, so without the gate Dismiss would work for minutes at a time), dismissible, self-retracting when rows return, and **kind-scoped clearing** so a `corrupt-open` marker survives. **Both writers feed it** — whichever ticks first tells the user. Marker I/O extracted to `src/db/recoveryMarker.ts` (a guard writing through `connection.ts` would close an import cycle; `connection.ts` re-exports). **Known gap HS-9585:** the guard itself disarms on the next restart — `noteClusterCreatedEmpty` fires only when the open finds no `PG_VERSION`, so a restart that OPENS the empty cluster leaves the writers free again; the banner persists, the protection does not.
136. `136-terminal-session-survival.md` — **terminal session survival across server restarts** (HS-9662, DESIGN ONLY): make terminals + their AI sessions **survive an accidental node-server death** (OOM / §45 watchdog SIGKILL / crash / `--replace` / HS-9656 auto-restart) and **auto-restore as tabs** on reconnect, while still tearing them down on EXPLICIT actions (close a terminal tab, close a project tab, quit the app) — maintainer decision 2026-08-14. Root cause (audited): PTYs are non-detached children of the node server's process group (`node-pty` in `src/terminals/registry/lifecycle.ts`), the `sessions` registry is in-memory, and *dynamic* terminal identity (`dynamicConfigs`) has zero disk persistence — so any death kills them and some can't even be re-listed. Design: a **detached PTY broker** in its own process group owning all PTYs (+ scrollback + dynamic identity) over a control socket; the node server becomes a client that **re-adopts** live sessions on restart; explicit closes + app-quit signal the broker; a client-lease heartbeat prevents orphan shells. Client reuses the existing `GET /api/terminal/list` + checkout-WS `history` replay (docs/54) surface. Accidental *node* death survives; broker death / reboot does not (accepted scope). Also flags the 2026-08-14 `--replace` port double-bind (two servers on 4174) as a companion fix. Absorption layer = HS-9656/docs/134; prevention = docs/128/131 (HS-9566).
137. `137-permission-auto-approve.md` — **auto-approve permission requests after a timeout** (HS-9702, SHIPPED): a per-project **opt-in** Settings → Permissions dropdown (`Off`/15s/1/2/5/15/60 min → `permission_auto_approve_ms`, **local-only** like the §47 allow-rules) that auto-**approves** any surfaced permission request the user leaves unanswered for the chosen window. **Client-driven visible countdown** (`Auto-approving in M:SS` + a **Cancel** that keeps the popup open) fires the normal allow path on expiry; §47 auto-allow rules still take precedence; audit-logged as `Auto-approved (timeout)`. Off by default. Pure logic in `src/permissionAutoApprove.ts` (fail-closed `parseAutoApproveMs`); the main server attaches a computed `auto_approve_at` deadline to each poll entry (owning project's window, so a background-project popup counts down correctly); `peekPending(now, loneTtlMs?)` extends the channel's lone-request backstop to cover long windows so a counting-down request isn't abandoned mid-countdown (no channel-version bump — the `/permission` wire is unchanged). Complements §47.

Other docs: `docs/architecture-diagram.md` (**visual end-to-end map** — Mermaid diagrams of the app side (Tauri shell + client) and server side (Hono + PGLite + subsystems), the process/IPC boundaries, client↔server transports, channel/AI-drive, worker pool, data stores, telemetry; grounded in the code with anchor files. Start here for the big picture), `docs/tauri-architecture.md` (Tauri v2 sidecar, launch/build/CI signing), `docs/tauri-setup.md` (build prereqs, signing keys, release workflow), `docs/dependency-security.md` (npm + cargo audit posture, Dependabot, triage), `docs/plugin-development-guide.md` (AI-focused plugin-building guide — **keep up to date** whenever the plugin system changes, so an AI can build a working plugin without reading source), `docs/demo-plan.md` (story-driven demo script for showing Hot Sheet + Glassbox to potential users), `docs/feature-health.md` (**does each feature actually work?** — per-feature Solid / Shaking out / Underbaked / Incomplete / Unknown, a point-in-time snapshot regenerated by the `/feature-health` skill. Distinct from `docs/ai/requirements-summary.md`, which tracks whether a feature was *built*; this one tracks whether it's *verified*. Read it before promising a feature works).
...
- Other docs: `docs/architecture-diagram.md` (visual end-to-end Mermaid map, app + server sides), `docs/tauri-architecture.md`, `docs/tauri-setup.md`, `docs/dependency-security.md`, `docs/plugin-development-guide.md`, `docs/demo-plan.md`.
