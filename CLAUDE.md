...
137. `137-permission-auto-approve.md` — **auto-approve permission requests after a timeout** (HS-9702, SHIPPED): a per-project **opt-in** Settings → Permissions dropdown (`Off`/1/2/5/15/60 min → `permission_auto_approve_ms`, **local-only** like the §47 allow-rules) that auto-**approves** any surfaced permission request the user leaves unanswered for the chosen window. **Client-driven visible countdown** (`Auto-approving in M:SS` + a **Cancel** that keeps the popup open) fires the normal allow path on expiry; §47 auto-allow rules still take precedence; audit-logged as `Auto-approved (timeout)`. Off by default. Pure logic in `src/permissionAutoApprove.ts` (fail-closed `parseAutoApproveMs`); the main server attaches a computed `auto_approve_at` deadline to each poll entry (owning project's window, so a background-project popup counts down correctly); `peekPending(now, loneTtlMs?)` extends the channel's lone-request backstop to cover long windows so a counting-down request isn't abandoned mid-countdown (no channel-version bump — the `/permission` wire is unchanged). Complements §47.
