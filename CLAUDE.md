...
93. `93-websocket-push-sync.md` — WebSocket push (`/ws/sync`) design implementing §46.3: server event bus + ring/seq, `?since` catch-up + `resync`, heartbeat, client reducer + exponential-backoff reconnect + auto-fallback to `/api/poll` (additive — long-poll stays). Design only; decomposed into HS-8978–8982, gated on HS-7940 for non-local use
