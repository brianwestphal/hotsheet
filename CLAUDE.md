...
- **Reading order** — documents are numbered for linear reading, from high-level overview to specific subsystems:
  ...
  56. `56-magnified-grid-nav.md` — Magnified terminal-grid navigation (HS-8028): while a tile is centered (single-click overlay) or dedicated (double-click full-pane) in the §25 dashboard or §36 drawer-grid, **Shift+Cmd+Arrow** (macOS) / **Shift+Ctrl+Arrow** (Linux/Windows) switches the magnified target to the next tile in the indicated direction. Direction computed from each tile's bounding-rect centroid using a perpendicular-weighted cone metric so a same-row neighbour beats a diagonal one. Capture-phase document keydown listener bound on `centerTile` / `enterDedicatedView`, unbound on the last-magnified-state exit. Skips non-alive tiles. Wrap-around past the grid edge is a no-op.
