...
121. `121-codex-app-server-drive.md` — Codex persistent drive via its `app-server` JSON-RPC protocol (HS-9381, design only): one Hot Sheet-owned `codex app-server` child per project, play/custom commands = `turn/start` into a persisted thread (the Claude-channel analog codex lacks over MCP), approvals → §47 overlay ACP-bridge-style, Commands Log transcript streaming; Experimental-tab toggle **default ON**, disabled = hide play + codex prompt buttons (no one-shot fallback — supersedes the §115.6a exec drive at ship). Maintainer-decided 2026-07-23; spike + implementation tickets filed.
