...
- **Reading order** — documents are numbered for linear reading, from high-level overview to specific subsystems:
  ...
  43. `43-attachment-backups.md` — Attachment backups (HS-7900 design / HS-7929 implementation): hash-addressed centralised store under `<backupRoot>/attachments/<sha256>` + per-backup `backup-<TS>.attachments.json` manifest sibling. Daily orphan GC. Closes the gap left by §7 (tarball) and §41 (JSON co-save), neither of which carries the attachment binaries themselves.
  ...
  46. `46-service-client-decoupling.md` — Service / client decoupling design spike (HS-7938): detach the Hot Sheet service (HTTP + dataDir + plugins + backups + channel + terminal PTYs) from the Tauri desktop client so multiple clients (desktop browser, Tauri shell, mobile browser, future iOS app) can connect simultaneously to a single service instance. WebSocket-based push synchronisation replaces today's `/api/poll` long-poll model. Conflict resolution = last-write-wins for scalars + append-only for notes + optimistic concurrency for risky ops. Tauri shell becomes co-located (default, today's behaviour) OR remote (`--service-url`). Service-only mode (`--service-only`) for headless deployment. Implementation phased: HS-7940 (`--bind` + `isTrustedOrigin` + GET-secret enforcement), HS-7944 (service-only + remote shell), HS-7945 (WebSocket push), HS-7946 (conflict UX), HS-7941 (PWA + mobile responsive), HS-7942 (Tailscale UX sugar).
  ...
  52. `52-terminal-prompt-overlay.md` — Terminal permission-prompt overlay design (HS-7971): detector watches the active terminal's xterm buffer (debounced 100 ms scan of last 10 rows), pluggable parser registry recognises Claude-Ink numbered-choice (`> 1. Foo / 2. Bar / Enter to confirm · Esc to cancel`) + yes/no shapes, surfaces a Hot Sheet overlay anchored to the terminal pane with clickable choices, falls back to a monospaced reproduction + free-form input for unparseable prompts the heuristic still fires on. Per-project always-allow rules keyed on `parser_id + question-hash + chosen-option-index` mirror §47.4 (settings file + audit-log entry, never auto-respond from generic fallback). Phased: Phase 1 (detection + Claude numbered + Allow/Cancel), Phase 2 (yes/no + generic fallback + per-instance suppression), Phase 3 (always-allow gate + audit), Phase 4 (Settings → Permissions sub-section + master toggle). Overrides HS-6602's earlier rejection of terminal scraping; rationale in §52.1.
  ...
  57. `57-shell-command-button-spinner.md` — Shell custom-command-button spinner & stop (HS-8056): when a §15 Shell-target command is running, the button itself surfaces an absolute-positioned spinner with stop icon at the right edge (background: inherit so it visually punches the button colour, no reflow). Clicking the running button opens a confirm dialog → `POST /api/shell/kill`. Multiple commands can run simultaneously; per-button state tracked via a module-private `runningButtons: Map<commandKey, logId>` that the existing `/api/shell/running` poll keeps in sync.
  58. `58-channel-auto-approve.md` — Channel auto-approve / known-channel allow-list (HS-8210, design only): extends §52's terminal-prompt always-allow path with a Tier 0 matcher keyed on Claude's `Channels: server:<name>` line. Parser extracts the channel via `/^Channels:\s+(\S+)/m` into a new optional `channel?: string` field on `NumberedMatch`; allow-rule schema gains `match_channel?: string`; first manual approval implicitly creates a channel-keyed rule (no checkbox required, opt-out via "Don't remember"). Tier 0 fires before Tiers 1–4 so the channel-keyed rule wins regardless of question-hash / preview / choice-shape / choice-label drift. Implementation phased: HS-8211 parser + types, HS-8212 schema + Tier 0, HS-8213 implicit creation + UI, HS-8214 Settings label + audit-log update.
  ...
  60. `60-reactivity-primitive.md` — Fine-grained reactivity primitive design (HS-8166): adopt `@preact/signals-core` (~1.4 KB) behind a thin `src/client/reactive.ts` re-export, ship `bindText` / `bindAttr` / `bindList` keyed-binding helpers in `src/client/reactive-bind.ts`, migrate one trial callsite (project tabs) under HS-8235 then high-traffic surfaces (ticket list, command log, terminals) under HS-8236 and the long tail under HS-8237. `jsx-runtime.ts` + `toElement` unchanged; signals only decide WHEN to re-mount.
...
- **Use `toElement()` instead of `document.createElement()`** — when creating DOM elements in client code, use the `toElement()` helper from `dom.ts` with JSX: `toElement(<div className="foo">bar</div>)`. Resolve JSX to DOM elements only at the last moment. Never use `document.createElement()` directly. The three intentional exceptions are: `src/client/dom.ts::toElement` itself (the helper's implementation), `src/client/terminalCheckout.tsx` orphaned-xterm parking sink (xterm.js owns the element lifecycle), and `src/client/terminalFonts.ts` `<link>` stylesheet injection (must construct a real `HTMLLinkElement` for the `load`/`error` events).
