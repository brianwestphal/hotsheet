...
describe('openWorktreesPanel (HS-8938)', () => {
  ...
  it('HS-8936 — "Open terminal" opens a claude terminal in the worktree cwd', async () => {
    ...
    await flush();
    // HS-9036 / HS-9334 — launches via the `{{aiCommand}}` token (server resolves it to the
    // channel-connected command so the worktree's agent routes permissions to Hot Sheet).
    expect(mockOpenTerminal).toHaveBeenCalledWith('{{aiCommand}}', expect.stringContaining('feat'), follower.path);
