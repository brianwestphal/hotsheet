 ...
 *  the batch. Pure data — no UI; callers toast via `dispatchAndReport`. */
export async function dispatchTicketsToWorker(worker: string, label: string, ids: readonly number[]): Promise<DispatchResult> {
  let dispatched = 0;
  const failures: string[] = [];
  for (const id of ids) {
    try {
      await claimTicket(id, { worker, label });
      dispatched++;
    } catch (e) {
...
export function dispatchSummary(result: DispatchResult, label: string): string {
  const parts: string[] = [];
...
/** Dispatch + toast the outcome. The common entry-point wrapper. */
export async function dispatchAndReport(worker: string, label: string, ids: readonly number[]): Promise<DispatchResult> {
  const result = await dispatchTicketsToWorker(worker, label, ids);
  showToast(dispatchSummary(result, label));
  return result;
}
