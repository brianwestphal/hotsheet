...
export function bindPluginSettings() {
  // HS-9003 — the plugins settings panel (and `#plugin-install-btn`) is only
  // server-rendered when `PLUGINS_ENABLED` (pages.tsx). With plugins disabled
  // the button is absent, so the old `byId('plugin-install-btn')` THREW at bind
  // time → an unhandled rejection → the generic HS-9455 crash popup on every
  // load for a plugins-disabled instance (surfaced by the demo capture, which
  // runs with `PLUGINS_ENABLED=false`). Nothing to bind when the panel is gone.
  const installBtn = byIdOrNull('plugin-install-btn');
