...
async function startAndConfigure(port: number, dataDir: string, strictPort: boolean, bind?: string): Promise<{ actualPort: number; secret: string }> {
  ...
  // injected rather than imported so `diagnostics/` keeps no dependency on `db/`.
...
async function setupInstanceLifecycle(actualPort: number): Promise<void> {
  ...
  // HS-9383 / HS-9493 — also pre-import the drive backing services so their children
  // are killed on the same synchronous exit path (a leaked session would keep an orphan
  // `codex app-server` + its MCP child alive). Asked of every registered service rather
  // than of codex by name (docs/132 §132.1.1).
  const { shutdownAllDriveServices } = await import('./aiTools/serverCapabilities.js');
  const cleanupInstance = (): void => {
    try { destroyAllTerminals(); } catch { /* already torn down */ }
    try { shutdownAllDriveServices(); } catch { /* already torn down */ }
    removeInstanceFile();
  };
