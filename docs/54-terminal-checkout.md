...
**HS-8619 — top-of-stack resize gate.** `handle.resize(cols, rows)` is a no-op unless the calling handle is the current top of the stack. Only the consumer actually rendering the live xterm may drive its size; a bumped-down consumer (e.g. the drawer pane while a §25 dashboard tile borrows the same terminal) whose own fit / `onResize` wiring keeps firing must not resize the shared term out from under the active consumer — that produced the dashboard "resize weirdly" oscillation, scoped to the active project because only its terminals carry a live drawer-pane consumer. The gate covers the `handle.resize` path centrally; consumers that run a `FitAddon` must ALSO gate their own `fit.fit()` calls on `isTopOfStack()` (the drawer's `onRender → doFit` + the panel / window `ResizeObserver → doFit` do), because `fit.fit()` calls `term.resize` directly and bypasses the handle. Note this is renderer-independent — the WebGL-on-scaled-tiles raster-scaling that motivated HS-8619 is fixed separately by the §54 `scaled` flag → DOM renderer reconcile (HS-8488 `webglWantedForConsumer`).
...
The HS-7968 IntersectionObserver-driven mount/dispose path is preserved verbatim — `softDisposeTile` now calls `tile.checkout?.release()` instead of disposing the local term/ws. The empty-stack dispose-the-entry path in checkout (§54.3.3) means a virtualized-off-screen tile that's the only consumer fully disposes the xterm + closes the WS, matching pre-fix behavior. If a dedicated view is up while the tile virtualizes off-screen, the dedicated's checkout keeps the entry alive — the tile's `release()` just decrements the stack count.

`ensureTileMounted(tile)` (the click-before-IO defensive force-mount path used by `centerTile` / `enterDedicatedView` when the IO hadn't fired yet) routes through the same `mountTileViaCheckout` so behavior is consistent across the cold-mount paths.
...
**HS-8287 follow-up (2026-05-07).** The reconnect path reuses the existing `entry.term`, so the server's `history`-frame replay (the entire ring buffer) needs to OVERWRITE the term's existing buffer, not append. Pre-fix `applyHistoryReplay` was just `term.write(bytes)` — a transient WS flap left the user's pre-disconnect scrollback on screen, then the replay appended a duplicate copy. Fix calls `entry.term.reset()` AND `entry.term.clear()` at the top of `applyHistoryReplay` (reset clears terminal state; clear drops scrollback explicitly) BEFORE the HS-8064 capture-time resize, so the reflow logic operates on a clean buffer and the replayed bytes are authoritative. Neither call fires `onResize`, so the HS-8064 snapshot-before-resize timing contract is undisturbed. The pairing matters because xterm.js 6.0.0's `reset()` runs the `ESC c` sequence (resets terminal state) but the visible buffer + scrollback handling varies by version + renderer (DOM vs canvas vs WebGL); the user reported the doubled-scrollback symptom persisted after the initial reset()-only fix in WKWebView, and pairing with `clear()` closes that gap regardless of internal xterm behavior.

**HS-8285 follow-up (2026-05-07).** Defensive `pruneDetachedHandles(entry)` in `checkout()` (before the bump-down evaluation) and `releaseInternal()` (after the splice, before the restore-the-top reparent) drops any handle whose `mountInto` is no longer attached to the document. Pre-fix a handle whose container was destroyed without `release()` running (popup torn down by an outer error path, dashboard tile whose section re-rendered before its dispose hook fired, project tab reorder that rebuilt the strip without flushing every per-tab teardown) stayed at the top of the stack. The next legitimate `checkout()` then wrote the placeholder onto the visible surface, and any restore-on-release reparented the live xterm into a detached node — producing the "Terminal in use elsewhere" symptom on the surface the user is actually looking at. Pruning sets `_released = true` on stale handles (so a late `release()` from the original owner is a no-op) and splices them out of the stack. Three regression tests cover the bump-down avoidance, the restore-on-release walk-down, and the dispose-when-only-stale-handles-remain corner case.

**HS-8285 second follow-up (2026-05-07) — flow-mode tile-key collision.** The user reported the symptom kept recurring in flow layout mode (§25.10.5) despite the prune. Root cause was a separate registry bug in `src/client/terminalTileGrid.tsx::mountTileGrid`: the internal `tiles` / `virtState` / `virtTimers` / `virtRootToId` / `visibleTileIds` Maps all keyed entries by `entry.id` alone. In sectioned mode each handle is per-project so `entry.id` is unique within its handle, but in flow mode ONE handle holds tiles from EVERY registered project — and two projects routinely have terminals with the same id (every project starts with a `default` terminal). The second `tiles.set('default', tile)` silently overwrote the first project's entry. The orphan tile was rendered into the DOM (its `renderTile` return value was appended to the container) but absent from the registry, so on `rebuild()` / `dispose()` `teardownAll()` only iterated the survivor and the orphan's checkout was never released. After `replaceChildren()` blew away the orphan's mountInto, the handle was a stale entry on the checkout stack — reachable by the prune above, but the bug was that the orphan also leaked via the every-tile-cycle path, not just the late-release path. Fix: composite `${secret}::${id}` keying inside `mountTileGrid` (new `tileKey(secret, id)` / `tileKeyFor(entry)` helpers, all five Maps + the `data-secret` + `data-terminal-id` DOM lookups). `syncBellState`'s parameter changed from `Set<terminalId>` to `Set<tileKey>` so flow mode no longer cross-lights two projects' same-id tiles. Both dashboard and drawer-grid callers updated to build `${secret}::${id}` keys. Three new regression tests in `terminalTileGrid.test.ts` cover the rebuild + dispose paths with same-id-across-projects entries.
...
## 54.12 Device-status reply suppression during replay (HS-8610)

Switching tabs surfaced garbage like `?49;86R` / `3R3R` in the foreground program's input line. Root cause: the server's scrollback `history` frame includes whatever device-status **queries** the foreground program emitted before the disconnect — a `claude` render commonly ends with `\x1b[?6n` (DECXCPR, extended cursor-position report) or `\x1b[6n` (CPR), and color-aware programs emit `\x1b]11;?` (OSC background-color query). When `applyHistoryReplay` writes those bytes into the reused xterm, xterm's parser **auto-emits the reply** (`\x1b[?49;86R`, etc.) through `term.onData` — the exact same channel as user keystrokes — so the `createEntry` keystroke pipe sent the reply to the PTY as input. It landed in Claude's prompt as `?49;86R`, and repeated queries stacked up as `3R3R`.

Fix: a per-entry `replaying: boolean` flag (`StackEntry`). `applyHistoryReplay` sets it `true` immediately before `entry.term.write(buf, cb)` and clears it in the write callback; the `term.onData` handler does `if (entry.replaying) return;` at the top. Because xterm emits the query reply synchronously **during** the write's parse pass (before the write callback fires), holding the flag from just-before-the-write until the callback reliably drops every reply regardless of how many chunks xterm splits the buffer into. This catches **all** auto-reply query types (CPR / DECXCPR / DA / DSR / OSC color / DECRQM) without enumerating them — the user can't be typing during a synchronous programmatic replay, so nothing real is dropped. Tests: `terminalCheckout.test.ts` "device-status reply suppression during replay (HS-8610)" — the gate drops `onData` while replaying + pipes otherwise; `applyHistoryReplay` holds the flag across the write + clears it; replaying a buffer ending in `\x1b[6n` sends nothing to the PTY.
