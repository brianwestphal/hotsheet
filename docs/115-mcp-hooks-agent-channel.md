...
- **Other MCP-native agents** — if another MCP+hooks CLI agent appears, generalize `antigravity.ts`'s config-writer + `antigravityDrive.ts`'s drive into a per-agent abstraction rather than agy-specific modules.
