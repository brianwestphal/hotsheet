...
5. The sidebar cost widget is **zeroed** for the active project (`clearSidebarWidgetCostForActiveProject`) so a just-cleared project drops to $0 / hides immediately. **HS-8620:** a plain `refreshSidebarWidgetCost()` was insufficient — the HS-8531 sticky cost cache deliberately keeps showing the last value for any project the next `today-cost-by-project` fetch omits, and a just-cleared project IS omitted (zero cost today), so the stale dollar value persisted until the next new prompt arrived. The clear path now explicitly sets the active project's cached cost to 0. The analytics-dashboard telemetry section (§71) and cross-project stats page (§70) pick up the change on their next 30 s poll tick.

**HS-8621 — status reset.** The "Cleared N telemetry rows." line is one-shot and project-scoped, but `#settings-telemetry-clear-status` is a static element reused across project switches. Without a reset, switching to a different project tab and reopening Settings still showed the prior project's count. `resetClearTelemetryStatus()` (exported from `telemetryClearUI.tsx`) is now called from the Settings-dialog open handler in `settingsDialog.tsx`, so every open starts with a blank status line.
