...
- The client polls this on a 2 s tick (`commandSidebar.tsx::startShellPoll`) to drive each running button's busy spinner + to detect completion (an id leaving the list). A command's **final output** is written to its Commands Log entry's `detail` (after the `---SHELL_OUTPUT---` separator) when the process emits `close`. (HS-9185 removed the live partial-output streaming — there is no per-command partial buffer.)
