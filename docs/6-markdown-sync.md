...
**HS-8486 (2026-05-22) — detection gate changed from "AI tool's project folder exists" to "AI tool's CLI is installed on PATH."** Pre-fix the user's first launch of the AI tool ran without the Hot Sheet skill in scope: the dotfolder (`.claude` / `.cursor` / `.windsurf`) only existed AFTER the first launch, but Hot Sheet only installed skills when the dotfolder existed → race lost on first run. Post-fix `ensureSkillsForDir` probes `PATH` for the matching executable (`claude`, `cursor`, `windsurf`) and installs the skill if the binary is found, AS WELL AS the legacy folder-presence check (preserved as a fallback so projects in the old state stay covered). The Claude path also `mkdirSync`s `.claude` before writing settings — pre-fix `ensureClaudePermissions` assumed the folder existed because the legacy gate required it. **Copilot retains the folder-only gate** (no reliable executable name — GitHub Copilot lives as a VS Code extension, not a stand-alone CLI). **HS-9500 (2026-07-29) — a committed `CLAUDE.md` also counts as evidence for the Claude gate.** It always had for the INSTRUCTIONS path (`aiInstructionsTools.ts::TOOLS`), but not for skills generation, so a project with a `CLAUDE.md` and no `.claude/` — a fresh clone, or a second machine without the CLI — got its instruction file maintained while its skills were never generated. The two predicates now agree, which `skills.test.ts` asserts on one fixture in both directions (a union that detected everything would pass the positive case while being just as wrong). **Also fixed in HS-8486:** `src/projects.ts::registerProject` was calling `ensureSkills()` (which uses `process.cwd()`) instead of `ensureSkillsForDir(projectRoot)` — wrong project root in the multi-project Tauri path where CWD is the binary's start dir, not the project being opened.
...
**Orphan sweep (HS-9476).** Version tracking pushes a correction to every generated file by iterating the skills we *would* write — so it silently skips a file whose category has since been removed. Those orphans freeze at whatever the generator wrote long ago and every future fix misses them: `hs-m` (marketing, a category that no longer exists) and `hs-req-change` (superseded by `hs-requirement-change`) sat at versions 10 and 2 against a current 25, and had to be scrubbed by hand **twice** — once for a baked-in secret, again for a baked-in port. They were not inert either: Claude Code lists a skill from the file on disk, not from the category list, so `/hs-m` was still offered and would have created a ticket in a category that doesn't exist.

`ensureSkillsForDir` now deletes them before regenerating (`sweepOrphanedGeneratedSkills`), across every layout — `.claude/skills`, `.agents/skills`, `.gemini/skills`, `.cursor/rules/*.mdc`, `.github/prompts/*.prompt.md`, `.windsurf/rules/*.md`. Four constraints keep a destructive step safe:

- **Only the `hs-` prefix we generate.** `hotsheet` / `hotsheet-worker` are not category-derived and are never candidates; anything else in the directory belongs to the user.
- **Only files carrying the version header.** That header is the ownership marker — an orphan still has one (an old number, which is the whole problem), while a hand-written skill that happens to be named `hs-…` has none and is left alone.
- **Only the file, then the directory if that leaves it empty.** If the user put something else in there, it stays.
- **Only when the caller supplied categories.** `skillsState.categories` is a process-global holding whatever project set it last (the HS-8910 foot-gun); deleting files off a possibly-other-project's list is not a risk worth taking, so `ensureSkills()` and the worktree wiring generate but never sweep.

Swept regardless of the project's `ai_tool`: the HS-9311 narrowing deliberately stops *refreshing* an unselected tool's files, but an orphan is stale for every tool.
...
- **Secret in requests**: AI tools send the secret as an `X-Hotsheet-Secret` header with every API request. **HS-9475 — generated skill files no longer contain the secret or the port.** `.claude/skills` / `.cursor/rules` are committed and shared, so baking in a machine-specific value both leaked it and produced a diff conflict on every teammate's machine. The skill bodies now tell the reader to read `.hotsheet/secret.json` and `.hotsheet/settings.local.json` into shell variables instead (and prefer the `hotsheet_*` MCP tools, which need neither). `worklist.md` is gitignored, so it still carries the values directly.
