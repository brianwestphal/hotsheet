...
>
> **HS-8297 / HS-8304 (2026-05-08) — AskUserQuestion shape support.** User reported `/test-permission-yes-no` and `/test-permission-multiple-choice` (which call Claude Code's `AskUserQuestion` tool) rendered numbered-choice TUIs in the terminal that the §52 overlay never surfaced. Two parser shortcomings: (a) `NUMBERED_FOOTER_RX` only accepted `Enter to confirm` / `Esc to cancel` — AskUserQuestion's footer is `Enter to select  ↑↓ to navigate  Esc to cancel` with both a different verb (`select` vs `confirm`) AND multi-space separators (vs ` · `). New regex `^(?:Enter to (?:confirm|select)|Esc to cancel)(?:(?:\s+·\s+|\s{2,})\S.*)?$` accepts both verbs and either separator style. (b) The walk-upward in `claudeNumberedParser.match` broke at the first non-numbered row when ≥ 1 option was already captured — but AskUserQuestion renders each option's description on the line(s) below it (e.g. `1. Yes` / `   Confirm and continue with the test action.`) AND inserts blank rows between options. The walk now (1) skips blank rows during the upward sweep so blank-separated options still get captured, and (2) tracks the leftmost numbered-option indent and skips non-numbered rows whose indent is STRICTLY GREATER than that — treating them as continuation/description rows. The strict-greater-than discriminator preserves the existing dev-channels-warning + LingoGist Read-permission shapes where body lines and option markers sit at equal indent — those still break the walk into the question region as before. 7 new unit tests in `parsers.test.ts` — 4 footer-matcher cases (`Enter to select` accepted bare, full `Enter to select  ↑↓ to navigate  Esc to cancel` footer accepted, trailing-whitespace tolerated, lookalike prose rejected) + 3 end-to-end parser cases (2-option AskUserQuestion repro from HS-8297, 4-option from HS-8304, defence-in-depth re-assertion of the existing dev-channels prompt shape).
>
> **HS-8294 (2026-05-08) — post-§52-answer §47 re-mount fix.** HS-8245's gate suppressed §47 *while the AI in-terminal prompt was live*. But after the user picked Yes via §52, Claude proceeded based on the TUI keystroke without explicitly notifying the channel server's MCP `permission_request` (which has its own 2-min `PERMISSION_TTL_MS`); the next `processPermissionPollResponse` saw `liveAiPromptSecrets` empty (server-side pendingPrompt cleared) and re-mounted §47 for the already-resolved decision. The user's repro: `/test-permission-write` surfaced a second popup for `mkdir -p /tmp/claude-permission-test` after the §52 had already been answered. Fix in two pieces: (a) `permissionOverlay.tsx` tracks `lastSeenPendingBySecret: Map<string, string>` (updated every poll tick — set when `perm !== null`, deleted otherwise) + new exported `dismissChannelPermissionForSecret(secret)` which adds the recorded request_id to `dismissedRequestIds` AND tears down any currently-mounted popup. (b) `bellPoll.tsx::openCrossProjectOverlay::onSend` lazy-imports `dismissChannelPermissionForSecret(secret)` whenever an AI-parser numbered choice is picked (gate: `isAiParserId(match.parserId) && extras?.choiceIndex !== undefined` so cancel paths via Esc / X / cancel-button don't fire). The MCP request stays alive on the channel server (Claude's MCP layer typically resolves it shortly after the user's TUI choice; the 2-min TTL handles the rest) — we just stop re-mounting §47 for the same request_id. 11 new unit tests across `permissionOverlay.test.ts` (8: per-secret tracker populate/overwrite/drop/reset, dismiss adds to dismissedRequestIds + tears down popup, blocks re-mount of same request_id, allows new request_id, no-op for unknown secret, scoped to requested secret) + `bellPoll.test.ts` (2 negative cases: yesno doesn't fire dismiss, Esc cancel doesn't fire) + 1 integration test in `permissionOverlay.test.ts` that drives the real bellPoll → permissionOverlay end-to-end (positive case is intentionally tested there because vitest's mock factory in `bellPoll.test.ts` only intercepts ONE dynamic-import call site — the dispatcher's mount-time §47 dismiss — and the second `import('./permissionOverlay.js')` from `onSend` resolves to the real module).
>
> **HS-8245 (2026-05-06) — INVERTED HS-8228; §52 wins for AI prompts.** Despite HS-8228's two-way mutual exclusion, the user kept seeing two popups for the same Claude decision in practice ("fairly often I ended up seeing two separate permissions popups for effectively the same thing"). The user's verdict: when an AI tool (Claude / Codex / etc.) is detected at an in-terminal prompt, the §52 overlay is *always* the better surface — its borrow-terminal interaction sends keystrokes the AI's TUI is already listening for, so the channel popup is redundant ("we can just ignore channel based permissions prompts"). Fix in three pieces: (a) new `AI_PARSER_IDS = new Set(['claude-numbered'])` constant + `isAiParserId()` helper in `src/shared/terminalPrompt/parsers.ts` — extensible point for future Codex / Aider parsers. (b) `bellPoll.tsx::dispatchPendingPrompts` recomputes a `liveAiPromptSecrets: Set<string>` every tick from pendingPrompts (any match with `isAiParserId(match.parserId)`); exports `hasAiTerminalPromptForSecret(secret)`. The HS-8228 "skip §52 candidate when §47 owner === secret" gate is GONE — AI candidates always dispatch. When the dispatch IS for an AI parser, the dispatcher additionally lazy-imports `permissionOverlay.dismissPermissionPopupForSecret(secret)` to tear down any §47 popup mounted for the same project. (c) `permissionOverlay.tsx::showPermissionPopup` adds an early-return gate `if (hasAiTerminalPromptForSecret(secret)) return;` — suppresses §47 mount entirely while the AI prompt is live. The MCP request stays alive on the channel server; the next `processPermissionPollResponse` for-each iteration re-checks the gate (every 100 ms) and surfaces §47 naturally if the AI prompt clears without resolving the MCP side. New `dismissPermissionPopupForSecret(secret)` (replaces HS-8228's `dismissTerminalPromptOverlayForSecret` — direction inverted) tears down the popup DOM, releases the §54 checkout (so the borrowed live xterm reparents back into its previous owner per HS-8182), clears `activePopupRequestId` / `activePopupOwnerSecret` / `autoDismissMissCount` — does NOT mark the request as responded. 11 new unit tests across `bellPoll.test.ts` (5: AI-detection set tracking, AI-clears-on-server-clear, dispatcher fires dismiss-§47 on AI parser, dispatcher does NOT fire dismiss for yesno, AI detection decoupled from candidate eligibility for generic-shape) and `permissionOverlay.test.ts` (6: AI gate suppresses mount, OTHER projects still mount, mounts on next poll once AI clears, dismissPermissionPopupForSecret tears down without responding, dismiss is no-op for non-matching project / no popup, suppression effective across many ticks). The HS-8228 `dismissTerminalPromptOverlayForSecret` export + the `getActivePermissionPopupOwnerSecret` export are both deleted (no callers after the inversion). Lint cleanup as a side-effect: replaced `vi.importActual<typeof import('./...')>` patterns with `import type * as Module from './...'` to satisfy `@typescript-eslint/consistent-type-imports`.
>
> **HS-8228 (2026-05-06) — dedup with §47 channel-permission popup. SUPERSEDED BY HS-8245.** Pre-fix HS-8228 mounted two-way mutual exclusion: §52 dispatcher consulted `getActivePermissionPopupOwnerSecret()` and skipped same-secret candidates; `permissionOverlay.tsx::showPermissionPopupBody` called `dismissTerminalPromptOverlayForSecret` to tear down any active §52 overlay for the same project before mounting §47. The user reported the duplicate-popup pattern still recurred, and the inversion in HS-8245 reflects the user's clarified intent: "when an AI terminal is detected, that's the better thing to go with" — §52's borrow-terminal technique is authoritative for AI prompts.
>
> **HS-8216 (2026-05-06) — `Esc to cancel · Tab to amend` footer support.** User reported that the §52 overlay never surfaced for a Read-permission prompt in a project whose claude session was NOT MCP-connected (the §47 popup path therefore couldn't fire either, leaving the user stranded). Root cause: Claude Code 2026-05 builds emit `Esc to cancel · Tab to amend` as the trailing footer when the prompt has an "amend" affordance (e.g. a `Yes, allow reading from <dir>/ during this session` choice the user can amend with Tab to scope the allow tighter). The pre-fix parser used a hardcoded `Set<string>` of two literals (`Enter to confirm · Esc to cancel`, `Enter to confirm`) so the new footer fell through and `claudeNumberedParser.match()` returned null. Fix in `src/shared/terminalPrompt/parsers.ts` — replaced the Set with a regex `NUMBERED_FOOTER_RX = /^(?:Enter to confirm|Esc to cancel)(?:\s+·\s+\S.*)?$/` that accepts any trailing footer starting with `Enter to confirm` or `Esc to cancel` followed by an optional ` · <suffix>` clause. Specific enough that arbitrary docs/output rows almost never start with these phrases on the trailing line; future-proof against further claude-side renames of the suffix. 7 new unit tests in `parsers.test.ts` — 6 footer-matcher cases (`Esc to cancel · Tab to amend` accepted, bare `Esc to cancel` accepted, trailing-whitespace tolerated, future suffix clauses like `· Tab to expand`/`· Shift+Tab to skip` accepted, lookalike prose `Esc to cancel the operation` rejected) + 1 end-to-end regression pinning the user's exact LingoGist Read-permission prompt shape (`Do you want to proceed?` / 3 numbered choices with `❯` cursor / `Esc to cancel · Tab to amend` footer). Also closes the user's confusion about the asymmetry between projects: the Hot Sheet project showed a §47 MCP popup because that project's claude was MCP-connected; LingoGist's wasn't, and §52 was the only fallback — so when §52's parser dropped the match, no UI surfaced anywhere.
>
...
- **HS-8106 — Settings rule-row layout: 2 visual lines per rule.** Pre-fix the rule row was a single grid with parser / question / choice / date / trash on one row; long question previews collapsed to "W A..." under the line-clamp because the question column couldn't hold the multi-line preamble. Final layout (after the post-feedback refinement) — line 1 `[parser] [question single-line ellipsis]`, line 2 `[→ auto-response]` left-aligned with the row's full width to itself, with the trash button spanning both rows and `align-self: center` so it stays vertically centered regardless of how the question wraps. The created-date column was dropped per user feedback so the auto-response gets the full row width — the date is still surfaced inside the read-only inspector dialog opened by clicking the row.
- **HS-8071 — three-tier matching in `findMatchingAllowRule`.** Pre-fix the auto-allow gate matched only on `(parser_id, question_hash)`. When Claude TUI status-bar lines (`▶▶ accept edits on (shift+tab to cycle)`, `● high · /effort`, etc.) bled into the captured question region the hash drifted on a fraction of launches and the popup leaked through despite a saved rule. Fix landed in three steps: (1) every new rule records a `choice_shape` field (pipe-joined lowercase-trimmed numbered labels, or the literal `yes|no` for yesno) in `buildAllowRule`; (2) `findMatchingAllowRule` does a primary `(parser_id, question_hash)` lookup, then a `(parser_id, choice_shape)` drift-resistant fallback — so a hash-drifted match still recognises the prompt and auto-allows; (3) **back-compat preview-substring fallback (post-feedback iteration)** — for legacy rules created before the choice_shape work that were still leaking the popup ~50% of launches because they had neither a stable hash nor a `choice_shape`, a third tier matches when the rule's `question_preview` (≥ 15 chars, normalised whitespace) appears as a substring anywhere in the live capture's `questionLines` blob. Choice index is bounds-checked. Tier 3 ONLY fires for rules with no `choice_shape` so modern rules that legitimately don't match aren't silently widened. Also: removed the always-redundant Cancel button from numbered / yesno / generic overlays per user feedback — Esc still cancels via the capture-phase keyboard handler in `mountShellWithEsc` and the X-close button on the shell header dismisses without sending.
- **HS-8071 (final iteration) — strip Claude TUI input box from captured rows.** After the three-tier matcher shipped, the popup was *still* leaking through because Claude renders its own input prompt directly above the dialog modal as a 3-row block (`──── divider / ❯ <rotating placeholder text> / ──── divider`). The placeholder line varies on every launch (`Try "how does markdown.ts work?"` one render, `Try a question…` the next), per terminal width, and on Claude's own whims. That noise bled into the captured `questionLines`, shifting `question_hash` AND defeating the Tier 3 substring fallback (the original `WARNING: Loading development channels` preview no longer appeared verbatim). Fix: new pure helper `stripClaudeInputBox(rows)` in `src/shared/terminalPrompt/parsers.ts` — scans for the conservative 3-row pattern (BOTH outer rows must be `isDecorativeLine` AND the inner row must start with `❯` after optional whitespace) and removes the block in place; called from each parser's `match()` before `trimRows` so all three parsers (`claude-numbered`, `yesno`, `generic`) get the same noise-reduction. 12 new tests in `src/shared/terminalPrompt/parsers.test.ts` covering the helper directly (8) plus end-to-end through `claudeNumberedParser` (4 — same hash across rotating placeholders, dividers + ❯ stripped from `questionLines`, warning text above the input box preserved, question_preview substring stable across renders).
- **HS-8071 (2026-05-04 follow-up) — strip Claude TUI status-bar lines + add Tier 4 long-choice-label fallback.** After the input-box strip shipped, the user reported "THIS IS STILL HAPPENING" with a screenshot of the dev-channels popup whose captured rows now showed `▶▶ accept edits on (shift+tab to cycle)` and `● high · /effort` bleeding into the question region (NO input box this time — the prior strip didn't apply). Two complementary changes: **(1)** new pure helper `stripClaudeStatusBar(rows)` in `parsers.ts` filters lines matching the `(shift+tab to cycle)` mode hint OR the `● <high|medium|low|max> · /effort` indicator, called alongside `stripClaudeInputBox` in every parser's `match()`. Conservative pattern set — only strips lines containing distinctive Claude TUI signatures so a legitimate prompt that happens to mention "shift" or "/effort" survives. **(2)** added Tier 4 to `findMatchingAllowRule`: for legacy rules without `choice_shape` AND whose `question_preview` no longer appears in the live capture (Tier 3 miss), match when the rule's `choice_label` (≥ 20 chars — uniqueness floor) exactly equals `match.choices[choice_index].label`. Catches the user's real-world case where Claude updated the prompt text from "WARNING: Loading development channels" to "Experimental: inbound messages…" — same choices, same chosen label, but Tier 1/2/3 all fail. 8 new tests for `stripClaudeStatusBar` + 3 end-to-end tests through `claudeNumberedParser` showing the same hash regardless of status-bar configuration + 9 tests for the Tier 4 matcher (long-label match, uniqueness floor, label-mismatch reject, out-of-range reject, choice_shape skip, yesno skip, Tier 1+3 precedence, missing-label reject).
