...
>
> **HS-8228 (2026-05-06) — dedup with §47 channel-permission popup.** User reported seeing both the §52 terminal-prompt overlay AND the §47 channel-permission popup on screen at the same time for the same Claude prompt (terminal-prompt overlay covering the channel-permission popup). The two surfaces describe the same Claude decision when both fire on the same project: Claude's MCP `request_permission` triggers the §47 popup, and Claude's TUI text scrape (server-side scanner → bellPoll long-poll) triggers the §52 overlay. Fix is two-way mutual exclusion: (a) the §52 dispatcher in `bellPoll.tsx` consults `getActivePermissionPopupOwnerSecret()` (newly exported from `permissionOverlay.tsx`, returns `permissionState.activePopupOwnerSecret`) and skips any candidate whose secret matches — the §52 server-side `pendingPrompts` entry stays alive so a later tick (after §47 closes) can re-fire; (b) `permissionOverlay.tsx::showPermissionPopupBody` lazy-imports `dismissTerminalPromptOverlayForSecret` from `bellPoll.tsx` and tears down any active §52 overlay belonging to the same project before mounting the §47 popup, also dropping the dispatched-signature record so a re-dispatch is allowed once §47 closes. The §47 popup wins precedence because its Allow / Deny writes a real MCP response (authoritative); the §52 overlay is a visual scrape, useful as a fallback when MCP is offline. 7 new unit tests in `bellPoll.test.ts` — 3 cover the dispatcher's skip gate (same-secret suppressed / different-secret unaffected / next-tick re-fire after popup closes), 4 cover `dismissTerminalPromptOverlayForSecret` directly (matches secret tears down + clears active key, no-op when nothing open, no-op for non-matching secret, signature-cleared so re-dispatch can fire). Lazy import in `permissionOverlay.tsx` avoids the bellPoll ↔ permissionOverlay static-import cycle.
>
