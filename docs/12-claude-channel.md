...
- **PreToolUse** hook sends a "heartbeat" state signal (HS-9262 — re-arms busy the instant a tool call begins)
- **PostToolUse** hook sends a "heartbeat" state signal
- **Stop** hook sends an "idle" signal

Each heartbeat or busy signal extends a 30-second sliding timer per project. The Stop hook immediately clears the busy state without waiting for the timer.

**Spinner liveness gate (HS-9262).** When the 30-second stale timer fires for the *active* project it no longer clears busy unconditionally — it consults the PTY spinner (`busyStaleDecision`, `src/terminals/claudeSpinner.ts`). If Claude painted its spinner within `CHANNEL_BUSY_SPINNER_FRESH_MS` (8 s), the session is genuinely mid-work (a long **single** tool call keeps the spinner animating but emits no `PostToolUse` heartbeat), so busy is **sustained** and re-checked every `CHANNEL_BUSY_RECHECK_MS` (5 s) instead of a premature-off. A hard `CHANNEL_BUSY_MAX_SUSTAIN_MS` (5 min, measured from the last real heartbeat) caps the sustain so a stuck/mis-read spinner can never pin busy forever — this also gives the **stuck-on** case (a dropped `Stop` hook) a deterministic ceiling, on top of the self-heal once the spinner goes quiet. Non-active projects have no live spinner reading (the 2 s poll follows the active project), so they keep the plain 30 s clear. Verified against Claude Code **2.1.197** — all four hook events are stable in that version; no speculative version-specific fields (`background_tasks`, `PostToolBatch`, `active_time.total`) are relied on. Tests: `claudeSpinner.test.ts` (`busyStaleDecision` transition matrix) + `claude-hooks.test.ts` (four-hook install).
...
**Per-project attribution (HS-9260 → HS-9263).** The hooks live in the GLOBAL `~/.claude/settings.json` (one file, all sessions). **HS-9263** made the single global hook route correctly across *multiple Hot Sheet instances*: instead of a baked-in port + a fuzzy `$CLAUDE_PROJECT_DIR`, the hook command is a small `node` script that, at execution time, reads the serving instance's `port` from the project's own `<CLAUDE_PROJECT_DIR>/.hotsheet/settings.local.json` and the project `secret` from `secret.json`, then POSTs the **secret** (the exact identity used everywhere else) to *that* instance's heartbeat endpoint. So a project served by the §87 `HOTSHEET_HOME` test instance, or by a second `hotsheet` on another port, gets its sessions' signals to the right instance — and a port change is picked up automatically (no stale baked-in port, which also hardens the `Stop`→idle signal per HS-9262). The server resolves the project via `matchHeartbeatProject` (`src/routes/channel.ts`): **exact secret match first**, falling back to the HS-9260 **most-specific (longest-`rootDir`) prefix match** on `projectDir` for legacy hooks that predate the secret (a session in a subdir / nested / worktree project still attributes to the innermost owning project, not whichever registered first). Busy is keyed per-project throughout (`channelStore.busySecrets`); `isChannelBusy()` reads the *active* project's entry. Installing is idempotent + self-migrating (`installHeartbeatHook()` rewrites legacy baked-port `curl` hooks to the new port-less `node` form on next launch). **On a project switch** `initChannel` → `syncActiveBusyIndicator()` re-points the single-instance side state (the sidebar `busy` slot, the `mostRecentSpinnerAtMs` reading, and the 2-second spinner poll) at the new active project, so the sidebar label reflects the project you switched to instead of carrying the previous one's spinner/label. (The destructive `heartbeat-status` drain race was fixed in **HS-9261** — see the endpoint table below; the positive-liveness gate for stuck-on/premature-off shipped in **HS-9262** — see "Spinner liveness gate" above.) Tests: `channelHeartbeatMatch.test.ts` (longest-prefix match, incl. nested-project + sibling-prefix cases).
...
- Claude's raw `input_preview` JSON is formatted for readability: Bash commands show just the `command` field; other known tools (Read, WebFetch, WebSearch, Glob, NotebookRead) show their most useful field; unrecognized JSON is flattened to `key: value` lines. Truncated JSON (common for long Bash commands) still gets primary-field extraction with a trailing ellipsis (HS-6634)
- Compact green Allow (checkmark) and red Deny (X) icon buttons on the right
- Two text links at the bottom-left of the popup (HS-7266):
  - **"Minimize"** drops the popup into the owning project tab's pulsating blue dot — the dot advertises a waiting check, clicking the tab re-shows the same popup (switching to that project if needed), and the minimized popup auto-dismisses after 2 minutes so it can't linger forever (HS-6637)
  - **"No response needed"** dismisses the popup outright (use this when the user plans to respond via Claude directly) — the request stays pending server-side, the attention dot stays blue, and the popup does not re-appear until the channel server resolves the request (HS-6637)
...
The channel server auto-expires a stale pending request, but with a **two-tier** window (HS-9299). A head that is **blocking a newer queued request** expires after the short `PERMISSION_TTL_MS` = **120 s** (the single-slot wire returns only the head, so an abandoned head must not block newer requests). A **lone** pending request — nothing queued behind it — gets a much longer `PERMISSION_LONE_TTL_MS` = **15 minutes** backstop instead, so the popup the user is still deliberating on isn't torn down after a couple minutes (the reported "it vanishes if I don't answer within a few minutes"). The backstop still exists (rather than never-expire) because the channel is **not** told when the user answers in the *terminal* — only a UI response (`completePermission`), a channel disconnect (`clearAllPermissions`), or this expiry clears an entry — so a terminal-answered / abandoned lone popup would otherwise linger until manually dismissed. Expiry never sends a `deny` to Claude; it just drops the queue entry, so Claude Code's own terminal prompt keeps waiting either way.
...
| `/api/channel/heartbeat-status` | GET | Heartbeat updates after the client's `?since` cursor (`{ updates: [{ secret, state, seq }], seq }`). **HS-9261** — a non-destructive monotonic-seq RING (not a drain): each client tracks its own cursor so two tabs/windows both receive every busy/idle update (the old destructive drain let the first poller consume everyone's, leaving the other stuck "working"). A fresh client (no `since`) syncs to the latest `seq` without replaying history. |
