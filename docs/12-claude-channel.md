...
**Per-project attribution (HS-9260).** The hooks live in the GLOBAL `~/.claude/settings.json`, so every Claude session on the machine posts its `$CLAUDE_PROJECT_DIR` to the one Hot Sheet port. The server maps that dir to the owning project via `matchProjectDirToProject` (`src/routes/channel.ts`), which chooses the **most-specific (longest-`rootDir`) prefix match** — so a session in a subdirectory, or in a nested / worktree project, attributes to the innermost owning project rather than whichever was registered first (the pre-fix `Array.find` picked the first prefix match, bleeding busy state onto the wrong project). Busy is keyed per-project throughout (`channelStore.busySecrets`); `isChannelBusy()` reads the *active* project's entry. **On a project switch** `initChannel` → `syncActiveBusyIndicator()` re-points the single-instance side state (the sidebar `busy` slot, the `mostRecentSpinnerAtMs` reading, and the 2-second spinner poll) at the new active project, so the sidebar label reflects the project you switched to instead of carrying the previous one's spinner/label. (The destructive `heartbeat-status` drain race was fixed in **HS-9261** — see the endpoint table below; the positive-liveness gate for stuck-on/premature-off shipped in **HS-9262** — see "Spinner liveness gate" above.) Tests: `channelHeartbeatMatch.test.ts` (longest-prefix match, incl. nested-project + sibling-prefix cases).
