...
- **Middle-click a tab closes it (HS-9265)** — same behavior as the context-menu "Close Tab": it's guarded so the last remaining project can't be closed, and it prompts (the §37-style running-terminals confirmation) when the tab has running/non-exempt terminals. (The browser's middle-click autoscroll is suppressed on the tab.)
...
- Keyboard shortcuts for tab management:
  - Cmd/Ctrl+Shift+[ or ] — Switch to previous/next tab (works even in text fields). The matcher accepts the shifted characters `{` / `}` as well as `[` / `]`: a browser's `KeyboardEvent.key` carries the character the chord *produces*, so with Shift held these keys arrive as `{` / `}`. Matching only the unshifted form made both chords unreachable (HS-9443) — and since the Arrow variant below deliberately ignores text fields, that left no working keyboard tab switch from the draft input. Matching the character rather than the physical `e.code` is deliberate: `BracketRight` is a US-layout position that carries a different character elsewhere (measured on macOS via TIS/`UCKeyTranslate`: on German that key prints `+`/`*`, and `BracketLeft` prints `ü`/`Ü`).
    - **Keyboard-layout reach (HS-9444).** The matcher does not consult `altKey`, which is what keeps the chord alive where the brackets sit behind Option/AltGr: on **French** macOS `[` is Option+Shift+5, so `Cmd+Option+Shift+5` matches. On **German** macOS every bracket is Option WITHOUT Shift (`[`=⌥5, `]`=⌥6, `{`=⌥8, `}`=⌥9), so no keystroke produces a bracket while Shift is held and these chords have **no route at all** — combined with the Arrow variant's text-field fallthrough, a German-layout user has no keyboard tab switch from an input. Closing that needs a new layout-independent chord, not a matcher change; tracked in HS-9444.
...
- **HS-8796 (2026-06-06; column view 2026-06-16)** — the draft row renders in a dedicated host (`#new-ticket-host`) **above the batch (selected-ticket) toolbar**, not below it, so the "New ticket…" line stays at the very top even while a multi-select toolbar is showing. The host is populated for both the default list variant **and column/board view** (`syncNewTicketHost(true)`) and cleared for trash/preview/dashboard surfaces (`syncNewTicketHost` / `clearNewTicketHost` in `src/client/draftRow.tsx`). Column view originally appended its own draft row inside `#ticket-list` (which sat *below* the toolbar — the user's "the toolbar is above the new-ticket row" report); it now routes through the same host so the entry is on top in every view. The draft row is mount-once in the host across re-renders (preserves typed text + focus). **HS-8735 / HS-8734 (2026-06-06)** — the line has no gray fill and no dashed bottom border (plain input row; the toolbar below separates it), and the empty checkbox-spacer is hidden so the type badge starts near the left edge. **HS-8736 (2026-06-07)** — the decorative ○ status placeholder is dropped from the new-ticket line, and the type badge + title input are wrapped together in one subtle rounded-rectangle border (`.draft-entry` — 1px `--border`, 6px radius, no fill; `--accent` on `:focus-within`) so the line reads as a single entry control with the type pill inside it at the left, as if the badge were part of the text field. **HS-8815 (2026-06-16)** — the trailing priority/star placeholders are now `display:none` (previously dimmed-but-spacing), so the rounded entry box fills the full available width minus the row's own 16px padding instead of reserving ~56px of invisible spacers on the right.
...
- **HS-9291 (2026-07-02) — discoverability for new users.** The normally-subtle input becomes easier to notice two ways:
  - **Empty-state hero (`.draft-row-hero`).** On the main views (`open` / `up-next`) when the list is empty — the new-user moment — the input renders as a prominent accent-tinted card with an **"Add your first ticket"** heading + a **"Type a title, then press ↵ Enter"** subtext (the type badge un-dims, the input pill sits on a solid fill). It relaxes back to the subtle row the instant a ticket exists, toggled reactively off the `filteredTickets` signal (`updateDraftHero` driven by an `effect` in `ticketList.tsx`; the hero is scoped to the two main landing views so an empty category/priority sub-view stays subtle). Set initially in `createDraftRow` so a freshly-built row is correct before the effect fires.
  - **Launch spotlight pulse (`.draft-pulse`).** On each app launch where the hero is NOT showing (i.e. tickets already exist), a one-shot glow pulse plays once on the input to draw a returning user's eye (`maybePulseDraftInput` in `draftRow.tsx`, called from `app.tsx` init after the initial load; guarded to once per page load, and a no-op while the hero is up).

### 4.6 Detail Panel

- Opens when a ticket is clicked; shows full ticket details.
- **HS-9223 / HS-9293 / HS-9294 (docs/111) — Review Proof section.** Below the per-ticket telemetry block, a presence-gated **"Review Proof (N)"** block surfaces the Glassbox `.pr-notes/` review notes whose `workItemUris` reference the ticket (READ side of §110's inducement). Each note row = kind + `file:line` + summary; clicking a row expands it to the note's **actual artifacts inline** — screenshots as `<img>` (served by `GET /tickets/review-proof/artifact`, with a "not pulled (Git LFS)" fallback when a screenshot isn't LFS-pulled), text output inlined lazily, plus a per-note **"Open in Glassbox"**. Shown only when matching notes exist (no dependency on the `aiReviewNotes` toggle).
...
- **Multi-word search = union of words (HS-8646).** A query with multiple whitespace-separated words matches on the **union of those words**, not the literal phrase: the query is split on any run of whitespace, and a ticket matches when **every** term appears in **at least one** of the five searched columns (AND across terms, OR across columns), in any order or position. So `login bug` finds a ticket titled "bug in the login flow" — words that a literal-phrase ILIKE would have missed because they aren't adjacent. An all-whitespace query narrows nothing. The split rule lives in `splitSearchTerms` (`src/ticketNumber.ts`), shared by the server's `buildTicketWhereClause` + `countSearchMatchesInExcludedStatuses` and the client's `ticketMatchesSearch` so the two layers (and the §40 include-row counts) stay in sync. An **exact ticket-id** query (e.g. `HS-100`) is still treated as a single whole-id reference, not split.
...
- **Feedback-needed tickets bubble to the top (HS-9711).** Regardless of the chosen sort, any ticket whose LAST meaningful note contains `FEEDBACK NEEDED` (the §21 feedback state — the same rule behind its purple row/card border) is ranked ahead of every non-feedback ticket. When several are feedback-needed, they order among themselves by the selected sort. This is a leading `ORDER BY` term applied SERVER-SIDE (`feedbackBubblePrefix` in `src/db/tickets.ts`, wired into `getTickets` + `queryTickets` and set on by the list/column display routes only — the worklist/markdown sync keep plain order). Server-side rather than a client re-sort so it survives list-view pagination: a feedback-needed ticket that would otherwise sort past the 100-row `LIMIT` still surfaces on page 1. **Column view** groups by status client-side while preserving the server order, so the same bubbling lifts a feedback-needed card to the top of its OWN column. A live note append re-bubbles automatically: `reduceMutation` (`src/client/wsSync.ts`) routes any `notes`-carrying update through a refetch instead of an in-place patch (a placement change in-place can't express).
...
| Space | When exactly one ticket is selected, open the reader on its latest note (Details fallback) — same as the "Read Latest Note" context-menu item (§65 / HS-8830). Skipped when a `<button>`/`<a>`/role-button is focused (so Space still activates it) and when a modal/reader is open. |
...
- **Sidebar widget**: Always-visible compact widget below the stats bar showing a 7-day throughput spark chart, weekly completion count with trend arrow, and WIP count. Clicking it opens the full dashboard.
  - **Spark chart geometry (HS-9463).** The bars span the **full width of the widget**, edge to edge inside its normal 16px padding: the SVG is `width="100%"` with `preserveAspectRatio="none"`, and `src/client/sparkBars.ts::sparkBarGeometry` solves the bar width so N bars plus (N−1) gutters exactly fill the viewBox — the first bar starts at the left edge and the last ends at the right, for any number of days. It was previously a hardcoded 98px (7 bars × a 14px pitch) inside a wider sidebar, which stopped two-thirds of the way across and read as *missing data* rather than as a chart.
  - **Days with no activity draw a flat gray baseline**, not a gap. A zero-completion day previously scaled to `height: 0` — an invisible rect, i.e. a hole in the middle of the week, indistinguishable from the chart being broken. It now renders a `minHeight` bar in `currentColor` at 0.25 opacity (so it follows the theme rather than a hardcoded gray), which reads as "that day, nothing happened".
  - The same `minHeight` floors **non-empty** bars, so a day with a single completion can never render shorter than an empty one — without that floor the chart inverts at the bottom of its scale (1 of a 21 max is 0.95 units, below the 1.5-unit baseline) and would actively mislead.
  - Coverage: `src/client/sparkBars.test.ts` (10 pure cases — edge-to-edge for 1…14 days, uniform gutters, empty-day baseline, the no-inversion floor, all-zero and negative inputs) + `e2e/sidebar-sparkline.spec.ts` (rendered width equals the widget width minus its computed padding; no zero-height rects; first bar at 0 and last ending at the viewBox edge).
...
- **Built-in defaults (HS-9247)**: the standard categories ship with sensible default auto-context so a fresh project gets useful per-category guidance without any setup. Defaults are provided for the six software built-ins (`issue`, `bug`, `feature`, `requirement_change`, `task`, `investigation`) and the design/creative preset categories (`concept`, `revision`, `feedback`, `asset`, `research`, plus the marketing `design` id); product / marketing / personal-specific categories ship with no default. The constant lives in `src/autoContextDefaults.ts`.
  - **Read-time fallback, never on disk**: defaults are applied when the worklist is generated (`loadAutoContext` merges the user's saved entries over the defaults per `type:key`, via `resolveAutoContextWithDefaults`) — mirroring how categories fall back to `DEFAULT_CATEGORIES`. A user entry always overrides the default for that category; an explicit **empty-text** entry suppresses the default entirely (turns the context off). Deleting a user entry reverts to the default. **HS-9256** — a category whose **shared** entry is **locally hidden** (deleted on this machine via the §95 local delta) also does NOT fall back to the default: `loadAutoContext` reads the local layer's `hidden` list and passes those `type:key` ids to `resolveAutoContextWithDefaults` as suppressed, so the local disable isn't defeated by the built-in default re-injecting.
