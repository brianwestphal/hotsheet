...
- **Deterministic slice landed (HS-9330):** `src/acp/acpAgents.ts` — `resolveAcpAgentCommand` (`opencode` → `opencode acp`), `isAcpDrivenTool`, `buildHotsheetMcpServerEntry` (the validated stdio entry), `ACP_PROTOCOL_VERSION = 1`. Pure + unit-tested (7 tests), consumed by the still-to-build `src/acp/client.ts`.
- **Remaining for the client (HS-9330):** add the `@zed-industries/agent-client-protocol` SDK (or a thin ndjson JSON-RPC transport — the wire is simple), the connection lifecycle around `acpAgents.ts` + `acpMapping.ts`, the option-driven §47 overlay rewrite, and a live `session/prompt` → `stopReason` smoke test. The **`--print`/one-shot vs. persistent** question doesn't apply — ACP is inherently a persistent session.
