...
**Live findings:**
- **HS-9340 — delegated fs ops (SHIPPED + live-validated):** OpenCode DELEGATES the write via an `fs/write_text_file` REQUEST (`{ sessionId, path, content }`) — **regardless of the declared `fs` capability**. Pre-fix `acpClient` replied method-not-found, so the file write FAILED. Now `acpClient` routes `fs/read_text_file` (`{path}`→`{content}`) + `fs/write_text_file` (`{path,content}`→`{}`) to an injected `AcpFsHandlers` (`callbacks.fs`), and advertises the fs capability when wired. `acpDrive` supplies **`acpFs.ts::makeProjectFsHandlers(projectDir)`** — reads/writes CONFINED to the project dir (`normalizePath` realpaths the deepest existing ancestor so a symlinked root compares correctly; a path outside → JSON-RPC error, never a hang). **Live-verified: the edit now lands** (`hello.txt` written with the exact contents through the confined handler). Unit-tested (`acpFs.test.ts`, `acpClient.test.ts` fs block).
- **HS-9341 — make OpenCode ask (SHIPPED + live-validated):** OpenCode AUTO-APPROVES tools unless `permission: ask` is configured, so the overlay was never reached. There is **no per-session ACP knob** (probed live: `session/new`'s `configOptions` expose only `model` + `mode` (build/plan); `session/set_config_option` rejects a `permission` key) — permission is a config-FILE setting. Solution: **`OPENCODE_CONFIG` env** at spawn → Hot Sheet's managed `permission:{edit,bash,webfetch: ask}` config under `.hotsheet/opencode-acp.json` (`src/acp/opencodeConfig.ts::ensureOpencodeAcpConfig`), wired in `acpDrive` for the `opencode` command. **Crucially, `OPENCODE_CONFIG` is LOWER priority than the user's own project `opencode.json`** (probed: a user `permission.edit:allow` overrode our `ask`) — so it's a DEFAULT that makes the overlay work out-of-the-box while **respecting a user who wants auto-approve**, and it never touches the repo. **Full end-to-end live-validated:** `OPENCODE_CONFIG` (no repo config) → opencode asks → the `requestPermission` callback fires → the edit lands on disk. (`acp/` unit tests: `opencodeConfig.test.ts` + the `acpDrive` OPENCODE_CONFIG test.)
