...
5. **Investigation — Tauri client-cert presentation** (§112.5 / O2) — determine how the desktop WebView presents the client cert on outbound `wss://`/`https://` per platform (WKWebView / WebView2 / WebKitGTK); recommend the mechanism (native store vs. Rust-side mTLS proxy vs. picker). **DONE (HS-9306)** → Rust-side loopback mTLS proxy. **Scaffold SHIPPED (HS-9307, maintainer-accepted):** `src-tauri/src/mtls_proxy.rs` — `build_client` (reqwest rustls: presents the device cert + trusts ONLY the project CA, `tls_built_in_root_certs(false)`) + an **axum loopback (127.0.0.1:0) HTTP forwarder** + `start_mtls_proxy`/`stop_mtls_proxy` Tauri commands (wired across `lib.rs` `generate_handler!` + `build.rs` + both capability files; a per-origin registry). New Rust deps: `reqwest` (rustls-tls) + `tokio` + `axum`. Rust tests: `build_client` rejects bad PEM, loopback-URL format; `cargo check` + all 32 Rust tests green. **Deferred to HS-9309:** the WebSocket-proxy leg (`tokio-tungstenite` + a shared rustls config for `/ws/sync` + terminal WS), the client-side loopback wiring (call `start_mtls_proxy` on mount + point the origin-aware transport at the loopback URL under Tauri), and **Phase A — the real-server validation spike** (the proxy is currently UNVALIDATED against a real server; the web client is unaffected).
