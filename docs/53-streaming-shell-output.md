...
- **Twin-pre running-shell row + click-to-expand (HS-8015 follow-up #2).** Pre-fix the running-shell render emitted a single `<pre class="command-log-detail command-log-shell-partial" data-shell-partial-id>`. Two compounding bugs hid the live output: (1) `.command-log-detail` is declared LATER in `styles.scss` than `.command-log-shell-partial` and at equal specificity, so its `display: -webkit-box; -webkit-line-clamp: 3; overflow: hidden` won the cascade — the live preview was pinned to the FIRST 3 lines with no scroll, and a chatty command's most recent output was never visible. (2) `renderLogEntry`'s click handler gated expansion on `hasMore`, which was computed from the still-empty entry-detail (`displayDetail = entry.detail` = the command line itself, typically <300 chars / 1 line) — so clicking the row did nothing while the command ran. Users reported "i still dont see data coming in as it's ready" and asked to "expand WHILE it's running and just keep growing." The fix mirrors the completed-shell pattern: each running-shell row now mounts TWO partial pres (preview + full), both sharing `data-shell-partial-id` so the live writer + hydrate find both, plus a `data-shell-partial-mode` attribute (`preview` / `full`) that selects what each pre receives. The new pure helper `writePartialIntoPre(pre, partial)` reads that attribute and writes either `tailLines(stripAnsi(buffer), 3)` (preview) or the full `stripAnsi(buffer)` (full pre). The full pre carries the new `.command-log-shell-partial-full` class with `max-height: 240px; overflow-y: auto` so a chatty long-running command doesn't push every other entry off-screen. The click handler now treats `isExpandable = hasMore || isRunningShell`, so clicking a running-shell row toggles `.expanded` and the existing `.command-log-detail` ↔ `.command-log-detail-full` display swap reveals the full pre. The expanded state survives the post-completion re-render because `expandedEntryIds` is keyed on entry id. The `.command-log-shell-partial` SCSS rule lost its `max-height/overflow-y` in favour of the more-specific `.command-log-detail-full.command-log-shell-partial-full`, ending the cascade conflict. Also restored the `tailLines` helper in `src/client/stripAnsi.ts` (HS-8015 had deleted it alongside the sidebar preview).
...
- When disabled: server still buffers and `hotsheet:shell-partial-output` events still dispatch, but the lone surviving consumer (`applyShellPartialEvent` in `commandLog.tsx` — HS-8015 removed the sidebar preview) gates rendering on `state.settings.shell_streaming_enabled` and short-circuits before mutating the Commands Log `<pre>`. Re-enabling mid-run picks up at the next chunk because the server-side partial buffer is unaffected.
- **First-use toast.** `maybeFireShellStreamFirstUseToast()` exported helper in `commandSidebar.tsx`. Fires the very first time a `hotsheet:shell-partial-output` event reaches `applyShellPartialEvent` after the feature is enabled (HS-8015 moved the call site from the now-removed sidebar listener into the Commands Log live-render path so the discoverability nudge survives). Persistence via `localStorage["hotsheet:shell-stream-toast-dismissed"]` — set on first fire so subsequent events no-op. Toast text: `Shell command output now streams live in the Commands Log — Settings → Experimental to disable.` 7 s duration via the existing shared `showToast` helper. Localstorage-disabled environments still show the toast (defensive `try`/`catch`); the lack of persistence in that mode is acceptable.
