...
- The project's `appName` (falls back to the project folder name — same resolution as the project tab's label). **Clickable (HS-7943)** — clicking the heading exits the dashboard and switches to that project's normal ticket view, mirroring the HS-6832 project-tab-while-in-dashboard pattern. Hover state surfaces a `cursor: pointer` + `--accent` tinted underline. Same affordance applies to the per-tile `{ProjectName} ›` prefix in flow mode (§25.10) so the two layouts behave consistently.
...
**Terminal title left-aligned below each tile, with a right-aligned per-project stats cluster (HS-9000 left-align; HS-9056 stats cluster).** Each tile is a composite `[preview area][title label]` stack. The title row sits **below** the preview area and is a **flex row**: the name (left-aligned, `flex:1`, truncates with an ellipsis in `.terminal-dashboard-tile-label-main`) plus, on the dashboard only, a right-aligned **stats cluster** (`.terminal-dashboard-tile-stats`, `margin-left:auto`). The cluster shows the tile project's **open** ticket count (`status IN ('not_started','started')`) and **up-next** count (`up_next` and not deleted/backlog/archive) — the canonical defs from `src/db/tickets.ts` — and a small **spinner** while Claude is busy in that project. The title resolves to the same name the drawer tab would show (configured `name`, else derived basename — see `effectiveTabLabel` in §22 / §23). The runtime title from an OSC 0/2 title escape is _not_ used here — terminal-set titles can be long and per-cwd noisy, and the dashboard wants stable labels for recognition. The label row adds its own fixed height (e.g. 20 px + small margin) on top of the tile's preview area.

**Stats cluster data + responsiveness (HS-9056).** Counts come from `GET /api/projects`, whose per-project row now carries `openCount` / `upNextCount` (one `COUNT(*) FILTER (...)` pass alongside the existing `ticketCount`); they flow through `projectsStore` (`projectsByIdSignal[secret]`) and the tile reads them reactively. The busy state is `channelStore.busySecrets.has(secret)` (already cross-project + reactive). While the dashboard is open it refreshes the project list every ~20 s (`refreshProjectTabs()` on a timer set in `enterDashboard`, cleared in `exitDashboard`) so the counts stay current. The cluster is **gated on the `showProjectStats` tile-grid option** — set only by the dashboard mount, never the drawer grid (§36). The label row is a `container-type: inline-size` query container so a `@container (max-width: 250px)` rule **hides the counts text until the tile is wide enough to fit the name + counts** (the tiny spinner stays at every width). HS-9000 — the name is PRIMARY and must always show left-aligned; the counts are a "when there's room" bonus, so the threshold (raised from 150px) keeps the fixed-width counts cluster from crowding the name on smaller tiles. **Critically, the label is pinned `width: 100%`**: `container-type: inline-size` establishes size containment, which made the label STOP stretching to the tile and collapse to ~0 width (the name overflowed + the container query always read `0 < threshold`, so the counts hid and, depending on the parent's overflow, the name vanished). The `width:100%` gives the query a real width to measure and keeps the name inside the label. e2e regression guard: `terminal-dashboard-tile-label.spec.ts` asserts the label has a non-zero (≈ tile) width. Built + torn down with the tile via `installTileStatsCluster` in `terminalTileGridRender.tsx` (effect disposer in `InternalTile.statsEffectDispose`, released in `disposeTile`).
...
- **HS-8176** — a discrete range slider (`min="1" max="10" step="1"`, native `<datalist>` for browser-side integer snapping) at the far right of the header lets the user pick **how many tile columns to render**. The slider element is LTR (1 leftmost, 10 rightmost), but the user-facing column count is the inverse — `perRow = MIN + MAX - sliderPosition` per `sliderPositionToPerRow` — so the **leftmost** position renders 10 tiles per row (smallest tiles) and the **rightmost** position renders 1 big tile filling the row. Spec choice (HS-8176): *"fewer columns indicates larger terminals, and size is really what the scale is about"*. Implementation: `tileWidthFromColumnCount(perRow, rootWidth)` in `src/client/terminalDashboardSizing.ts` (pure, unit-tested):
   ```
   tileWidth = (rootWidth - (perRow - 1) * TILE_GAP) / perRow
   ```
   Tile width is allowed to fall below the pre-HS-8176 `SLIDER_MIN_TILE_WIDTH` floor — the user explicitly chose `perRow=10`, so legibility at very narrow widths is their call. Math is clamped to a 1 px positive minimum to keep CSS `width: 0` from collapsing the xterm. Pre-HS-8176 the slider was a continuous 0..100 float that mapped linearly between `SLIDER_MIN_TILE_WIDTH` (~133 px) and the full root width.
- The slider defaults to `DEFAULT_TILES_PER_ROW` (4) on first dashboard entry and **persists across reloads + relaunches** under the `/file-settings` key `dashboard_columns_per_row` (integer 1..10). The pre-HS-8176 `dashboard_slider_value` key (continuous 0..100) is migrated on read by `legacySliderValueToColumnCount` — `0` → `1`, `100` → `10`, linear in between, in-range integers passed through. `parsePersistedColumnCount(rawNew, rawLegacy)` (pure, unit-tested in `terminalDashboard.test.ts`) prefers the new key when present and falls back to the legacy key otherwise; out-of-range / non-numeric / NaN / Infinity values yield `null` so the dashboard keeps its default. Persistence is debounced 250 ms after the last `input` event so a fast drag doesn't spam `/file-settings`. Hydration is awaited on the first render path so the very first paint applies the saved scale.
- **Snap points (HS-7271 / HS-8176):** the native `<datalist>` lists ten options (1..10) so the slider thumb visually snaps to integer positions natively. Custom 1 px tick marks render below the track in lockstep — every integer position gets a tick whose tooltip reads `"N per row"`. Pure helper `computeColumnSnapPoints(rootWidth)` returns the snap-point array (one per integer, with `tileWidth` precomputed for tooltips). Per-tick `left` position uses `tickLeftPx(pctPosition, sliderWidth, thumbWidth)` after converting the slider's 1..10 LTR value to the helper's 0..100 percentage space. Pre-HS-8176 the snap points were dynamically computed based on `rootWidth` (positions where an integer number of tiles fit exactly) — with the new integer-only slider, every position is a snap, so the dynamic computation is gone along with the magnetic snap-to-nearest helper.
- If the chosen tile height overflows the viewport height, **the content area vertical-scrolls** — same behavior as before, just always driven by the slider instead of the auto-fit algorithm. This is the only scroll direction; horizontal overflow wraps tiles to new rows within the same project section.
...
**`.xterm-screen` ResizeObserver also drives the PTY-resize resync (HS-7603).** The observer doesn't just rescale the visual transform — it also re-runs the cell-metrics → term/PTY resize logic via `resyncTilePtyFromCellMetrics(tile)`. The earlier wiring fired the resize-to-native pass only from (a) the initial `requestAnimationFrame` after `term.open()` and (b) the WebSocket history-frame handler. Both could land while xterm hadn't yet committed `.xterm-screen` to the DOM — most reliably for tiles initially scrolled below the dashboard's overflow viewport, where layout was computed but `screen.offsetWidth` could read 0 in a first-frame race. `tileNativeDimsFromXterm` then returned the fallback 80×60, the term + PTY were resized to 80×60, and even after the user scrolled the tile into view xterm's first real render couldn't push the PTY back to native dims because the resync logic only ran from rAF / history. Routing the same resync through the screen ResizeObserver fixes it: the observer fires every time `.xterm-screen` changes size — including the first-paint transition from 0×0 to native — so the term + PTY snap to authoritative cell metrics on the first render that actually has them. The `resyncTilePtyFromCellMetrics` helper gates on BOTH `term.cols/rows` (current term shape) AND `tile.targetCols/Rows` (last value sent to the server) so the call is idempotent across rAF + observer + history-frame entry points: it only resizes when one of those two trails behind native, and never feeds back on itself once both are converged. Regression test: `e2e/terminal-dashboard-tile-rendering.spec.ts` — "tiles below the fold end up at native dims, not the fallback 80×60" mocks 12 alive PTYs at a tight viewport, asserts that every tile's xterm rows ≥ 50 (proxy for native ~60), and explicitly checks at least one tile is below the fold so the test is exercising the offscreen path.
...
**Top-aligned letterbox (HS-6997).** Since the HS-7097 follow-up pins the tile's xterm to tile-native 4:3 dims, uniform scale typically fills both axes of the 4:3 tile without leaving dead space. The top-align policy is retained as a safety net for edge cases where cell metrics drift enough from 4:3 to leave a tiny letterbox (font loads, exotic DPIs): the scaled xterm is **top-aligned** inside the tile so any dead space falls below the content rather than splitting top and bottom. The tile preview's background is painted with the live theme bg (HS-8059, see below) so the letterbox area reads as part of the terminal frame. Horizontal remains centered. Encoded in `computeTileScale` as `top: 0, left: Math.max(0, (tileWidth - scaledWidth) / 2)` in `src/client/terminalDashboardSizing.ts`.

**Placeholder-mode box (HS-8288).** When a tile is bumped down by a competing checkout consumer (the dedicated view, a quit-confirm preview pane, the §47 permission popup), the live xterm reparents out of the tile's `xtermRoot` and the `Terminal in use elsewhere` placeholder div takes its place. `applyTileScale` in `terminalTileGrid.tsx` clears every inline size style at the top of the function before measuring the natural size from `.xterm-screen`. Pre-fix the function then bailed (no `.xterm-screen` to measure) leaving `xtermRoot` with no inline width / height — and the placeholder's CSS `width: 100% / height: 100%` collapsed against the now-0-height parent, producing a blank tile (or a totally missing tile slot in the layout — the "0×0 px" symptom from the HS-8288 screenshots). Fix: when no `.xterm-screen` is present, snap `xtermRoot` to the tile slot dims (`position: relative; width = tileWidth; height = tileHeight`) so the placeholder fills the box just like a live xterm would. Pinned by a unit test in `terminalTileGrid.test.ts` under `terminalTileGrid — placeholder tiles keep their box (HS-8288)`.

**Cascading-refresh during initial load (HS-8288 follow-up).** A separate code path also produced an HS-8288-flavoured "tile blank with no placeholder + no entry" symptom that the placeholder-box fix above could not address. `subscribeToDefaultAppearanceChanges` (registered inside `enterDashboard`) called `refreshDashboardGrid()` whenever any project's `terminal_default` value loaded. `fetchProjectSections` calls `loadProjectDefaultAppearance(secret)` once per project; on app boot the per-secret cache is empty, so each project's first load fired a non-deduplicated change event. Pre-fix the handler's early-return only triggered when `dashboardState.lastSectionData.length > 0` — and during the initial fetch, `lastSectionData` was empty, so every event fell through to `refreshDashboardGrid()`. With four projects the dashboard tore down + restarted `renderDashboardGrid` four times in rapid succession during the initial paint; tiles were torn down mid-mount, leaving some with an empty `xtermRoot` div in `tile.preview` (no live xterm, no placeholder, no entry in the checkout map) and no path to recover. Always reproduced on the same project — typically whichever project's appearance load happened to resolve last in the cascade. Fix: skip the refresh when `lastSectionData.length === 0`. The initial paint already sees the right appearance values because `setProjectDefault` writes the per-secret cache BEFORE dispatching the event; by the time `paintDashboardSections` runs, `getProjectDefault(secret)` returns the freshly loaded value for every project. Defense in depth: `mountTileViaCheckout` in `terminalTileGrid.tsx` also detects a freshly-mounted tile whose `xtermRoot` is still empty after `checkout()` returned (the only way this happens is the silent `term.element === undefined` branch in `terminalCheckout.tsx::reparentXtermInto`) and recovers by releasing the broken handle and restoring `tile.preview` to `renderPreviewContent` so the next intersection-observer cycle / manual click triggers a fresh mount attempt.

**Theme-colored tile-preview frame (HS-8059).** `.terminal-dashboard-tile-preview`'s background is set inline (via JS) to the active terminal theme's `background` field at mount time inside `terminalTileGrid.tsx::mountTileViaCheckout`, mirroring the §22 drawer's HS-7960 treatment, the §25 dedicated view's HS-7960 treatment (above), and the §37 quit-confirm preview's HS-8058 treatment. Pre-fix the tile preview painted as `var(--bg)` (white in the default app theme) which produced a visible color seam between the frame and the xterm canvas under any non-white theme (Dracula, Solarized, etc.) — the gap between the scaled `.xterm-screen` and the preview's edges showed an app-bg gutter rather than a terminal-frame gutter. The inline bg is cleared on the placeholder transitions (`softDisposeTile` + `spawnAndEnlarge`'s "Starting…" branch) so the cold/exited card states still paint with `--bg-secondary`. The shared mount path is also used by §36's drawer-grid (different `cssPrefix` only), so `.drawer-terminal-grid-tile-preview` gets the same treatment for free. Pinned by 4 unit tests in `terminalTileGrid.test.ts` under `terminalTileGrid — preview bg cascade (HS-8059)`.
...
- Click **Back**.
- Press **Esc**. HS-7526 exception: when focus is inside the dedicated-view search input (`.terminal-dashboard-dedicated-bar .terminal-search-input`, mounted directly into the dedicated bar per HS-8341), the capture-phase Esc handler in `terminalDashboard.tsx` blurs the input and returns focus to the dedicated xterm instead of exiting. A second Esc (now that focus is on the xterm, not the input) exits as normal. The handler recovers the widget root via `dashboardState.dedicatedSearchHandle.root` rather than a fixed DOM id, since the widget now lives inside the dedicated overlay rather than the app header. This matches the user intuition that Esc from a text field "exits the field" before it "exits the view"; it also mirrors the HS-7393 blur-without-clear behavior that the app-header search and the drawer-terminal search already follow.
...
**Theme-colored body gutter (HS-7960).** `.terminal-dashboard-dedicated-body`'s background is set inline (via JS) to the active terminal theme's `background` field at dedicated-view-mount time, mirroring the §22 drawer's HS-7960 treatment. Pre-fix the 16 px frame painted as `var(--bg)` (white in the default app theme) which produced a visible color seam between the frame and the xterm canvas under any non-white theme (Dracula, Solarized, etc.). The dedicated view doesn't expose appearance changes inside its own UI, so the bg is captured once at mount and stays — no live re-apply path needed. Companion drawer fix in `terminal.tsx::reapplyAppearance` re-syncs the drawer body bg whenever the user changes appearance via the gear popover.
...
**Dedicated overlay pins to the viewport, not the dashboard's scroll content (HS-7602).** `.terminal-dashboard-dedicated` uses `position: fixed; inset: 0; z-index: 600`, not `position: absolute`. The dashboard root is a vertical scroll container (`.terminal-dashboard { overflow-y: auto; position: relative }`), and an absolutely-positioned overlay inside that root scrolls along with the content — meaning when the user had scrolled down before double-clicking a tile near the bottom, the dedicated view rendered at the top of the scrolled-up content (above the visible viewport) and the dashboard grid stayed visible underneath while the user "scrolled all the way back up to find it." Fixed positioning anchors the overlay to the viewport unconditionally, which is safe because `body.terminal-dashboard-active` hides every other surface so the dashboard occupies the full viewport for as long as the overlay can mount. The dashboard's prior scroll position is preserved automatically because we never touch it — exiting the dedicated view leaves the user looking at the same scrolled grid they double-clicked from. Regression covered by `terminal-dashboard.spec.ts` "dedicated view covers the visible viewport even when the dashboard is scrolled (HS-7602)".
...
- **Test-env fallback.** When `IntersectionObserver` is undefined (some test envs without a polyfill), the grid falls back to the pre-virtualization eager-mount behavior so unit tests don't need to install a polyfill.
...
**Eye-icon hidden-count badge (HS-7823).** When N > 0 terminals are currently hidden, a small numeric badge sits at the top-right of the eye icon (`.hide-btn-badge`, accent-colored, `99+` cap). The dashboard's badge sums hidden terminals across every project (`countHiddenAcrossAllProjects`); the drawer-grid's badge (§36.5) scopes to the active project (`countHiddenForProject`). Updates fire from the same `subscribeToHiddenChanges` subscription that re-paints the tile grid, so toggling a row in the dialog reflects on the badge instantly without re-fetching `/projects` or `/terminal/list`. Removed from the DOM entirely when the count drops back to 0.
...
**HS-8399 — defensive rAF sizing pass after re-paint.** `paintDashboardSections` already re-runs `applyAllSizing()` synchronously after the layout-mode branch (sectioned / flow) populates the new grid, on the theory that reading `container.clientWidth` would force layout. In real browsers the synchronous read can still report `clientWidth === 0` for a few frames after `replaceChildren()` + `appendChild()` — the early-bail in `applySizing` (`if (rootWidth <= 0) return;`) then leaves every tile without an inline `width`, and because `.terminal-dashboard-tile` has no CSS width fallback (width is applied inline by `applyTileSizing()` per the comment at `styles.scss:1207`), tiles collapse to content width. User-visible symptom: every tile shrinks to a tiny strip after toggling visibility in the dialog or switching the active grouping; closing + reopening the dashboard recovers because `renderDashboardGrid` re-paints against an already-attached root. Fix: queue a second `applyAllSizing()` via `requestAnimationFrame` after the synchronous pass. The rAF callback fires once the browser has settled layout, so the recovery read sees the real `clientWidth`. Idempotent + cheap; the same pattern is already used by the window-resize handler. Regression coverage: `e2e/dashboard-visibility-change-tile-sizing.spec.ts` (3 cases — single hide via the dialog, repeated hide/show stress, visibility-grouping switch via the `<select>`) plus the unit-level cases in `src/client/terminalTileGrid.test.ts`.
...
**Persistence.** The active mode is stored in `~/.hotsheet/config.json` under `dashboard.layoutMode: 'sectioned' | 'flow'` (default `'sectioned'`) — moved from the per-project `/file-settings` key under HS-8290 because the dashboard is inherently cross-project. Per-user across reloads (NOT per-project — flow is a viewing preference, not a per-project setting). User confirmation: "a" was the chosen option from §25 layout-mode question 2.

**HS-8292 — schema-enum fix.** Pre-fix `GlobalConfigSchema` (in both `src/global-config.ts` and `src/routes/validation.ts`) declared `layoutMode: z.enum(['sectioned', 'flat'])`, but the client emits `'flow'`. Every layout-toggle PATCH 400'd at the validation gate and `readGlobalConfig` fell back to `{}` when reading a stored `'flow'`, so flow mode never persisted across reloads. Both schemas now declare `z.enum(['sectioned', 'flow'])`. Regression coverage: `src/routes/dashboard.test.ts::accepts layoutMode: "flow"` and `src/global-config.test.ts::round-trips dashboard.layoutMode = "flow"` pin the validation gate + the read/write round-trip.

**Tile ordering.** Tiles flow in **registered-project order**, then in tile order within each project — the same iteration order the sectioned layout uses, just without the per-project section breaks. This preserves the user's muscle memory across mode switches. User confirmation: question 4 = "a".

**Tile labels — every tile shows the full project name (HS-7967).** Every tile in flow mode displays its label as `{ProjectName} › {TerminalName}`. Originally (HS-7662) the prefix was only on the first tile of each project's run; the visual run was supposed to carry the grouping for subsequent tiles after HS-7824 dropped the colored badge dots. The user reported back that lone first-tile prefixes didn't reliably tell them which project a given subsequent tile belonged to in practice (especially after window resizes that changed how many tiles fit per row). Always-prefix is unambiguous + symmetric, costs only a few extra characters per label.

**Slider behavior.** Identical to sectioned mode — same width-driven `tileWidthFromSlider` math, same snap-points based on tiles-per-row computed against the (now full-width) grid. Sizing logic is shared verbatim with §25.4. User confirmation: question 3 = "same behavior. sizing logic should be the same".

**Per-tile lifecycle.** Identical to sectioned mode — all tile state (mount, history replay, click-to-center, double-click-dedicated, bell indicators, lazy/exited placeholders, OSC 7 CWD chip, OSC 9 toasts) flows through the same `terminalTileGrid.tsx` module. The flow layout is just one container with one `TileGridHandle` instead of N (one per project). User confirmation: question 6 = "same behavior".

**Empty projects are hidden.** Sectioned mode shows a per-project empty-state row when a project has zero terminals (§25.10). Flow mode has no section to anchor an empty row to, so empty projects are dropped from the layout entirely — the flow grid only contains tiles. User confirmation: question 5 = "hidden entirely".
...
- Persisted slider scale (HS-7948): `/file-settings` key `dashboard_slider_value: number` (default `33`). Same cached-promise hydration pattern as `dashboard_layout_mode`; debounced 250 ms persistence on every slider `input` event. Out-of-range / non-numeric values are rejected by `parsePersistedSliderValue` so corrupt settings can't wedge the slider.
...
- SCSS: `.terminal-dashboard-tile-project` (muted-color `{Project} ›` prefix), `.terminal-dashboard-tile-name` (the bare terminal name), `.terminal-dashboard-layout-toggle` (26 × 26 muted-pill matching the §22 hide-terminal eye icon, with `.active` accent treatment). Drawer-grid parallel classes (`.drawer-terminal-grid-tile-project` / `-name`) are emitted for symmetry but currently unused by the drawer-grid module (which doesn't span projects). HS-7824 removed the previously-defined `.terminal-dashboard-tile-badge` / `.drawer-terminal-grid-tile-badge` rules.
