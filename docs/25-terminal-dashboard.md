...
**Theme-coloured body gutter (HS-7960).** `.terminal-dashboard-dedicated-body`'s background is set inline (via JS) to the active terminal theme's `background` field at dedicated-view-mount time, mirroring the §22 drawer's HS-7960 treatment. Pre-fix the 16 px frame painted as `var(--bg)` (white in the default app theme) which produced a visible colour seam between the frame and the xterm canvas under any non-white theme (Dracula, Solarized, etc.). The dedicated view doesn't expose appearance changes inside its own UI, so the bg is captured once at mount and stays — no live re-apply path needed. Companion drawer fix in `terminal.tsx::reapplyAppearance` re-syncs the drawer body bg whenever the user changes appearance via the gear popover.
...
**Slider behaviour.** Identical to sectioned mode — same width-driven `tileWidthFromSlider` math, same snap-points based on tiles-per-row computed against the (now full-width) grid. Sizing logic is shared verbatim with §25.4. User confirmation: question 3 = "same behavior. sizing logic should be the same".
