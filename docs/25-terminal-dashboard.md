...
- The slider defaults to `33` on first dashboard entry (HS-7129) and **persists across reloads + relaunches** (HS-7948) under the `/file-settings` key `dashboard_slider_value`. `parsePersistedSliderValue(raw)` (pure, unit-tested in `terminalDashboard.test.ts`) accepts the saved entry; out-of-range / non-numeric / NaN / Infinity values fall back to the `33` default so a hand-edited corrupted settings file can't wedge the dashboard. Persistence is debounced 250 ms after the last `input` event so a fast drag doesn't spam `/file-settings`. Hydration is awaited on the first render path (alongside `loadLayoutMode`) so the very first paint applies the saved scale rather than the default 33. The earlier `50` default left tiles large enough that only 1–2 fit per row on a typical laptop root width, undercutting the dashboard's "see every terminal at a glance" purpose; `33` lines up with three tiles across the common root width while still keeping xterm text readable.
- **Snap points (HS-7271):** subtle 1 px tick marks sit under the slider track at each position where an integer number of tiles fits a row exactly, accounting for `TILE_GAP` between tiles. Pure helpers `computeSliderSnapPoints(rootWidth)` + `maybeSnapSliderValue(raw, points)` in `terminalDashboardSizing.ts` drive both the tick rendering and the magnetic snap on drag. When the slider is within `SLIDER_SNAP_THRESHOLD` (2.5 slider units) of a snap, input auto-snaps to the exact value; past the threshold, intermediate positions are freely reachable. Snap positions recompute on window resize so ticks stay aligned when the layout width changes (fewer tiles fit per row in narrower windows).
