...
**Tile labels — first-of-run gets full project name, subsequent get a small badge.** The first tile of each project's run displays its label as `{ProjectName} › {TerminalName}`. Subsequent tiles in the same project's run display a small project-color badge to the left of the terminal name (so the cross-project layout reads at a glance without taking real estate for repeated project names). User confirmation: "sure a small badge on subsequent tiles is fine" (question 1).
