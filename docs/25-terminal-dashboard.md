...
**Tile labels — first-of-run gets full project name, subsequent are bare.** The first tile of each project's run displays its label as `{ProjectName} › {TerminalName}`. Subsequent tiles in the same project's run display only the bare terminal name — the run itself is what visually groups them. (HS-7824 dropped the colored badge dots that originally accompanied subsequent tiles per question 1's "sure a small badge on subsequent tiles is fine"; in practice they didn't actually clarify grouping enough to earn the visual weight.)
