...
- Windows (**HS-9027**, was an explicit v1 stub): PowerShell `Get-CimInstance Win32_Process | ForEach-Object { … }` emits `ProcessId<TAB>ParentProcessId<TAB>Name` per process, parsed by `parseWin32ProcessOutput` into the SAME `PsRow` shape so the platform-agnostic `descendantChain` / `pickForegroundProcess` are reused unchanged. `Get-CimInstance` (not the deprecated `wmic`) and `Name` runs through `normalizeComm` so `powershell.exe` / `cmd.exe` match the shell list. The highest-pid "most recent child" heuristic is weaker on Windows (pid allocation isn't strictly monotonic) but is a best-effort improvement over the prior always-prompt. Pre-HS-9027 Windows returned the safe-default unconditionally, so the quit prompt fired over-eagerly (warned even when nothing was busy).

A new server endpoint `GET /api/terminal/foreground-process?terminalId=` returns `{ command: string, isShell: boolean, isExempt: boolean, error: string | null }` for a single terminal. Aggregated cross-project in `GET /api/projects/quit-summary` so the JS can compute the prompt's terminal list with one round-trip total. The implementation lives in `src/terminals/processInspect.ts` (helpers: `normalizeComm`, `parsePsOutput`, `parseWin32ProcessOutput`, `descendantChain`, `pickForegroundProcess`, `inspectForegroundProcess`) with pure-helper unit tests in `processInspect.test.ts` + the Windows-specific `processInspectWin32.test.ts` (HS-9027 — fixture `Get-CimInstance` output → parse + foreground pick + idle/real-work/exempt cases, runnable on any host). Server-side, `getTerminalPid(secret, terminalId)` in `src/terminals/registry.ts` exposes the live PTY's pid; `listAliveTerminalsAcrossProjects()` enumerates every alive terminal across every registered project for the cross-project aggregator.
