...
**Master-detail row preview (HS-7969).** The dialog body is a two-column master-detail grid: a left list of terminal rows + project headings, and a right preview pane that swaps content as the user picks rows. Selecting a row lazy-fetches `GET /api/terminal/scrollback-preview?terminalId=<id>&maxLines=<n>` and replays the last ~30 lines into a monospaced `<pre>` (`.quit-confirm-detail-preview`). Cached per (secret, terminalId) so re-selecting a row is free; an internal token-counter guard discards out-of-order async resolutions when the user clicks faster than the network responds. The first row is auto-selected on dialog open so the preview pane shows useful content immediately. The preview pane is **painted with the terminal's resolved theme + font** — the endpoint returns `{ text, textWithAnsi, maxLines, theme, fontFamily, fontSize }` (resolution layers: terminal config → project default → null) and `applyAppearanceToPreview` looks the ids up via the existing `getThemeById` / `getFontById` registries (with `loadGoogleFont` ensuring the right face is loaded before the colour swap). The earlier (HS-7969 Phase 1) click-to-expand accordion was replaced because the row collapsing read as visually noisy and didn't honour terminal themes. Lets the user decide whether the terminal is mid-build / mid-test / sitting idle without having to dismiss the dialog and switch contexts.

**Rich (ANSI) rendering in the preview (HS-7969 follow-up #2).** Pre-fix the preview was painted via `pre.textContent = response.text`, where `text` was the ANSI-stripped scrollback. Every visual cue from coloured prompts, error highlights, syntax-coloured tool output, etc. was lost. The follow-up adds a parallel `textWithAnsi` field on the response (server-side `buildScrollbackPreviewWithAnsi` + `getTerminalScrollbackPreviewWithAnsi`) that keeps SGR / CSI sequences in the output, and a small client-side ANSI-to-safe-HTML converter (`src/client/ansiSpans.ts::ansiToSafeHtml`) renders coloured / bold / italic / underlined `<span>` markup against the resolved theme's 16-colour palette. The converter handles 30-37 / 90-97 (fg basic + bright), 40-47 / 100-107 (bg basic + bright), 38;2;R;G;B / 48;2;R;G;B (true-colour), 38;5;N / 48;5;N (256-colour, degraded to nearest basic-8), reverse video (code 7), and bold / italic / underline (codes 1/3/4). All text fragments are HTML-escaped before concatenation so assigning to `innerHTML` is safe. Non-SGR CSI / OSC / SIMPLE-ESC sequences are dropped — they're noise in a static preview.
