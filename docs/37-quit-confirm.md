...
A new server endpoint `GET /api/terminal/foreground-process?terminalId=` returns `{ command: string, isShell: boolean, isExempt: boolean, error: string | null }` for a single terminal. Aggregated cross-project in `GET /api/projects/quit-summary` so the JS can compute the prompt's terminal list with one round-trip total. The implementation lives in `src/terminals/processInspect.ts` (helpers: `parsePsOutput`, `descendantChain`, `pickForegroundProcess`, `inspectForegroundProcess`) with 22 pure-helper unit tests in `processInspect.test.ts`. Server-side, `getTerminalPid(secret, terminalId)` in `src/terminals/registry.ts` exposes the live PTY's pid; `listAliveTerminalsAcrossProjects()` enumerates every alive terminal across every registered project for the cross-project aggregator.
...
**Tauri quit-handler.** A `tauri::WindowEvent::CloseRequested` handler in `src-tauri/src/lib.rs` intercepts ⌘Q + traffic-light close + Alt+F4. The handler checks a `QuitConfirmed: AtomicBool` state — when false (first close attempt), it `api.prevent_close()` + emits a `quit-confirm-requested` event to the JS frontend. The JS side (`initQuitConfirm` in `src/client/quitConfirm.tsx`) listens via `__TAURI__.event.listen`, runs `runQuitConfirmFlow()` which fetches `/api/projects/quit-summary` and shows the dialog, then on "Quit Anyway" invokes the new `confirm_quit` Tauri command. `confirm_quit` flips the AtomicBool to true and re-issues `window.close()`; the second CloseRequested fire sees the flag set and lets the close proceed normally. The decision logic itself (`evaluateQuitDecision`) is a pure function in the same module, covered by 9 unit tests in `quitConfirm.test.ts`.
