...
- **No force-quit button.** Each step is bounded (`STEP_TIMEOUT_MS = 3 s`) and the whole pipeline by `OVERALL_TIMEOUT_MS = 8 s` (HS-8828), so a hung step can't stall indefinitely.
