...
A new server endpoint `GET /api/terminal/foreground-process?terminalId=` returns `{ command: string, isShell: boolean, isExempt: boolean }` for a single terminal. Aggregated per-project in `GET /api/terminals/quit-summary` so the JS can compute the prompt's terminal list with one round-trip per project.
...
**Tauri quit-handler.** A new `tauri::WindowEvent::CloseRequested` handler in `src-tauri/src/main.rs` intercepts ⌘Q + traffic-light close + Alt+F4. It calls into the JS side via a new `confirm_quit` IPC command that returns `'cancel' | 'proceed'`. JS side fetches `/api/terminals/quit-summary` for every active project, applies §37.5's per-project setting logic, surfaces the prompt if needed, and resolves the IPC promise.

**CLI `hotsheet --close`.** The CLI calls `POST /api/shutdown`. The shutdown route gains an optional `?confirm=true&summary=...` flow: when invoked with `--close`, the CLI first hits `/api/terminals/quit-summary`, evaluates the same logic locally, and prompts in the terminal (`y/n`) before proceeding. If the user is invoking `--close` non-interactively (CI / scripts), pass `--force` to skip.
...
- **Status:** Spec only. Implementation is HS-XXXX (filed as a follow-up).
