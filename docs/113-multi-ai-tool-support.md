...
- **A. CLI agents Hot Sheet DRIVES (the play/permission/busy loop).** Terminal agents that Hot Sheet launches + prompts + gets permission requests from. **Codex CLI, Antigravity (`agy`), OpenCode, Goose, Copilot CLI.** **⚠ REVISED (2026-07-04, HS-9310 spike):** the HS-8007 investigation assumed these speak **ACP**, and Gemini CLI was the reference. But Gemini CLI is now decommissioned; its replacement **Antigravity** speaks **MCP + Claude-style hooks, NOT ACP** — and the spike proved it drives Hot Sheet via the `hotsheet_*` MCP tools on the **same rails as Claude** (register the server in `mcp_config.json`, drive via `--print`/persistent, hooks for busy/done). So the **primary Tier-A drive path is MCP+hooks**, not ACP; ACP ([114-acp-channel.md](114-acp-channel.md)) is retained only for a genuinely-ACP agent if one appears. Claude Code stays on its current MCP-notification channel. Per-agent binary may differ from the id (Antigravity → `agy`).
