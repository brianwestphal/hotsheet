...
1. **Per-project `ai_tool` setting** (HS-8009) — `ai_tool` in `<dataDir>/settings.json`, enum `claude | codex | gemini | opencode | goose | cursor | copilot | windsurf | auto` (default `auto` = today's detect-everything). Surfaced in Settings → General. Drives:
   ...
   - **Client-state freshness of `ai_tool`** — **FIXED (HS-9406):** the hydration is **unconditional** — a project that leaves `ai_tool` unset resets state to `auto`, rather than inheriting the previously-viewed project's value (`loadSettings()` runs on every project switch via `reloadAppState()`, so "only assign when present" was a stale-carryover bug of the same class as HS-8451's frozen app title). The Settings → General dropdown also mirrors its new value into `state.settings.ai_tool` on change, so every state-reading label updates without a reload. Symptom both fixed: a Claude project's command-editor "AI agent" segmented control (HS-9364) reading "Codex". Tests: `settingsLoader.test.ts` (hydration transitions) + `e2e/command-editor-agent-label.spec.ts` (dropdown + two-project switch).
