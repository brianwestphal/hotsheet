...
**HS-8625 → HS-8874 — loaded-projects scoping via per-project fan-out.** The page shows only **currently-loaded** project data. Under **HS-8874** telemetry is stored per-project (each project's own DB; see §67.6 "Per-project storage"), so the `/telemetry/dashboard` route passes the loaded projects as `getAllProjects().map(p => ({ secret: p.secret, dataDir: p.dataDir }))` to `getDashboardPayload`, which **fans out**: for each project P it runs every rollup under `runWithTelemetryDb(P.dataDir, …)` filtered by **P's own secret**, also reads the central store, and **merges in JS** (sums window totals, groups cost-by-model by model, concats cost-by-project + cost-over-time, unions ingested dates, merges the 168-cell heatmap, concats + re-totals announcer usage). A closed project simply isn't in the list, so its DB is never read — no "Unknown project" stale rows (the HS-8622 symptom) and no "clear orphaned data" path needed. Reading each project's DB filtered by its own secret prevents the non-destructive migration (§67.6) from double-counting an un-deleted foreign row still sitting in a project's old launch-default DB. `null` projects = the ambient-context single-DB path (pre-fan-out behavior, kept for tests). (Pre-HS-8874, when telemetry was a single shared store keyed by `project_secret`, this was done via an `allowedSecrets` `project_secret IN (...)` filter on one DB.) The HS-8622 `resolveProjectName` "Unknown project (…)" fallback stays as defense-in-depth.
