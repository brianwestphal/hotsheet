...
2. **Subscription-cost disclaimer (HS-8543).** Gray rounded notice with a lucide asterisk icon and a single sentence — "For users with Claude Pro / Max / other subscriptions, the costs shown are estimates only, based on API-equivalent usage." Always shown above the chips, regardless of cost-mode state. Distinct from the HS-8497 `.telemetry-subscription-notice` accent banner that only fires in `cost_mode === 'subscription'`. Rendered via the shared `renderSubscriptionDisclaimer` helper in `src/client/telemetrySubscriptionDisclaimer.tsx`. Same helper is reused by the per-project analytics-dashboard "Claude usage" section (§71). The matching breadcrumb on the sidebar widget is a small `*` superscript appended after the dollar amount via `.sidebar-widget-cost-asterisk`.
3. **Window-total chips.** Four monospace dollar-amount tiles: Today / This week / This month / All time. Always rendered (regardless of the window selector); the selector only narrows the sections below. Each chip's meta line shows the combined real-work `tokens` + prompt count; a dimmer second line shows the **input / output split** (`"{in} in / {out} out"`, HS-8628) when token data is present.
...
4. **Cost by project.** Sortable table — Project / Cost / Tokens / Prompts / Last activity columns. Sort ascending or descending per column with a `▲` / `▼` indicator on the active sort header. Row click switches to that project and opens the analytics dashboard (which carries the per-project "Claude usage" sub-region from §71). **HS-8622 — unknown-project labeling.** `resolveProjectName(secret)` resolves the row's secret to the registered project's name (or its dataDir basename when the name is empty). Telemetry rows outlive the project that produced them, so a closed/unregistered project's secret can't resolve — those rows are labeled **"Unknown project (`<8-char prefix>`)"** rather than the bare hex prefix (which read as a "weird project name"). Clearing such a project's lingering data has no UI path today (HS-8625).
5. **Cost by model.** SVG donut + legend. Per-slice color from `MODEL_DONUT_COLORS` (cycled), one row per model with percent + absolute cost, plus a per-model meta line with the **input / output token split** and the **derived $/Mtok estimate** (`cost / tokens × 1e6` via `formatRatePerMtok`; HS-8628 — see §67's "Input vs output token split" for why it's derived rather than a hardcoded price table). Single-slice case shows a "100% — only one model used this window." caption.
...
**HS-8625 — loaded-projects scoping.** The page shows only **currently-loaded** project data. Telemetry rows outlive the project that produced them (the §67.6 shared store keyed by `project_secret`), so without scoping a closed project's stale rows linger in the totals / cost-by-project table forever (the HS-8622 "Unknown project (116305a6)" symptom). The `/telemetry/dashboard` route passes `getAllProjects().map(p => p.secret)` (the registered project tabs) as `allowedSecrets` to `getDashboardPayload`, which threads it into **every** cross-project sub-query (`getWindowTotals` ×4, `getCostByProject`, `getCostByModel`, `getHourlyActivityHeatmap`, `getCostOverTime`) via a shared `project_secret IN (...)` clause (`buildSecretsInClause` / the extended `buildProjectAndWindowClauses` in `src/db/otelQueries.ts`). So totals, donut, heatmap, and cost-over-time all agree on the loaded set — a closed project simply drops out everywhere (no "clear orphaned data" path needed; the §67.6 retention sweep eventually reaps the hidden rows). `null` allowedSecrets = every project (the pre-HS-8625 behavior, kept for tests + any non-route caller); an empty set = nothing (no project loaded). The aggregates were already computed in real time with short caching (the live PGLite queries + the HS-8572 payload cache below), satisfying the rest of the HS-8625 ask. The HS-8622 `resolveProjectName` "Unknown project (…)" fallback stays as defense-in-depth but should now rarely fire, since unresolvable (closed) projects are filtered out upstream.
...
**HS-8626 — restore the analytics dashboard's per-control inline styles when supplanting it.** The two surfaces hide the header controls by *different* mechanisms: the cross-project page hides `.header-controls` wholesale via the `body.cross-project-stats-active` CSS rule (cleared when the body class is removed on teardown), but the **analytics dashboard** hides the individual controls via inline `style.display = 'none'` on each one (the §4 `DASHBOARD_HIDDEN_IDS`). When cross-project stats supplants an *active* analytics dashboard, `showCrossProjectStatsPage` → `exitDashboardModeIfActive` renames `#dashboard-container` back to `#ticket-list` and marks the dashboard supplanted — but the analytics dashboard's leftover **inline** `display:none` on each control survives the body-class teardown, so the controls stayed hidden after returning to the normal ticket view ("toolbar controls have gone missing"). The fix calls `restoreDashboardToolbarControls()` (shared `src/client/dashboardToolbarVisibility.ts`, the single source of truth for the hide/restore + the id list, also used by `dashboardMode.tsx`'s enter/exit) inside `exitDashboardModeIfActive`. `restoreTicketList` also calls it unconditionally now as belt-and-suspenders, since its prior restore was gated on `#dashboard-container` still existing (which this path renames away).
