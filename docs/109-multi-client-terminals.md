...
4. **Client placeholder + "take control" affordance** — the non-active placeholder variant with the claim button; wire the `active-device-changed` event to flip rendering. **SHIPPED (HS-9191).**
   ...
   **Phases 3+4 implementation (HS-9191):**
   ...
   - **Deferred to follow-ups:** (a) *attach gating* — a non-active device keeps its terminal WS open + placeholdered (the resize gate prevents thrash); actually **closing** the socket to stop streaming PTY bytes to a hidden placeholder is a bandwidth optimization left as a follow-up. (b) `active-device-changed` delivery over the `/api/poll` long-poll fallback for clients not on WS.
