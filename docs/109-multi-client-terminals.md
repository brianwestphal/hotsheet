...
> **Status:** Design (HS-9167) + **fully SHIPPED** — server lease (HS-9189), server resize gate (HS-9190), client device-id/claim/render/take-control (HS-9191), two-device e2e (HS-9192). Two follow-up optimizations remain: close a non-active device's terminal WS for bandwidth (HS-9300) + deliver `active-device-changed` over the `/api/poll` fallback (HS-9301). The maintainer selected the **active-device** approach (not size-arbitration). This doc captures the design + a phased build plan; implementation is decomposed into follow-up tickets (see §109.9). Cross-refs: [54-terminal-checkout.md](54-terminal-checkout.md) (checkout/borrow + `slotPlaceholder`), [46-service-client-decoupling.md](46-service-client-decoupling.md) (per-client vs per-project state), [93-websocket-push-sync.md](93-websocket-push-sync.md) (`/ws/sync` event bus), [94-strong-remote-auth.md](94-strong-remote-auth.md) (mTLS per-client identity).
...
1. **Server active-device lease primitive** — pure lease module (claim/renew/supersede/expiry/release) + the `deviceId` resolution (Tier-1 `clientId` / Tier-0 synthetic) + the claim API (WS frame + POST fallback) + the `active-device-changed` `/ws/sync` event. Unit tests for the state machine. *(Foundation — everything else depends on it.)* **SHIPPED (HS-9189):** `src/devices/activeDeviceLease.ts` (pure `createActiveDeviceLeases` — per-secret holder map, lazy expiry, last-claim-wins, `changed` signal; singleton + `claim/release/getActiveDevice` helpers that broadcast `active-device-changed` on a real change; `startActiveDeviceLeaseSweep` 5 s unref'd expiry sweep wired in `server.ts`). Claim transport: the `/ws/sync` `claim-active`/`release-active` frames (`src/routes/wsSync.ts` — Tier-1 pins the id to the cert `clientId`; a graceful close [1000/1001] releases; an abnormal drop leaves it for the TTL) + the `POST /api/devices/active{,/release}` + `GET /api/devices/active` fallback (`src/routes/devices.ts`, typed shapes `src/api/devices.ts`). New `active-device-changed` variant in `SyncEventInputSchema` (+ `coalesce.ts` passthrough). Tests: `activeDeviceLease.test.ts` (state machine + broadcast helpers), `devices.test.ts` (HTTP contract + cert-id-wins). The `claim-active` WS frame path is exercised by the Phase 4 e2e (HS-9192).
2. **Server output/resize gating** — only the active device attaches/subscribes; `resizeTerminal` rejects non-active `deviceId`s. Tests that a non-active socket can't size the PTY. **SHIPPED (HS-9190):** the resize gate is the server half. `mayResizePty(activeDeviceId, socketDeviceId)` (pure, in `src/devices/activeDeviceLease.ts`) drops a resize frame **iff** there is a live active holder AND the socket carries a `deviceId` that differs from it — so the single-device / pre-active-model case (no holder, or a socket with no id) is never gated. The terminal WS now reads the device id off the handshake (`?device=<synthetic-uuid>` on Tier-0; the cert `clientId` overrides it on Tier-1, `src/terminals/websocket.ts`) and `handleControl`'s `resize` case consults `getActiveDevice(secret)` through the gate before touching the PTY. Tests: `activeDeviceLease.test.ts` (the pure gate transition matrix) + `websocket.test.ts` (real-server: a non-active device's resize is dropped; a device switch hands off resize control). The **attach/output gating** ("only the active device opens the terminal WS") is client-driven and lands with Phase 3 (HS-9191) — until the client sends a `deviceId` + claims active, the gate is inert (fully backward compatible).
3. **Client device identity + active claim** — `localStorage` `deviceId`, sustained-interaction debounced claim + lease renewal, and the active/non-active state driving live-vs-placeholder per terminal (reusing the §54 checkout placeholder). **SHIPPED (HS-9191).**
4. **Client placeholder + "take control" affordance** — the non-active placeholder variant with the claim button; wire the `active-device-changed` event to flip rendering. **SHIPPED (HS-9191).**

   **Phases 3+4 implementation (HS-9191):**
   - `src/client/deviceId.ts` — `getOrCreateDeviceId()`, a `localStorage`-persisted synthetic UUID (Tier-0). Sent on the terminal WS handshake as `?device=` (`terminalCheckout.tsx`); on Tier-1 the server overrides it with the cert `clientId`.
   - `src/client/activeDevice.ts` — the controller (pure `createActiveDeviceController` + a production instance). Tracks the holder (from the `active-device-changed` event + an initial `GET /api/devices/active`), computes "am I active?" (**holder is me, OR nobody holds it** — the single-device default, mirroring the server resize gate's "no holder → allow"), claims on a **debounced (750 ms) real interaction** (document `keydown`/`pointerdown`, not `window.focus`), **renews every 5 s** while it holds the lease, releases on `pagehide`, and re-reads on project switch. Claims/renews/reads go through the typed `api/devices` callers (per-project via the injected transport). `takeControl()` backs the placeholder button (immediate, last-claim-wins handoff).
   - `src/client/terminalCheckout.tsx` — a module-level `deviceActive` gate (default `true` → inert for single-device). `setDeviceActive(active)` flips **every** mounted terminal live↔placeholder through the single `renderEntryTopInto` choke (used by `checkout()` + the release-restore path); the non-active state renders a `terminal-checkout-placeholder-inactive` "Active on another device / View here" placeholder whose button fires the registered `takeControlHandler`. `handle.resize()` is also client-gated on `deviceActive` (server gate HS-9190 is the authority).
   - `src/client/wsSync.ts` — routes the `active-device-changed` frame to `onActiveDeviceChanged` (→ the controller); `app.tsx` starts the controller after `startWsSync()` and re-syncs it on project switch.
   - Tests: `deviceId.test.ts`, `activeDevice.test.ts` (pure state machine — claim/debounce/renew/supersede/free/release/resync), `wsSync.test.ts` (event routing), `terminalCheckout.test.ts` (live↔placeholder flip, take-control button, resize gate).
   - **Deferred to follow-ups:** (a) *attach gating* — a non-active device keeps its terminal WS open + placeholdered (the resize gate prevents thrash); actually **closing** the socket to stop streaming PTY bytes to a hidden placeholder is a bandwidth optimization left as a follow-up (HS-9300). (b) ~~`active-device-changed` delivery over the `/api/poll` long-poll fallback~~ **SHIPPED (HS-9301):** the poll carries only version numbers, so rather than change the poll protocol, `src/client/poll.tsx` re-reads the (idempotent) active-device holder via `resyncActiveDevice()` (→ `GET /api/devices/active`) on the poll tick while the WS is down — throttled by the pure `shouldResyncActiveDeviceOnPoll` gate (`ACTIVE_DEVICE_POLL_THROTTLE_MS = 3000`). Keeps the live↔placeholder rendering correct on the fallback; `poll.test.ts`.
5. **E2E two-device flow** — the §109.7 e2e. **SHIPPED (HS-9192):** `e2e/multi-client-terminals.spec.ts` — two browser contexts = two devices (distinct `localStorage` deviceIds) against one server/project. Covers: (1) the non-active device shows the `terminal-checkout-placeholder-inactive` "take control" placeholder while the other holds the lease; (2) clicking "take control" flips both (clicker → live xterm, previous holder → placeholder — the observable proof the superseded device stops driving); (3) a debounced *keypress* interaction (not the button) in the non-active device also claims + flips. Devices are made active/non-active click-free via `drawer_open` + `drawer_active_tab: 'terminal:default'` file-settings (a click would itself claim active); each test's active device claims explicitly (last-claim-wins) to be robust to cross-test lease leakage (the 15 s TTL). The exact **PTY-size-follows-active** handoff is pinned server-side by `src/terminals/websocket.test.ts` ("hands off resize control on a device switch") rather than e2e (`/api/terminal/list` exposes no PTY dims).
