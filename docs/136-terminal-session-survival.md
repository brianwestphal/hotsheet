...
**Status: VERIFIED in dev, enabled in the dev launcher (HS-9662).** Phases 1–4 ship behind `HOTSHEET_PTY_BROKER` (default OFF globally; the `tauri:dev` scripts now pass `=1`). Real-app confirmed 2026-08-14: killing the node server left the detached broker + PTYs alive, the fresh server re-adopted them per-project, and the client rebuilt the tabs on WS reconnect — terminals + AI sessions survived with **no** manual resume. 22 headless tests + graceful in-process fallback when the broker is unreachable. Remaining: flip the GLOBAL default-ON (HS-9665 — needs the unit-suite/e2e/CI opt-outs + a prod-packaged pass) and Windows parity + real-process e2e (HS-9666, phase 5). Phase status in §136.7. Maintainer decision (2026-08-14): terminals **must survive an accidental server death and auto-restore as tabs** when Hot Sheet comes back online, while still being torn down on **explicit** user actions (close a terminal tab, close a project tab, quit the app).
