...
**Status: feature-complete behind a gate (HS-9662).** Phases 1–4 shipped behind `HOTSHEET_PTY_BROKER` (default OFF): the detached broker owns PTYs, the node server proxies + re-adopts on restart, explicit closes/quit tear down, dynamic terminals restore, and it falls back to in-process spawn if the broker is unreachable. 21 headless tests. Remaining: enable + real-app kill-test to verify (the flip to default-ON follows a green verification), then Windows parity + e2e (phase 5). Phase status + the enable/verify recipe in §136.7. Maintainer decision (2026-08-14): terminals **must survive an accidental server death and auto-restore as tabs** when Hot Sheet comes back online, while still being torn down on **explicit** user actions (close a terminal tab, close a project tab, quit the app).
