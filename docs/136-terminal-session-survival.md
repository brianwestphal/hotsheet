...
**5. Windows parity + full e2e matrix — PENDING:** named pipe instead of a unix socket + detach semantics (platform-branch the spawn, mirror the `build_kill_command` pure-function pattern); a real-process accidental-death survival e2e, explicit-close e2e, orphan-exit e2e, and the `--replace` port-release fix (§136.6).
