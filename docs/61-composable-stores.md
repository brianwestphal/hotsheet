...
- **HS-8336** — HS-8331 follow-up: FLIP animation restoration for the default-list-view bindList path. **Shipped 2026-05-11.** New `setTicketsAnimated(tickets)` wrapper in `ticketList.tsx` captures the FLIP snapshot via `captureSnapshot()`, calls `ticketsStore.actions.setTickets(tickets)` (the kerf signal fires + the bindList reconciles + the DOM reorders all on the same synchronous call stack), then runs `flipAnimate(snapshot)` so each row gets the inverse-transform + 200 ms ease-out it had pre-HS-8331. Migrated 3 of the 4 `loadTickets` setTickets callsites: custom-view fetch, custom-view empty fallback, default-view fetch. `loadPreviewTickets` deliberately keeps the bare `setTickets(...)` (the preview path still uses the wholesale-rebuild branch in `renderTicketList`, which already has its own captureSnapshot / flipAnimate pair — migration to bindList lands in HS-8333). The renderTicketList captureSnapshot / flipAnimate pair is retained — it still drives FLIP for the column / trash / preview branches, all of which mutate DOM inside renderTicketList rather than via the bindList. No unit tests added — FLIP is a visual behaviour, `getBoundingClientRect()` returns zero rects in happy-dom, so the wrapper is exercised by the existing `setTickets` store coverage + a new entry in the manual test plan §10. 1393 / 1393 client tests pass; tsc + lint + build clean.
