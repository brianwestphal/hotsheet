...
- **HS-8326** — HS-8239 follow-up #1: `ticketList.tsx::renderTicketList` rebuild loop → `bindList(parent, filteredTicketsSignal, t => t.id, renderRow)`. Includes the column view per-column bindLists + extends the `filteredTickets` computed body with the full view / includeBacklog / includeArchive filter logic. Preserves FLIP + focus + branch logic. Risk MEDIUM-HIGH; ~1–2 sessions.
- **HS-8327** — HS-8239 follow-up #2: filter-state migration. `state.view` + `state.search` + `state.includeBacklogInSearch` + `state.includeArchiveInSearch` move into `ticketsStore.filter` via the same getter+setter pattern as `state.tickets`. ~200 callsites mechanical. Risk MEDIUM; ~1 session.
