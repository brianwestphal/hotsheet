...
**Drop candidate:** `appIcon` — the picker UI is currently disabled (`#app-icon-picker-btn` is `display:none`). Classified Local for now; a follow-up evaluates removing it entirely.
...
- **Phase 2 (follow-up):** scope-control affordances — `shared-only` (no "+ Override": appName, ticketPrefix, categories) and `local-only` (hidden/locked in the Shared view: allow-rules, Announcer) variants; relabel the lock chips accordingly.
- **Phase 3 (follow-up, one per editor):** element-level per-layer editing for `custom_views`, `custom_commands` (group-aware), `terminals`, `auto_context` — the delta model in §95.3 + the reorder rule.
- **Follow-up:** evaluate dropping `appIcon`.
