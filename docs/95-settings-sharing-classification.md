...
- **Phase 3 (decomposed, HS-9010):** element-level per-layer editing for `custom_views`, `custom_commands` (group-aware), `terminals`, `auto_context` — the delta model in §95.3 + the reorder rule. Split into sub-tickets: **HS-9010a** (delta-merge infrastructure — the backward-compatible server resolve for the 4 keys + element-id stability; the prerequisite + riskiest read-path change, safe to land in isolation as a no-op until writers exist) → **HS-9010b** (custom_views) / **HS-9010c** (custom_commands, group-aware) / **HS-9010d** (terminals) / **HS-9010e** (auto_context). Each per-editor ticket unlocks its `data-scope-complex` marker once editable.
