...
- **Phase 2 (shipped, HS-9009):** scope-control affordances — a `share` classification on the scalar fields (`shared-only`: appName, ticketPrefix — editable in Shared, read-only "shared only" in Local, no "+ Override"; `local-only`: announcer_enabled — editable in Local, read-only "local only" in Shared) + `data-scope-complex="shared-only"` (categories — locks in Local only) / `data-scope-complex="local-only"` (Announcer key+topics — lock in Shared only) panel variants, with matching lock-chip text. (HS-9157 — the Permissions/allow-rules panel was reclassified from a `data-scope-complex="local-only"` lock to a fully scope-bar-hidden machine-local-only tab.)
