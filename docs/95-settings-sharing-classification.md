...
**Resolved is READ-ONLY (HS-9127).** The Resolved view shows the effective values with their origin tags but is **not editable** — you switch to Shared or Local to change anything (the default `data-scope-complex` surfaces were already read-only in Resolved per HS-9021; HS-9127 extends that to *every* layered surface). This applies to: scoped scalar fields (`decorateField` disables them), all `data-scope-complex` panels (`lockComplexPanels` locks every variant in Resolved), and the scope-aware list/tree editors (auto_context, terminals, custom_views, custom_commands render with no add/edit/delete/reorder in Resolved). The scope-note reads "Effective values in use — read-only. Switch to Shared or Local to edit." **Exceptions** (stay editable in Resolved): machine-global settings (no layer — diagnostics, WebGL, cost mode, the global Announcer fields) and **DB-only project settings that aren't file-layer keys** — `trash_cleanup_days` / `verified_cleanup_days` write to the DB via `updateSettings`, can't be Shared/Local-layered, and so are deliberately NOT in `SCOPED_FIELDS` (locking them in Resolved would make them uneditable since Shared/Local writes misroute to a file key the server ignores for them).
...
`settingsScope.tsx` tracks the active tab (`setActiveSettingsTab`, fed by the tab-switch handler in `settingsDialog.tsx`); `HIDDEN_SCOPE_BAR_TABS = {keys, updates, plugins, devices, permissions}` drives `updateToolbar`, which adds `.scope-bar-hidden` (CSS `display:none`) on those tabs. The **Tab order** of the tail is now `…, Experimental, Announcer, Permissions, Plugins, API Keys, Updates` (HS-9119 moved API Keys before Updates; HS-9124 moved Plugins before API Keys; HS-9157 moved Permissions to just before Plugins and Announcer to just before Permissions, and made Permissions a scope-bar-hidden machine-local-only tab with a "local to this machine" note — no longer a `data-scope-complex` lock).
...
- **categories** — **Shared only.** No local overrides. **HS-9177** — since there's no Shared|Local choice to make, the Categories tab now **hides the scope bar** entirely (it's in `HIDDEN_SCOPE_BAR_TABS`) and shows a "Shared with your team" note instead of the old `data-scope-complex="shared-only"` lock; the editor is always editable and always saves shared (mirrors how Permissions became a hidden-bar machine-local tab in HS-9157, just with shared wording).
...
- **custom_commands** — Local may **hide individual shared commands** + **add local-only commands**, including **adding into a shared-defined group**. If a parent group later disappears from shared, a local child still survives (in its own local group). No order override. A per-row **shared↔local move** button (HS-9014/HS-9094) promotes a local-only command to shared or demotes a shared one to local, editing both layer files.
- **terminals** — Local may **hide individual shared terminals** + **add local-only terminals**. No order override. **HS-9209** — a per-row **shared↔local move** button (the trailing ↑/↓, mirroring custom_commands) promotes a local-only terminal into `settings.json` or demotes a shared terminal to machine-only (folding in any local override). The pure move lives in `settingsDelta.ts` (`moveArrayItemToShared`/`moveArrayItemToLocal`, generic over any `ArrayDelta` list) and is driven client-side by `settingsScopeList.tsx::moveScopedListItem` (reusable by custom_views / auto_context when their UIs adopt it).
...
For the element-level keys, the local layer holds an object `{ hidden: [id…], added: [item…], overrides: { id: partial } }`. Resolved = shared list minus `hidden`, each surviving item shallow-merged with its `overrides`, then `added` appended. The pure resolver lives in **`src/settingsDelta.ts`** (`resolveDeltaArray` / `isArrayDelta` / `ArrayDelta<T>`), wired into `readFileSettings` (`src/file-settings.ts`) for `custom_views` (idOf = `id`), `terminals` (idOf = `id`), and `auto_context` (idOf = `type:key`). It's gated on the local value being a **non-array object** (the delta shape) — a plain-array / absent local is left untouched (legacy whole-replacement + legacy stringified arrays preserved), so it's a **true no-op for non-delta values**. `custom_commands` is a nested group tree and gets its own tree-aware resolver in HS-9010c.

**HS-9210 — an EMPTY delta `{}` resolves to the shared list.** Local mode writes `{}` when you switch to it and save without edits. `{}` isn't an `isArrayDelta` (it has no `hidden`/`added`/`overrides` key), so the resolve gate is the broader `isDeltaShape` (non-array object), NOT `isArrayDelta`. Pre-fix the `{...shared, ...local}` spread left the effective value as `{}`, which read as "every shared item locally hidden / disabled". The resolvers fall back to the shared list/tree for an empty / non-delta object.

**HS-9212 — a HIDDEN item keeps its override (hide → un-hide round-trips the local customization).** The array-based editors (terminals, auto_context) recompute the delta from their *visible* list each save, which would drop a hidden item's `overrides` entry. They now keep hidden shared items (with their customized config) in a separate `hidden` set passed to `saveScopedList`; `computeArrayDelta(shared, [...visible, ...hidden], idOf, hiddenIds)`'s `forceHidden` param marks them `hidden` while still emitting their `overrides`. On load, `loadScopedList` exposes the raw `localDelta` so the editor reconstructs each hidden row from `{…shared, …override}` and re-enables to the customized config — matching how the delta-native `custom_views` editor (`customViewsLayers.ts`) already keeps `overrides` independent of `hidden`.

**HS-9211 — hiding a shared terminal no longer confirms.** Hiding a shared terminal on this machine is non-destructive (a dimmed Re-enable row appears immediately in the list below) and clearly reflected, so `handleDelete` (`terminalsSettings.tsx`) now skips the confirmation dialog (and the drawer-preview reveal) for the `hidingShared` case and applies the hide right away. The irreversible **remove** of a *local-only* terminal still confirms ("Remove Terminal?") since it can't be re-enabled.

## 95.4 Classification (DECIDED 2026-06-24)

**Local (personal / machine — never committed):** `hide_verified_column`, `sort_by`, `sort_dir`, `layout`, `notify_completed` (and `notify_permission`), `auto_order`, `shell_integration_ui`, `terminal_scrollback_bytes`, `terminal_default`, `confirm_quit_with_running_terminals` + `quit_confirm_exempt_processes`, `db_snapshot_protection`, `telemetry_enabled` + sub-toggles + `telemetry_retention_days`, plus the existing locals (`backupDir`, `port`, allow-rules, `announcer_ai_key_id`/`announcer_last_listened_at`, `detail_*`/`drawer_*`, `*_nudge_dismissed`). **Phase 1 made these default to the `local` layer** (`LOCAL_SCOPE_KEYS` in `src/file-settings.ts`). On next startup `migrateLocalScopedKeys` relocates any committed values into `settings.local.json`. **HS-9271** — the Settings → Terminal **"Default appearance"** complex panel (`terminal_default` theme/font/size) was still rendered with the *default* `data-scope-complex` variant (editable in Shared), contradicting its local key scope; it's now `data-scope-complex="local-only"` (editable in Local, read-only in Shared), so the UI matches where the value persists.
...
- **Phase 2 (shipped, HS-9009):** scope-control affordances — a `share` classification on the scalar fields (`shared-only`: appName, ticketPrefix — editable in Shared, read-only "shared only" in Local, no "+ Override"; `local-only`: announcer_enabled — editable in Local, read-only "local only" in Shared) + `data-scope-complex="local-only"` (Announcer key+topics — lock in Shared only) panel variants, with matching lock-chip text. (HS-9157 — the Permissions/allow-rules panel was reclassified from a `data-scope-complex="local-only"` lock to a fully scope-bar-hidden machine-local-only tab. **HS-9177** — likewise, the Categories panel dropped its `data-scope-complex="shared-only"` lock for a fully scope-bar-hidden *shared-only* tab with a "shared with your team" note.)
