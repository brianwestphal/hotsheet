...
`src/client/terminalCheckout.test.ts` (25 tests) covers the §54 LIFO-swap behaviour the v2 flow relies on. The previously-extant `src/client/terminalSnapshot.test.ts` (23 tests) was deleted alongside `terminalSnapshot.ts` in HS-8177. No new automated test was added for the v2 checkout flow because (a) `showPermissionPopup` is module-private and the relevant behaviour is the DOM reparenting that `terminalCheckout`'s own tests already cover, (b) the only meaningful integration is the LIFO swap which `terminalCheckout.test.ts` already exercises. Manual coverage lives in `docs/manual-test-plan.md` §12 — see "Permission popup — live-terminal checkout (HS-8171 v2)".
