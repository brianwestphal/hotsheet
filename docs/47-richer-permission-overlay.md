...
2. **Synchronous snapshot before popup mount (HS-8171).** When a truncation indicator fires, `showPermissionPopup` calls `serializeLiveTerm(secret, 'default')` synchronously before `openPermissionDialogShell(...)`. If the live xterm has visible content, the result is used as the popup's body slot directly — a `.permission-popup-mirror-xterm` container with the captured stream mounted into it in the same JS task as the popup mount. **No intermediate render of the truncated preview.** When `serializeLiveTerm` returns null (rare — fresh launch / blank live term), the popup falls back to mounting with the diff or flat-string body, then `runSnapshotIntoBody(secret, overlay, isDismissed)` fires `captureTerminalSnapshot(secret, 'default', {tempCols: 200, tempRows: 80})` async to attempt the resize-based fallback.
3. **Capture orchestration** (`terminalSnapshot.ts::captureTerminalSnapshot`):
   - **Primary path (HS-8139 v3, 2026-05-04)** — `serializeLiveTerm(secret, terminalId)` reads the live `xterm.js` buffer for the `(secret, terminalId)` entry and returns its full visible region + 1000-row scrollback as a `SnapshotResult` at the live geometry. The popup mirror xterm renders the captured stream unchanged, with native mouse-wheel scrollback. **This is what the user is actually looking at.** No PTY perturbation, no resize flicker risk, no dependency on Claude redrawing on SIGWINCH.
   - **Fallback path (legacy HS-7999, rare)** — when the live xterm buffer has no visible content (fresh launch, no Claude output yet), the snapshot falls back to a freeze + temp-resize + serialize + restore + replay dance against the entry's PTY:
     ...
     - **Serialize** — `takePausedBytes` drains the buffer; the bytes are written to an OFFSCREEN `XTerm` at 200×80 with `SerializeAddon` loaded. `serialize()` produces an escape-sequence string. (HS-8158 — guarded by `streamHasVisibleContent`; structural-only redraws bail rather than mounting a black mirror.)
     ...
     - **Wait + unfreeze** — `resumeEntryWritesAndDrain` writes the (post-resize-back) accumulated bytes to the LIVE term and clears the pause flag. The live view goes from pre-snapshot state directly to the post-resize-back state with no intermediate rendering of the wider geometry.
4. **Mirror xterm in the popup body.** When `captureTerminalSnapshot` resolves with a `SnapshotResult`, `runSnapshotIntoBody` finds the shell's `[data-role="body"]` slot, replaces its contents with a `.permission-popup-mirror-xterm` wrapper, and `mountMirrorXterm(parent, stream, cols, rows)` builds a fresh read-only `XTerm` (`disableStdin: true`) inside it at the captured geometry and writes the captured stream. Picture-perfect colour / cursor / wrap. SCSS gives the wrapper `max-width: 100%; max-height: min(540px, 60vh); overflow: auto` (HS-8139 — bumped from `360px` so long prompts have more vertical room before the user has to scroll inside the mirror xterm; capped at 60vh so the popup still fits inside the dialog shell's `max-height: 80vh` on short laptop screens). The diff body's own `max-height` was bumped in lockstep to `min(420px, 50vh)`.
...
### HS-8159 / HS-8139 v3 — live-term scrollback as the primary path

The user reported on 2026-05-04 that the popup mirror xterm was showing **different content** than the actual terminal — Claude's input box / status bar appeared in the popup while the actual terminal showed the full diff + numbered question. Root cause: the resize-based capture writes only the bytes Claude emits during the resize wait into a FRESH offscreen xterm, so when Claude has settled into a permission dialog and only redraws the input box on SIGWINCH, the offscreen result lacks the dialog/diff context entirely. Even when `streamHasVisibleContent` returned true (HS-8158 had passed), the visible content was just the input-box redraw — not what the user was looking at.

`serializeLiveTerm(secret, terminalId)` is now the **primary** path: serialize the LIVE entry's `xterm.js` buffer (visible region + 1000-row scrollback) via `SerializeAddon` and return it as a `SnapshotResult` at the live geometry. The popup mirror xterm renders the captured stream, and mouse-wheel inside the mirror lets the user scroll up through the FULL diff Claude wrote — the user's stated requirement: "lets the user scroll through in the prompt to see the entire thing being asked".

The legacy resize-based capture only fires as a fallback when the live term is itself blank (rare — fresh launch with no Claude output yet). When it does fire, both bail conditions (zero bytes / structural-only stream) now return null cleanly rather than re-attempting the live serialize that already returned null.
...
`src/client/terminalSnapshot.test.ts` (23 tests, happy-dom): pure-helper coverage for `dropBeforeSecondClear` (no-clear, single-clear, two-clears, back-to-back, end-of-buffer, partial-prefix-no-false-positive), `streamHasVisibleContent` (HS-8158 — empty / whitespace / CSI / OSC-BEL / OSC-ST / printable-survives / Unicode-whitespace), `serializeLiveTerm` (HS-8159 — no-entry-null, blank-buffer-null, scrollback-content-rendered-at-live-geometry), plus the early-return paths in `captureTerminalSnapshot` (no entry → null, blank-live-term + no-WS → null) AND the new HS-8139 v3 live-first assertion (live term has visible content → returns live serialization at LIVE geometry, NOT the temp 200×80, proving the resize path was bypassed). The interesting end-to-end test (real WS, real PTY, real SIGWINCH redraw) is filed as a Playwright follow-up — happy-dom doesn't ship `WebSocket` so the full orchestration can't run there.
