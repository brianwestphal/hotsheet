...
**Settings-outline parity (HS-7562, reworked by HS-9272).** The Edit Terminal dialog in `terminalsSettings.tsx` (the per-project terminals outline editor in Settings → Terminal) has an **Appearance** section below the Name / Command / CWD / Lazy fields.

**HS-9272 — `Default | Custom` segmented control (replaces the HS-7562 disclosure).** Appearance is now **all-or-nothing**, not per-field:
- A **`Default` | `Custom`** segmented control (`.term-appearance-segmented` / `.term-appearance-seg`) sits where the old `<details>Appearance</details>` disclosure was. It opens on **Custom** iff the entry already sets any of `theme` / `fontFamily` / `fontSize`, else **Default**.
- **Default** — the theme/font/size fields are hidden; on Save the dialog **clears** `theme` / `fontFamily` / `fontSize` from the entry, so the terminal inherits the user's **Default appearance** (§35.6, the per-machine `terminal_default` — HS-9271) and keeps tracking it.
- **Custom** — the three controls (`.term-edit-theme` / `.term-edit-font` / `.term-edit-font-size`) reveal; on Save they're written verbatim as per-terminal values. The theme dropdown **omits the `default` (app-chrome) theme** (id `DEFAULT_THEME_ID`) — choosing "Default" appearance is the segmented option, so listing a `default` theme too would be redundant.

This replaces HS-7562's always-write-verbatim behavior (which baked the project default in as a per-terminal override the user couldn't clear from the UI) with an explicit Default⇄Custom toggle. On Save the dialog dispatches a `hotsheet:terminal-config-changed` custom event with `{terminalId}` payload — `terminal.tsx`'s `initTerminal` listens and calls `reapplyAppearance(inst)` for the matching mounted instance so the live xterm re-resolves without a page reload (mirrors the `hotsheet:terminal-default-changed` flow for the project-default panel).
...
These write through to `terminal_default`. **HS-9271 — this is a PER-MACHINE setting:** `terminal_default` is in `LOCAL_SCOPE_KEYS` (`file-settings.ts`), so it persists to the gitignored `settings.local.json`, not the committed `settings.json` — each person's default theme/font/size is their own, not imposed on the team. The panel is therefore `data-scope-complex="local-only"` (editable in **Local** mode, read-only in Shared), matching where the value goes. Changing the default immediately re-resolves every live terminal's appearance (the client dispatches a `hotsheet:terminal-default-changed` custom event that each mounted xterm instance listens for).

**HS-8283 — per-project default cache.** The cache is keyed by project secret (`projectDefaultsBySecret: Map<string, Partial<TerminalAppearance>>` in `terminalAppearance.ts`). The Terminal Dashboard shows tiles for terminals across every open project simultaneously; pre-fix every tile resolved against a single shared `projectDefault` cache (which only ever held the active project's value), so adding a new project folder rewrote the cache to the new project's empty default and the dashboard re-rendered every tile across every project against the wrong default — producing "all my terminals temporarily revert to default coloring." Now: `setProjectDefault(secret, next)` writes per secret and dedups identical writes; the change event carries the changed secret in its detail; `getProjectDefault(secret)` looks up by secret. The dashboard's `fetchProjectSections` calls `loadProjectDefaultAppearance(secret)` once per project so each section's tiles resolve against their own project's cached default. Subscribers in `terminal.tsx` (drawer) and `terminalDashboard.tsx` filter on the changed secret before doing any expensive re-render work.
