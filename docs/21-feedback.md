...
- **HS-9289 — auto-generated SYSTEM notes don't count as "the most recent note."** The claim-lease-reclaim note (`Claim lease expired — reclaimed from \`…\`.`, appended by `src/db/claims.ts` when a stale worker lease is reclaimed) is a status note, not a response. The three feedback detectors skip **trailing** system notes so one appended *after* a `FEEDBACK NEEDED` note doesn't falsely clear the pending state (the ticket kept looking like it wasn't waiting). The predicate + shared text live in the dependency-free `src/systemNotes.ts` (`isSystemStatusNote` / `buildClaimReclaimNote` / `lastMeaningfulNoteIndex`), used by `getTicketFeedbackState` (`feedbackDialog.tsx`), `notesEndWithFeedback` (`feedback-state.ts`), and the Announcer's last-note check (`collectSignals.ts`). System notes also render **dimmed** in the detail panel (the `.note-system` class) and are not narrated by the Announcer.
