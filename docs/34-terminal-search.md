...
2. **Dashboard dedicated view (HS-8341).** The search widget mounts directly into the dedicated-view top bar (`.terminal-dashboard-dedicated-bar`), right-aligned via `margin-left: auto` on the `.terminal-search-box` child rule so it sits opposite the Back button + breadcrumb label. Pre-HS-8341 it mounted into a `#terminal-dashboard-search-slot` slot in the app-header, but the dedicated overlay is `position: fixed; inset: 0; z-index: 600` and always covered the header — the user never saw the widget. The slot has been removed.
...
- `enterDedicatedView(tile, ...)` in `src/client/terminalDashboard.tsx` invokes the `onDedicatedBarMount(bar, entry, term)` callback wired by `buildFlowDedicatedBarMount` / `buildSectionedDedicatedBarMount`. Each callback calls `attachDedicatedBarSearch(bar, term, entry.label)`, which loads a `SearchAddon` onto the live xterm, calls `mountTerminalSearch(term, addon)`, and appends the returned `handle.root` directly to the dedicated bar (HS-8341). The handle is stashed on `dashboardState.dedicatedSearchHandle` so the §25.8 Esc handler can detect focus inside it.
- `exitDedicatedView()` invokes the disposer returned from the bar-mount callback, which calls `handle.dispose()` (clears `onDidChangeResults` + `clearDecorations`) and removes the widget root from the bar — order matters: dispose BEFORE term.dispose so the addon subscription doesn't race with xterm disposal.
- `teardownAllHandles()` (called from `exitDashboard` via the project-tab / toggle-click / Esc paths) clears `dedicatedSearchHandle` defensively in case a handle was torn down out of band.
