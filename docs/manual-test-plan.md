...
- [ ] **Custom commands (HS-9324) — same UX on the command tree.** Settings → Commands: click a **top-level** command or group row (not its edit/delete/move buttons, drag handle, or a group's editable name) → it tints + Copy flips to **"Copy Selected"**; Cmd/Ctrl + Shift multi-select as above. A **selected group copies whole** (its children come with it). Copy with nothing selected copies the entire tree. **Child rows are NOT selectable** (only top-level). Editing/reordering rows still works with a selection active.
