...
- [ ] **HS-8332 column-view in-place edits leave card DOM stale (known limitation; HS-8335 will fix).** With a ticket card in column view, click the category badge → pick a new category. The category badge color / label on the SAME card stays the OLD value until a status change (which moves the card to a different column = bindList recreates it) or a column-view full rebuild (layout toggle, view change, hide_verified_column toggle, drag-drop reorder). Same regression class as HS-8331's list-view bindList — both fixed by HS-8335's per-row class-flip effects.
...
- [ ] **Drawer-grid dedicated view** — open the drawer-grid for a project (toolbar toggle, ≥2 terminals required). Double-click a tile. Same dedicated-view behaviour — fits the drawer-grid's dedicated body. Click Back; grid restores cleanly.
