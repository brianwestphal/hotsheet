...
- [ ] **HS-9036 — worker permission prompts surface.** Launch a **freshly-relaunched** git-worktree worker (`/hotsheet-worker` via the worker pool / Auto switch — workers started before this fix won't have the flag). When the worker's Claude hits a tool-permission prompt, it appears in the **main Hot Sheet** permission popup (NOT only in the worker's terminal). **Allow/Deny** from the popup actually answers the worker's prompt (the response routes back to the worker's channel server, not the main agent's), and the worker proceeds. Confirm with the main agent ALSO running so two channel servers are alive. **Root cause (the real fix):** the worker launch command now includes `--dangerously-load-development-channels server:hotsheet-channel-<slug>` (`workerLaunchCommand`) — that flag is what opts Claude into routing permission prompts to the channel server; pre-fix the bare `claude "/hotsheet-worker"` connected to the channel MCP for tools but prompted permissions in the terminal. (The HS-9036 multi-server polling was necessary but not sufficient on its own.)
