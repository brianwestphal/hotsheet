...
- [ ] **HS-8335 column-view per-row reactivity.** Same drill as the list-view test but on a column-card. Toggle star, change category, change priority — all update in place. Status changes still move the card cross-column (correct behaviour — different per-column bindList).
