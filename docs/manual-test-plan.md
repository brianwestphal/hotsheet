...
- [ ] **HS-9198 — claim-before-work enforcement.** (a) **Owner editing is frictionless:** with no workers, edit any ticket in the UI / via the main `/hotsheet` agent — it just works (the server auto-claims it as `owner` transparently). (b) **A fresh worker can't stomp the owner's ticket:** edit a ticket in the owner UI (so the owner holds the lease), then in a **freshly-relaunched** worker have it `hotsheet_update_ticket` that same ticket → it gets a **409 "held by `owner`"** and should pick another. (c) **A worker works its own claimed ticket fine:** a relaunched worker `claim_next`s a ticket then `update_ticket`s it (status/notes) with no conflict (its channel server injects `basename(cwd)` as the actor, matching its claim id). (d) **The auto-claim reminder:** the first `update_ticket` on an unclaimed ticket returns a `claim` block reminding the agent to renew/release. **CAVEAT:** workers launched **before** this change run an old channel server that doesn't inject the actor header — they will 409 on their **own** claimed tickets. **Relaunch workers** after updating (like HS-9036).
