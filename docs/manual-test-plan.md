...
- [ ] **Model-B: terminal owns the thread, drive discovers it (HS-9428/9429, docs/129).** Enable the gate: launch Hot Sheet with `HOTSHEET_CODEX_DISCOVER_THREAD=1`. Open a codex `{{aiCommand}}` AI terminal → it launches **`codex --remote 'unix://…' -C '<projectDir>'`** (a fresh daemon-hosted session, NOT plain `codex` and NOT `codex resume <id>`) — even BEFORE any play (the spawn awaits the daemon coming up, `codexTerminalNeedsDaemonEnsure`). Now click play → the drive DISCOVERS this terminal's live thread by cwd (`thread/loaded/list` ∩ `thread/list{cwd}`) and the turn renders **in your terminal**, no rejoin chip. Type a prompt yourself → it shares the same thread + transcript. With the gate OFF (default), everything reverts to model-A (§HS-9394 attach + rejoin chip above). If the daemon can't start, the terminal falls back to plain `codex`.
...
- **Local-only LIVE agent tests** (skip when the tool/daemon isn't present, so CI skips them; run automatically on a dev machine with the tool installed): codex model-B discovery against the REAL app-server daemon — fresh `thread/start` → `thread/loaded/list`+`thread/read` → discovered by cwd (`src/codexModelBLive.test.ts`, HS-9431, cost-free); OpenCode ACP `initialize` handshake against the REAL `opencode acp` binary asserting ACP protocol v1 (`src/acp/opencodeAcpLive.test.ts`, HS-9432, cost-free). Pattern for the remaining agent live tests (agy/gemini) is HS-9432.
