...
- [ ] **Codex reattach chip (HS-9397, docs/123 §123.8).** Open a codex terminal BEFORE any driven turn (launches plain `codex`), then click play and let the turn finish. Reopen/refresh the drawer → the terminal pane header shows an accent **"↻ Rejoin codex"** pill. Click → confirm dialog → the terminal restarts into `codex resume … --remote` showing the driven history; the pill disappears. Also: reset the thread (new session) + run a play turn → an attached terminal's pill reappears offering the new thread. The pill must NOT appear on non-codex terminals or after an unrelated terminal-config edit.
- [ ] **Codex terminal auto-attach (HS-9394, docs/123).** With the drive enabled and at least one driven turn done (rollout exists), open a NEW codex AI terminal → it launches `codex resume <threadId> --remote …` and shows the driven history; click play → the turn renders live in that terminal. Type a prompt in the terminal mid-driven-turn → it queues after the driven turn, shares the conversation, appears in the Commands Log transcript, and does NOT flip Hot Sheet's busy/done. With the drive toggle OFF, or before any driven turn, the terminal launches plain `codex`.
