...
- [ ] **AI-suggest N (HS-8963):** with several Up Next tickets queued + an Anthropic key configured (Settings → API keys), click **"AI: suggest"** → a dialog shows a recommended worker count + one-line rationale; accepting sets the target-N (and the pool reconciles to it). With NO key, the button still returns a heuristic estimate labeled "(no AI key — estimated)". With nothing unblocked, it reports there's nothing to work.
