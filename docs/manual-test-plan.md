...
- [ ] **QR pairing (HS-9026 + HS-9033), full device path:** Settings → Remote Access → **Pair a Device…** → confirm the reachable URL → a QR shows with a live countdown. On a phone/tablet **reaching the server over a trusted/tunnel channel** (the exposed mTLS port rejects the unenrolled device at the handshake), open **`/pair`** and **scan the QR with the in-page camera** (`BarcodeDetector`); the device generates its key + CSR in-browser, enrolls over the pairing token, and offers the `.p12` to download. Then **install the `.p12`** as a client cert and confirm `https://<host-ip>:4174` connects over mTLS and the device appears in the list. (Automated now: the `/pair` page, the paste-the-code path, in-browser keygen/CSR/`.p12` assembly, and the enroll round-trip — `e2e/pair.spec.ts` + `src/client/pairing/devicePairing.test.ts`. This manual item is specifically the **camera scan** and the **platform-specific client-cert install** — iOS profile / Android / desktop keychain / Firefox store — and the real mTLS connect afterward.)
