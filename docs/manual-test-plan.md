...
- [ ] **HS-8735 / HS-8734 new-ticket line appearance.** The new-ticket line has **no gray background** and **no dashed bottom border** (it reads as a plain input row; the toolbar's own background separates them), and there is **no extraneous leading space** before the decorative ○ circle + the type badge (the empty checkbox column is gone, so they start near the left edge).
