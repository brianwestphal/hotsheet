...
- [ ] **Play button drives agy (HS-9321).** With `ai_tool=antigravity` + the channel enabled, the sidebar **play button** shows **without** a "Claude not connected" warning. Click it → Hot Sheet spawns a one-shot `agy --print "<worklist prompt>"` in the project dir; agy works the Up Next tickets and calls `hotsheet_signal_done`, which clears the busy indicator. (Known limitation: with no per-run heartbeats, the busy indicator's 60s fallback clears early on a long agy run even though agy keeps working — cosmetic; tracked as a follow-up.)
