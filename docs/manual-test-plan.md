...
*(Every seam below is unit-tested AND module-level live-validated against real `opencode acp` — driving one turn through `acpClient`/`acpFs`/`acpAutoAllow`. This checklist is the one remaining piece: exercising the full flow through the running server + browser UI together, which can't be automated headlessly.)*
