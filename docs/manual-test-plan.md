...
*(Pure delta logic is unit-tested in `src/settingsDelta.test.ts` + `src/file-settings.test.ts`; the terminals load→hide→re-enable flow in `src/client/terminalsHideOverride.test.ts` and the sibling **auto-context** editor's load→re-enable / load→disable flow in `src/client/autoContextHideOverride.test.ts` (HS-9219). The manual checks below still verify the real Tauri Settings dialog + `settings.local.json` on disk end-to-end.)*
...
### Copy-selected auto-context entries + custom commands (HS-9323 / HS-9324)
*(The pure selection state machine — plain/Cmd/Shift click, label, ids-to-copy — is unit-tested in `src/client/settingsCopySelection.test.ts`; the command-tree DOM wiring in `src/client/commandEditor.test.ts` (HS-9324). These manual checks verify the real Settings dialog + the OS clipboard, which can't be automated headlessly.)*
- [ ] **Copy All is the default.** Settings → Context with ≥2 auto-context entries, nothing selected: the button reads **"Copy All"**; clicking it copies **every** entry (paste elsewhere to confirm).
- [ ] **Click to select → Copy Selected.** Click an entry's **header** (the "Category: …"/"Tag: …" area, NOT the textarea): the row gets an accent tint and the button flips to **"Copy Selected"**. Copy → only that entry lands on the clipboard. Clicking the **same** selected header again clears it (button returns to "Copy All").
- [ ] **Multi-select.** **Cmd-click** (macOS) / **Ctrl-click** (Win/Linux) additional headers to toggle each in/out; **Shift-click** selects a contiguous range from the last click. Copy → exactly the highlighted subset, in list order. Editing a textarea while rows are selected still works (typing doesn't change the selection).
- [ ] **Selection survives edits / prunes on delete.** With some rows selected, delete a selected entry → it drops from the selection (button/label stay correct); switching the shared/local scope toggle keeps only still-visible selections.
- [ ] **Custom commands (HS-9324) — same UX on the command tree, incl. children.** Settings → Commands: click a command, group, **or a child command** row (not its edit/delete/move buttons, drag handle, or a group's editable name) → it tints + Copy flips to **"Copy Selected"**; Cmd/Ctrl + Shift multi-select as above. Copy with nothing selected copies the entire tree.
...
- [ ] **Play button drives agy (HS-9321).** With `ai_tool=antigravity` + the channel enabled, the sidebar **play button** shows **without** a "Claude not connected" warning. Click it → Hot Sheet spawns a one-shot `agy --print "<worklist prompt>"` in the project dir; agy works the Up Next tickets and calls `hotsheet_signal_done`, which clears the busy indicator. Long runs stay busy (periodic heartbeats, HS-9327).
- [ ] **Interactive permissions (HS-9327) — CRITICAL headless-unverified check.** Enable **Settings → General → "Interactive permission prompts (Antigravity)"** (HS-9328; the checkbox shows only when the AI-tool dropdown = Antigravity), which sets `antigravity_interactive_permissions: true` + regenerates `.agents/hooks.json` (a `PreToolUse` hook → `<cli> __agy-permission-hook`) (a `PreToolUse` hook → `<cli> __agy-permission-hook`). **Trust the workspace in agy.** Press play → the drive now spawns `agy` WITHOUT `--dangerously-skip-permissions`. On agy's **first tool call**, confirm: (1) the **§47 permission popup appears** in Hot Sheet; (2) **Allow** → agy proceeds; **Deny** → agy blocks that tool. If instead agy **hangs** on the first tool with no popup, the hook isn't firing / the decision protocol differs — report it (the format is documented but couldn't be confirmed headlessly: agy's `--print` returns empty in test setups). This is the one piece of the Antigravity integration that needs a real on-device run to sign off.

### Codex (MCP+hooks) integration end-to-end (HS-9369/9359) — needs `codex` on PATH + a signed-in ChatGPT/Codex account
*(The drive + MCP registration + hook mechanics were live-validated headlessly — real `codex exec --json` runs made `hotsheet_*` `mcp_tool_call`s, a PreToolUse deny blocked a command, a PermissionRequest allow un-cancelled an MCP call, and the `__codex-permission-hook` CLI emitted correct shapes. This checklist is the full through-the-UI flow.)*

- [ ] **Setup writes the config.** Set Settings → General → AI tool = **Codex** (`codex` on PATH). Confirm `~/.codex/config.toml` gains `[mcp_servers.hotsheet-channel]` (the channel command, no `--data-dir`), and the project gets the `AGENTS.md` adapter + `.agents/skills` (HS-9366).
- [ ] **Play button drives the persistent app-server session (HS-9383, docs/121).** With `ai_tool=codex`, click play → no "Claude is not connected" error (the HS-9364 reopen); a `codex app-server` child spawns, the worklist turn runs, the busy indicator stays lit (heartbeats + `thread/status/changed`), the Commands Log shows the trigger + a "Codex finished" done entry, and busy clears on completion. `<dataDir>/codex-app-server.json` now holds the thread id.
- [ ] **Session continuity (O3).** Press play again → the SAME thread continues (`thread/resume` semantics — ask codex "what did you do last turn" via a custom prompt command and it remembers). Restart Hot Sheet → play resumes the persisted thread.
...
- [ ] **Model-B toggle (HS-9430, docs/129 §129.5–6).** Settings → Experimental → **"Codex terminals host the driven session"** is checked by default. Uncheck it, then open a NEW codex AI terminal → it launches **plain `codex`** (no `--remote`), and a play turn runs in the drive's own off-screen session (busy/done still work; the Commands Log transcript still fills). Re-check it and open another terminal → back to `codex --remote … -C`. The setting survives a reload, and no "↻ Rejoin codex" pill exists in the terminal header in either state (deleted in HS-9430).
- [ ] **Daemon transport + live watch (HS-9388, docs/121 §121.6).** With no codex daemon running (`~/.codex/app-server-control/app-server-control.sock` absent), click play → the daemon starts (socket appears) and the turn runs through it (no private `codex app-server` child of the Hot Sheet process). Read the thread id from `<dataDir>/codex-app-server.json`, then in a terminal: `codex resume <threadId> --remote unix://~/.codex/app-server-control/app-server-control.sock` → the TUI shows the driven history; click play again → the new turn renders **live in the TUI** with no interaction. Typing a prompt INTO that TUI must not flip Hot Sheet's busy/done (shared-thread guard). Driven hotsheet MCP calls succeed (HS-9395 — e.g. the agent's `hotsheet_signal_done` lands as the done entry, not just the fallback).
- [ ] **Model-B: terminal owns the thread, drive discovers it (HS-9428/9429/9438, docs/129).** Model-B is **default ON** (no env var needed; `HOTSHEET_CODEX_DISCOVER_THREAD=0` reverts to model-A). Open a codex `{{aiCommand}}` AI terminal → it launches **`codex --remote 'unix://…' -C '<projectDir>'`** (a fresh daemon-hosted session, NOT plain `codex` and NOT `codex resume <id>`) — even BEFORE any play (the spawn awaits the daemon coming up, `codexTerminalNeedsDaemonEnsure`). Now click play → the drive DISCOVERS this terminal's live thread by cwd (`thread/loaded/list` → `thread/read` per id) and the turn renders **in your terminal**. Type a prompt yourself → it shares the same thread + transcript. If the daemon can't start, the terminal falls back to plain `codex`.
  - [ ] **Shared approval answered in the TUI (HS-9447).** On a driven turn that needs approval, both Hot Sheet's §47 popup AND the codex TUI prompt appear (codex asks every subscribed client). Answer **in the terminal** → Hot Sheet's popup must disappear on its own within a second, and must not re-ask or leave the turn stuck. Answering in Hot Sheet instead should likewise clear the TUI's prompt (codex's own side).
  - [ ] **First-turn transcript + approvals (HS-9439).** On a brand-new terminal's FIRST driven turn, open the Commands Log: the `codex_turn` entry must fill with per-item detail (agent messages, `$ command` lines) — it used to be a bare start/end. And if that first turn needs an approval, the **§47 permission popup** must appear in Hot Sheet (before HS-9439 the prompt appeared only in the terminal TUI). Both rely on a `thread/resume` retry landing ~1.5 s into the turn.
  - [ ] **The HS-9438 cold case specifically:** play on a **brand-new** terminal that has never run a turn (its rollout doesn't exist yet, so `thread/resume` on it fails). The turn must still render in that terminal, and Hot Sheet's busy state must clear + `done` land when it finishes (that first turn's lifecycle comes from `thread/status/changed`, not `turn/completed`; expect no per-item transcript detail for it — the *next* turn has full detail once the subscription upgrades).
  - [ ] **Late terminal:** press play FIRST (no terminal open → the drive uses its own thread), then open the codex terminal, then press play again → the second turn renders in the terminal (per-turn re-discovery). Confirm it does not drift back to the drive's own thread on subsequent plays.

### OpenCode (ACP) integration end-to-end (HS-9330/9340/9341/9344/9346) — needs `opencode` on PATH + `opencode auth`
*(Every seam below is unit-tested AND module-level live-validated against real `opencode acp` — driving one turn through `acpClient`/`acpFs`/`acpAutoAllow`. The ACP `initialize` handshake against the real binary is now an AUTOMATED local-only test — `src/acp/opencodeAcpLive.test.ts`, HS-9432 (skips when `opencode` isn't installed; cost-free, no LLM turn). The items below are the remaining pieces: the full turn + permission flow through the running server + browser UI together, which can't be automated headlessly.)*

- [ ] **Setup writes instructions + config.** In a project, Settings → General → AI tool = **OpenCode** (`opencode` on PATH, authed). Apply the managed AI-instruction sections → the project gets `AGENTS.md` (HS-9344; opencode reads the AGENTS.md standard). On the first play, confirm `.hotsheet/opencode-acp.json` exists with `permission: { edit/bash/webfetch: "ask" }` (HS-9341), and the spawned `opencode acp` has `OPENCODE_CONFIG` pointed at it.
- [ ] **Play button drives `opencode acp` (HS-9330).** With `ai_tool=opencode`, the sidebar **play button** shows (busy label reads **"OpenCode"**). Click it → Hot Sheet spawns `opencode acp`, runs ONE turn (`initialize`→`session/new` with the `hotsheet_*` MCP entry→`session/prompt` = the worklist), the busy indicator stays lit during the turn (heartbeats), and clears on `stopReason` (turn end). The **Commands Log** shows a `trigger` entry and a `done` entry reading **"OpenCode finished"** (HS-9345 — not "Claude finished").
- [ ] **Permission popup → edit lands (HS-9340/9341, the critical path).** Give the agent a task that edits a file. On its first tool call, confirm: (1) the **§47 overlay appears** rendering the agent's OWN options as buttons (**Allow once / Always allow / Reject** — the option-driven layout, HS-9330); (2) **Allow** → the file edit **actually lands on disk** (HS-9340 — opencode delegates the write via `fs/write_text_file`, which Hot Sheet performs, confined to the project dir); (3) **Reject** → the change is blocked. If instead opencode **auto-approves with no popup**, the `permission: ask` config isn't taking effect (check `OPENCODE_CONFIG`).
...
- [ ] **Override persists + is Local.** Set Agent backend = **Claude channel (MCP)**; reopen Settings → it's still selected. Confirm it wrote to `.hotsheet/settings.local.json` (`agent_backend: "claude-channel"`), NOT the shared `settings.json` (it's a `local-only` §95 field — read-only in the Shared scope). Set it back to **Auto** → the key clears/reverts to auto.
- [ ] **Override changes the drive.** In a project with `ai_tool=opencode`, forcing Agent backend = **Claude channel** makes the play button use the Claude channel path instead of spawning `opencode acp` (and vice-versa where the agent is installed). *(Full cross-transport override needs the HS-9339 command input; forcing a transport the AI tool doesn't support no-ops until then.)*
...
- [ ] **HS-9036 — worker / worktree-terminal permission prompts surface.** Launch a **freshly-started** Claude in a git worktree, BOTH ways (workers started before this fix won't have the flag): **(a)** a worker-pool / Auto worker (`/hotsheet-worker`), AND **(b)** the worktrees panel **"Open terminal"** button on a worktree row (an interactive Claude, not the worker loop — the maintainer's actual repro). In each, when that Claude hits a tool-permission prompt, it appears in the **main Hot Sheet** permission popup (NOT only in the terminal). **Allow/Deny** from the popup actually answers the prompt (routes back to that worker's channel server) and Claude proceeds. Confirm with the main agent ALSO running so two channel servers are alive. **Root cause (the real fix):** both paths now launch with `--dangerously-load-development-channels server:hotsheet-channel-<slug>` — the worker pool via `workerLaunchCommand`, the worktrees panel via the `{{claudeCommand}}` token resolved server-side in the `runCommand` path. That flag is what opts Claude into routing permission prompts to the channel server; pre-fix a bare `claude` connected the channel MCP for tools but prompted permissions in the terminal. (The HS-9036 multi-server polling was necessary but not sufficient on its own.)
- [ ] **HS-9086 — residual workspace-trust prompt on a fresh worktree (expected, not a bug).** The **first** time a worker's Claude opens a brand-new git-worktree directory, Claude Code shows its **workspace-trust dialog** ("Do you trust the files in this directory?"). This is **expected** and cannot be pre-approved by any settings file (it's Claude Code's own directory-trust gate, separate from the MCP-tool/skill permissions HS-9058 writes). Confirm: after accepting the trust prompt **once**, the worker proceeds with **no further** allow prompts for the `hotsheet-channel-<slug>` MCP tools or the generated skills (those are pre-approved in the worktree's `.claude/settings.local.json`). See docs/64 §64.8.
...
### Multiple connections / cleanup (HS-8460 / HS-8948 / HS-9225)
- [ ] Open two Claude Code instances in the same project → the sidebar shows "2 Claude connections active … Disconnect all, then /mcp to reconnect the one you want." with a **Disconnect all** button.
- [ ] Click **Disconnect all** → **every** main channel-server is terminated (incl. the leader), a toast confirms "Disconnected N Claude connection(s) — run /mcp …", and the warning disappears. `mcp.log` shows a `multi-connection` roster line + a `multi-connection-cleanup` line. Then run `/mcp` in the Claude you want → it reconnects as the sole connection and triggers route to it.
- [ ] Reproduce an orphan (e.g. a Claude exits but its MCP child lingers) → the warning shows even with one Claude window; **Disconnect all** clears it (and `/mcp` reconnects cleanly).
...
- [ ] **Disconnect all spares workers (HS-9038 / HS-9225):** force a second *main* connection (two Claude windows in the owner repo root) while workers run → the warning shows; **Disconnect all** terminates **both** mains, leaving **every worker** connected.
...
- [ ] **QR pairing (HS-9026 + HS-9033 + HS-9097), full device path:** Settings → Remote Access → **Pair a Device…** → confirm the reachable URL → a QR shows with a live countdown. On a phone/tablet **reaching the server over a trusted/tunnel channel** (the exposed mTLS port rejects the unenrolled device at the handshake), open **`/pair`** and **scan the QR with the in-page camera** (embedded `@zxing/browser` decoder); the device generates its key + CSR in-browser, enrolls over the pairing token, and offers the `.p12` to download. Then **install the `.p12`** as a client cert and confirm `https://<host-ip>:4174` connects over mTLS and the device appears in the list. (Automated now: the `/pair` page, the paste-the-code path, **the in-page camera scan + ZXing decode** (HS-9097 — `e2e/pair-camera.spec.ts` drives a real QR through Chromium's fake camera), in-browser keygen/CSR/`.p12` assembly, and the enroll round-trip — `e2e/pair.spec.ts` + `src/client/pairing/devicePairing.test.ts`. This manual item is now specifically the **platform-specific client-cert install** — iOS profile / Android / desktop keychain / Firefox store — the **real-device QR camera scan on physical hardware** (the automated test proves the decode path; a real camera confirms framing/focus), and the real mTLS connect afterward.)
...
- [ ] **HS-9291 new-ticket empty-state hero + launch pulse.** In a project with **no tickets** on a main view (open / up-next), the new-ticket input renders as a **prominent accent-tinted card** with an "Add your first ticket" heading + "Type a title, then press ↵ Enter" subtext (badge un-dimmed, input on a solid pill). Type a title + Enter → the card **relaxes to the normal subtle row** as soon as the ticket appears. Switch to an empty **category / priority / completed** view → it stays the subtle row (NOT the hero). Then **relaunch the app** with tickets present → a brief one-shot **glow pulse** plays once on the input (drawing the eye), then never again that session; relaunch again to see it repeat. When the hero is showing (empty project) the pulse does NOT play. *(Hero view/empty gating + pulse-once/skip-when-hero logic is unit-tested in `src/client/draftRowHero.test.ts`; the visual card + animation are manual.)*
...
- [ ] **HS-9249 "Claude usage" block does NOT flash/hide while typing.** Open a ticket that HAS channel-attributed Claude usage (block visible). Click into the **Details** field and type continuously for several seconds. The block must stay visible the whole time — it must NOT blank out and reappear a moment later on each auto-save (the details field debounce-saves, which triggers a background detail reload; pre-fix that cleared the block then re-fetched it). Switching to a DIFFERENT ticket still updates the block correctly (shows the new ticket's usage, or collapses if it has none); switching back re-paints instantly from cache. (Unit-covered in `ticketTelemetryStats.test.ts`; this smokes the real typing/poll timing.)
- [ ] **HS-9248 channel trigger doesn't pick up the selected ticket.** Select (or create) a ticket that is NOT in Up Next, so it's open in the detail panel. Hit the channel play button with no typed message. The triggered Claude session must work the **actual Up Next worklist** — it must NOT start implementing the merely-selected ticket. (Root cause was the `<!-- hotsheet:ticket=… -->` attribution marker riding the trigger and reading like a directive; unit-guarded by `channelTriggerNoMarker.test.ts`.) Per-ticket "Claude usage" cost should still accrue for tickets the agent actually marks `started` (time-window attribution).
...
- [ ] **WebGL default in the drawer** — open a drawer terminal, run something with heavy output (`yes`, a long `claude` session, `top`). Output is smooth; text is crisp. (Settings → Terminal has no "Use software rendering" tick.)
- [ ] **Software-rendering opt-out** — Settings → Terminal → tick "Use software rendering for terminals". Open a NEW terminal. It renders via the DOM (still correct, just CPU). The row is hidden entirely on a browser without WebGL2.
...
- [ ] **HS-9000 tile label is left-aligned + always shown** — open the Terminal Dashboard. Each terminal's name (and the `{Project} ›` prefix in flow mode) sits flush against the **left** edge below its tile, not centered. The name is **always visible for every cell** at every grid size — it is never crowded out by the right-aligned stats cluster (the HS-9000 regression: on medium tiles the fixed-width counts used to collapse the name to nothing). At narrower sizes the counts text drops out before the name does.
- [ ] **HS-9056 per-project stats cluster + responsiveness** — open the Terminal Dashboard with terminals across a couple of projects. Below each tile, to the **right** of the name, the tile shows that project's **open** count and **up-next** count (e.g. `5 open  2 up next`). Trigger Claude in one project (play button / a channel command) → a small **spinner** appears in that tile's cluster while it's busy and disappears when idle. Change a project's tickets (mark some Up Next, complete one) → within ~20 s the counts on its tiles update without reopening the dashboard. Drag the size slider to make tiles **narrow** → the counts text drops out (tile too cramped) while the spinner (if busy) stays; widen again → counts return. The drawer terminal grid (§36) shows **no** stats cluster — only the dashboard does. (Counts data + busy spinner have unit coverage; this manual item is the live container-query reflow + busy visual, which the headless env can't size.)
...
- [ ] **Windows foreground detection (HS-9027)** — on a real Windows box (Parallels per the Windows-testing reference). With a `cmd`/`powershell` terminal sitting idle, Alt+F4 / close → **no** prompt (idle shell correctly detected via `Get-CimInstance`, not the old always-prompt). Then run a real command (e.g. `node`) in the terminal and close → the prompt **does** fire listing that process. Pre-HS-9027 the prompt fired unconditionally because Windows returned the safe-default; this confirms the PowerShell adapter resolves the foreground process. (The parse/selection logic is unit-tested host-agnostically in `processInspectWin32.test.ts`; this item covers the live PowerShell spawn that can't be exercised off-Windows.)
...
- [ ] **HS-9200 — focus ring tracks the TERMINAL, not the find widget.** In the dashboard, enter dedicated view (double-click a tile) or center a tile, then click into the terminal → the accent **focus ring** appears around it. Now open the **find widget** (⌘F / the search toggle) and click into its input: the ring around the terminal **disappears** (the terminal no longer holds keyboard focus — your keystrokes go to search). Close/blur search back into the terminal → ring returns. Pre-fix the ring used `:focus-within`, so it lingered while you typed in the search box (or tabbed to a toolbar button) — i.e. it showed a focus outline around a terminal that wasn't actually receiving keyboard input. The ring now keys off xterm's own `.focus` class (`:has(.xterm.focus)`).
- [ ] **HS-9484 — enlarging a tile keeps keyboard focus (Tauri desktop check).** Automated in Chromium (`e2e/terminal-dashboard-focus.spec.ts`); this item only exists to confirm WKWebView agrees, since the fix leans on suppressing a `mousedown` default. In the dashboard: (a) single-click a tile to zoom it, then **click inside the terminal** and type — the characters appear (pre-fix the click dropped focus to the page body and typing went nowhere, with no recovery short of Esc); (b) with a tile zoomed, **double-click it** to maximize, wait a full second, then type — the characters appear (pre-fix focus worked for ~300 ms and then died, because the uncenter animation's trailing blur landed on the terminal the maximized view had just focused). Also confirm the tile's own controls still respond to clicks while it is zoomed. Repeat both in the **drawer terminal grid** (§36), which shares the same tile-grid code.
- [ ] **HS-9200 (reopen) — demagnify clears the focus ring in the Tauri desktop build.** *Must be tested in the packaged Tauri app (WKWebView), NOT a browser — the bug doesn't reproduce in Chromium.* In the dashboard, single-click a tile to magnify (center) it and click into it so it takes keyboard focus (ring shows, typing works). Now click the backdrop (or another tile) to **demagnify**. The tile shrinks back to the grid AND its focus ring **disappears** — it must NOT keep an accent outline while no longer accepting keystrokes. Repeat via **dedicated view** (double-click to enter, click Back/Esc to exit): the ring clears on exit. Pre-fix, hiding the focused helper-textarea on demagnify didn't fire a blur in WKWebView, so xterm kept its `.xterm.focus` class and the ring lingered. Now an explicit `term.blur()` runs on the demagnify path. Same check applies in the **drawer terminal grid** (§36).
...
- [ ] **HS-8656 Cmd/Ctrl+Shift+[ / ] cycle tabs.** With a terminal focused, Cmd+Shift+[ / ] move between drawer/terminal tabs (same as Cmd+Shift+←/→); with focus outside the drawer they move between project tabs. (macOS Terminal.app parity — both brackets and arrows work.) **HS-9443 — the project-tab half is now automated** (`e2e/shortcut-bracket-tab-cycle.spec.ts`: real keypresses cycle forward + back, from the focused draft input). What still needs a human: the **terminal-focused** direction above, and — since key reporting differs between engines and only Chromium was measured — a check in the **Tauri WKWebView build** that both chords still fire there (the matcher accepts `[`/`{` and `]`/`}`, so it should, but the whole bug was an engine reporting the character we didn't expect).
...
- [ ] **HS-9198 / HS-9208 — claim-before-work enforcement.** (a) **Owner editing is frictionless AND doesn't claim (HS-9208):** with no workers, create a ticket and edit its title/details/tags/notes/attachments — it just works and the ticket stays **unclaimed** (no `owner` claim chip appears). Only setting status → **Started** stamps the `owner` claim. (b) **A fresh worker can't stomp the owner's ticket:** set a ticket to **Started** in the owner UI (so the owner holds the lease), then in a **freshly-relaunched** worker have it `hotsheet_update_ticket` that same ticket → it gets a **409 "held by `owner`"** and should pick another. (Editing the ticket's *metadata* in the owner UI alone no longer claims it — start it to hold it.) (c) **A worker works its own claimed ticket fine:** a relaunched worker `claim_next`s a ticket then `update_ticket`s it (status/notes) with no conflict (its channel server injects its canonical lease id — from `HOTSHEET_WORKER_ID`, HS-9676 — as the actor, matching its claim id). (d) **The auto-claim reminder:** the `update_ticket` that sets status→`started` on an unclaimed ticket returns a `claim` block reminding the agent to renew/release (metadata-only edits return no `claim` block). **CAVEAT:** workers launched **before** the HS-9198 change run an old channel server that doesn't inject the actor header — they will 409 on their **own** claimed tickets. **Relaunch workers** after updating (like HS-9036).

- [ ] **HS-9676 — reused worker claims under its canonical lease id, not the worktree folder.** Scale the pool up, let a worker finish + drain, then scale up again so slot `worker-1`'s worktree is reused (its folder becomes `hotsheet-worker-1-N`, N incrementing). Dispatch a ticket to `worker-1` (drag onto its tile / "Dispatch to…"). Confirm the worker **recognizes it owns** the dispatched ticket and works it (it does NOT report "no tickets for me"), and that its **claim / renew / status→started / release** all attribute to `worker-1` (the chip shows `worker-1`, never `hotsheet-worker-1-N`). Kill it mid-ticket and relaunch → the relaunched worker still uses `worker-1` (check `echo $HOTSHEET_WORKER_ID` in its terminal shows `worker-1`, and the launch prompt states it). This is the whole point of the fix — before it, a reused worker derived `hotsheet-worker-1-N` from its cwd and never saw `worker-1`'s dispatched work.
...
- [ ] **Per-worktree git chip (HS-9081, docs/102 §102.3):** while a worker is mid-task on its branch, its tile shows a compact git chip in the header — `↑N` for commits ahead of the target once it commits, `↓N` when the target has moved ahead (needs rebase), and `•dirty` while it has uncommitted changes. The chip updates on the ~3 s poll (commit on the worker branch → `↑` appears; the working tree goes clean → `•dirty` clears) and disappears entirely when the worker is clean + in sync. The sidebar (main-worktree) git chip is unaffected.
- [ ] **Glassbox "Review" (HS-9082, docs/102 §102.2):** with the Glassbox CLI installed, a worker that has committed work (its chip shows `↑N`) gets a **Review** button on its tile. Clicking it opens **Glassbox** focused on the diff of that worker's branch vs the integration target (`<target>..hotsheet/worker-N`) — i.e. exactly what integrating that branch would add. A worker with no committed work (`↑0`) shows no Review button; with Glassbox NOT installed, no Review button appears on any tile (probed when the panel opens). A *stopped* worker whose branch still has commits ahead keeps its Review button (you can still review + integrate its branch).
- [ ] **Prompt box → main agent (HS-9079, docs/101):** the worker-pool panel has a prompt box at the bottom. Type e.g. "parallelize the tickets tagged refactor" and click **Go** (or Enter): with Claude connected, the main agent receives a worker-management directive and uses the `hotsheet_*` tools to query → size → partition → dispatch (watch it size the pool + dispatch chunks across workers). The input clears; a toast confirms it was sent. Empty input does nothing. If Claude isn't connected, a warning toast appears (nothing sent). If the main agent is **busy** (mid-task), a confirm ("Main agent is busy… Send anyway?") appears first — Cancel sends nothing, Send queues it. (Live end-to-end orchestration — the agent's actual partition/dispatch — is the hard-to-automate part covered here.)
- [ ] **"Parallelize tag…" quick action (HS-9080, docs/101 §101.2):** below the prompt box, click **Parallelize tag…** → a menu of the project's tags appears. With ≥1 live worker and some Up Next tickets carrying the picked tag, picking a tag opens the **partition editor** pre-populated with the proposed split (which tickets → which worker, related tickets grouped, large/risky isolated). Reassign tickets between workers if you like, then **Apply/Dispatch** — each chunk is dispatched to its worker (their tiles flip to working); **Cancel** dispatches nothing. With no live workers, a toast says to start some first; with no tickets carrying that tag, a toast says so (no editor opens); with no tags on any ticket, the menu is replaced by a toast. The split + dispatch are client-side (no agent needed) and always preview before dispatching.
...
**Run 1 — 2026-08-12, pid 86406 (dev/tsx, HEAD `ce8d0916`), ~4 min to ~50 min uptime, 10 projects registered.**
...
| **19.4 memory pressure** | **pass** — no churn spiral | `sudo memory_pressure -l critical -s 60`. Kernel `memorystatus_vm_pressure_level` rose to **2 (warn)** for ~10 s (sampled at 2 s; free pages collapsed 34 k → **7 k** ≈ 112 MB). **`evictChurn` stayed 1** — the failure signal (churn climbing) did **not** appear. No forced eviction: the working set (open 11, ext ~1888 MB, used 24%) was far below the 8192 MB ceiling, so even a halved budget had nothing to shed — see reading. |
| **19.6 sleep / wake** | **pass** — wake path fired, no stampede | `sudo pmset relative wake 100 && pmset sleepnow`. Confirmed **~102 s system suspend** three ways: pmset `Software Sleep … 102 secs` (woke via **rtc**, the scheduled wake), a **101 s** gap in the 2 s pressure sampler, and a freeze-log `server-heartbeat` at 01:51:23Z with **dur 99682 ms, cpuMs 0, ratio 0.0000**. Memory samples straddling the wake are identical (`open=11 churn=1 idle=3` before **and** after) — **no post-wake stampede**. |
...
**Reading of 19.4:** the failure signal — `evictChurn` climbing (the cache fighting itself instead of shrinking) — did **not** appear. Two caveats on coverage. (1) The tool reached **warn (2)**, not **critical (4)**: on a 32 GB machine `memory_pressure -l critical` did not drive the kernel all the way to critical, so the docs/131 *critical → drop-to-floors* branch was not exercised — only the *warn → halve-the-room* branch. (2) Even that halving evicted nothing, because the working set was ~1.9 GB against an 8192 MB ceiling — there was nothing above the floor to shed. So this confirms the *no-churn* invariant and the warn-path budget response, but the **critical-path eviction-down behavior needs a run where the cache is genuinely near the ceiling** (same shape as HS-9619 — a large cache is the missing precondition). Filed as HS-9620. Non-root note: without `sudo` the tool cannot move the level at all (verified — it stayed 1 during a non-root `-l warn` run), and a raw large-allocator alternative was declined against the live instance (at ~39 MB free, dirtying enough to reach critical risks jetsam killing the Hot Sheet server itself, whose ~2 GB of PGLite WASM heaps make it a prime target).

**Reading of 19.6 (the headline):** this is the **first time the HS-9567 wake path has fired on macOS**. As that ticket predicted, macOS logs **no `server-wake` entry** — `isSuspendGap` is false because libuv's monotonic clock advances through sleep, so the wake is not the *confident* clock-divergence case. Instead it lands as a `server-heartbeat` gap with **cpuMs 0** over 99.7 s, and the weaker `looksLikeIdleGap` predicate (cpuMs/durationMs < 0.05) is what fires the wake listeners → the HS-8726 post-wake stagger. The stagger's job is to prevent the HS-9553 post-sleep stampede named as the 2026-08-01 trigger, and it held: cluster open/evict counters were **identical across the wake**, no burst. Note it took a few tries to get a real suspend — `pmset sleepnow` first produced only ~2 s software-sleeps that a `wifibt` network wake bounced instantly (below the 10 s `WAKE_GAP_THRESHOLD_MS`, so nothing fired); toggling WiFi/Bluetooth off removed the wake source and let the machine hold the full ~102 s until the scheduled RTC wake.
...
- **Local-only LIVE agent tests** (skip when the tool/daemon isn't present, so CI skips them; run automatically on a dev machine with the tool installed): codex model-B discovery against the REAL app-server daemon — fresh `thread/start` → `thread/loaded/list`+`thread/read` → discovered by cwd, plus (HS-9438) its reported rollout absent from disk and `thread/resume` on it erroring `no rollout found`, plus (HS-9435) the REAL `initialize` succeeding + the `thread/start` result parsing through the PRODUCTION `threadIdFromResponse` / `rolloutPathFromThreadPayload` mapping functions — a drift-catch the hand-authored `codexAppServerMapping.test.ts` unit tests can't give (`src/codexModelBLive.test.ts`, HS-9431/9438/9435, cost-free); the codex turn lifecycle (turn/start → approval/elicitation → done) is a real-turn concern covered by the opt-in `src/codexApprovalLive.test.ts` (`HOTSHEET_CODEX_LIVE=1`) + the schema-contract `src/codexApprovalSchemaContract.test.ts`; OpenCode ACP `initialize` handshake against the REAL `opencode acp` binary asserting ACP protocol v1 (`src/acp/opencodeAcpLive.test.ts`, HS-9432, cost-free). **Per-tool config-writer CLI contracts (HS-9436, cost-free):** the Codex MCP-config writer round-trips through the REAL `codex` — `ensureCodexMcpConfig` → `codex mcp add` → `codex mcp list --json` reads back `hotsheet-channel` with the exact channel-server command/args, against a throwaway `$CODEX_HOME` (`src/codexConfigLive.test.ts`); and the Gemini skills writer's `.gemini/skills` tree is discovered by real `gemini skills list` (folder-trust disabled in a throwaway HOME; `src/geminiSkillsLive.test.ts`). Pattern for the remaining agent live tests is HS-9432. **No cost-free CLI contract surface (HS-9436):** Antigravity (`agy` has no `mcp` subcommand — it reads `~/.gemini/config/mcp_config.json` directly) and OpenCode (reads `.claude`/`.agents/skills` directly, no skills-list command) can't be CLI-contract-verified; their config/skill writers stay unit-tested (`src/antigravity.test.ts` / `src/skills.test.ts`). Codex's AGENTS.md + `.agents/skills` adapter half likewise has no `codex`-CLI list surface — unit-tested only.
...
- Shared/Local scope-delta hide→un-hide override round-trip (§95.3 / HS-9212), for BOTH the terminals editor (`src/client/terminalsHideOverride.test.ts`) and the auto-context editor (`src/client/autoContextHideOverride.test.ts`, HS-9219): load an already hidden+overridden entry → re-enable restores the CUSTOM text and drops it from `hidden` while keeping `overrides`; disabling a visible overridden entry persists BOTH `hidden` and the override. Real-Tauri-dialog + on-disk `settings.local.json` verification stays manual (§95.3).
