...
- [ ] **Play button drives `codex exec` (HS-9369).** With `ai_tool=codex`, click play → no "Claude is not connected" error (the HS-9364 reopen); codex runs the worklist one-shot, the busy indicator stays lit (heartbeats), the Commands Log shows the trigger + a "Codex finished" done entry, and busy clears on completion.
- [ ] **Interactive permissions (HS-9359).** Enable **Settings → General → "Interactive permission prompts (Codex)"** (shows only when AI tool = Codex) → the project gains `.codex/hooks.json` (PreToolUse `^(Bash|apply_patch|Edit|Write)$` + PermissionRequest `*` → `__codex-permission-hook`). Press play → on codex's first shell/edit tool call the **§47 popup appears**; **Allow** → the call proceeds; **Deny** → codex reports the tool blocked and adapts. Hot Sheet's own `hotsheet_*` MCP calls do NOT pop (auto-allowed).
- [ ] **Toggle off cleans up.** Uncheck the setting → our entries leave `.codex/hooks.json` (user's other hooks, if any, remain) and play reverts to the unattended auto-approve run.
