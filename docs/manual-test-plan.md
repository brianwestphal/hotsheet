...
- **Local-only LIVE agent tests** (skip when the tool/daemon isn't present, so CI skips them; run automatically on a dev machine with the tool installed): codex model-B discovery against the REAL app-server daemon — fresh `thread/start` → `thread/loaded/list`+`thread/read` → discovered by cwd, plus (HS-9438) its reported rollout absent from disk and `thread/resume` on it erroring `no rollout found` (`src/codexModelBLive.test.ts`, HS-9431/9438, cost-free); OpenCode ACP `initialize` handshake against the REAL `opencode acp` binary asserting ACP protocol v1 (`src/acp/opencodeAcpLive.test.ts`, HS-9432, cost-free). **Per-tool config-writer CLI contracts (HS-9436, cost-free):** the Codex MCP-config writer round-trips through the REAL `codex` — `ensureCodexMcpConfig` → `codex mcp add` → `codex mcp list --json` reads back `hotsheet-channel` with the exact channel-server command/args, against a throwaway `$CODEX_HOME` (`src/codexConfigLive.test.ts`); and the Gemini skills writer's `.gemini/skills` tree is discovered by real `gemini skills list` (folder-trust disabled in a throwaway HOME; `src/geminiSkillsLive.test.ts`). Pattern for the remaining agent live tests is HS-9432. **No cost-free CLI contract surface (HS-9436):** Antigravity (`agy` has no `mcp` subcommand — it reads `~/.gemini/config/mcp_config.json` directly) and OpenCode (reads `.claude`/`.agents/skills` directly, no skills-list command) can't be CLI-contract-verified; their config/skill writers stay unit-tested (`src/antigravity.test.ts` / `src/skills.test.ts`). Codex's AGENTS.md + `.agents/skills` adapter half likewise has no `codex`-CLI list surface — unit-tested only.
