...
Subsequently widened (still one swappable surface) to also re-export `defineStore` / `resetAllStores` / `Store` (§61), `morph` (HS-8365), and — under **HS-8613 / HS-8614** — kerf's event-delegation helpers **`delegate(root, type, sel, handler)`** and **`delegateCapture(...)`**. `delegate` installs ONE listener at a stable container that dispatches via `closest(selector)` from `event.target` (auto-promoting the well-known non-bubblers — `focus`/`blur`/`scroll`/`load`/`error`/`mouseenter`/`mouseleave` — to the capture phase); `delegateCapture` is the explicit capture-phase escape hatch. **Its matching changed in kerf 2.0** (adopted under HS-9449): it was strict `matches()` against the event target, and is now the same `closest()` walk-up `delegate()` uses, handing the handler the matched ANCESTOR rather than the raw target — pass `{ match: 'direct' }` to restore strict matching. No Hot Sheet call site uses it today; `reactive.test.ts` pins both modes. Both return a disposer — capture it whenever the registration's scope is shorter than the page (kerf hard rule #5); for a genuinely page-lifetime root, opt out of the `kerfjs/require-delegate-disposer` lint with `void delegate(...)`. These replace per-element `addEventListener` that has to be re-attached on every full-container rebuild and goes stale under `morph()` (reading row identity from a `data-*` attribute instead of a closure-captured index).
...
### Event-delegation track (HS-8613 / HS-8614 / HS-8615 / HS-8616) — from the HS-8541 kerf audit

A distinct strand from the signal/`morph` migration above: the audit flagged per-element `addEventListener` re-bound on every full-container rebuild (instead of one delegated listener at the stable container). Adopting kerf's `delegate()` removes the re-attach churn AND removes the closure-captured-index fragility that blocks a later `morph()` flip (a preserved node would keep a stale closure).

- **HS-8614 (done):** the four Settings-dialog editable row lists — `settingsCategories.tsx` (category CRUD), `terminalsSettings.tsx` (terminal reorder/delete), `commandEditor.tsx` (command/group edit/delete/drag), `tagsDialog.tsx` (tag toggle). Each now binds one delegated handler set at its stable container, reading row identity from a `data-index` / `data-ref` / `data-tag` attribute. The lone exception is `terminalsSettings`'s stateless WebKit `mousedown` swallow, which must run at the button (before the draggable row) to keep the click alive in WKWebView and so stays per-element.
- **HS-8613 (done):** `noteRenderer.tsx` per-note + per-draft + add-button listeners delegated once at `#detail-notes` (the explicitly-deferred HS-8365 follow-up). Identity comes from `data-note-id` / `data-draft-id` + a module-level render context (the container is reused across tickets, so the ticketId/notes can't be closure-captured). This unblocked the `morph()` flip — completed in **HS-8651** below.
- **HS-8651 (done):** flipped `renderNotes` from `replaceChildren` to `morph()` (the HS-8613-unblocked goal). `commitNotesChildren(container, children)` builds a detached holder, appends the (already image-proxied) children, and `morph()`-es the container against it — morph is **childrenOnly** so `#detail-notes`'s id/class are never touched. The two reconcile hazards the HS-8613 note flagged are handled: (1) the `proxyGitHubImages` / `appendImageDownloadLinks` mutations are applied to the template entries BEFORE the morph, so morph reconciles proxied-against-proxied (`isEqualNode` no-op for unchanged notes) instead of reverting them; (2) an in-progress inline edit is preserved across an interleaving re-render by stamping `data-morph-skip` on any entry whose `.note-edit-area` is uncommitted (the save path sets `data-committed='1'` first, so a committed entry is NOT skipped and rebuilds into display mode). Entries carry `data-key` (note id / `draft-`-namespaced draft id) for keyed matching across add/delete. Verified: 4 happy-dom unit tests (`noteRenderer.test.tsx` — in-place node reuse, keyed add, in-progress-edit preservation, committed-save rebuild) + a passing Playwright e2e (`notes-morph.spec.ts` — a poll-driven re-render reuses note nodes in place + preserves scroll, asserted in real Chromium). Tauri WKWebView smoke is on the manual test plan.
- **HS-8615 (done):** swapped the 9 hand-rolled `container.addEventListener(type, e => …closest…)` sites to `delegate()` (consistency/readability; not bugs) — `backups.tsx`, `feedbackDialog.tsx` (disposer captured + freed on dialog close), `telemetryRecentPromptsList.tsx`, `crossProjectStatsPage.tsx` (×2), `commandLog.tsx`, `detailBindings/attachmentActions.tsx` (×2; one `.attachment-item` delegate + in-handler `closest()` branch preserves the reveal→delete→select priority), `terminal/gutterPopover.tsx`. Page-lifetime / element-bound roots use the `void delegate(...)` opt-out; the per-dialog feedback file-list captures its disposer.
- **HS-8616 (investigation done):** `bindList` has **PRESERVE** semantics — a surviving key's row is NOT re-rendered on data change (`reactive-bind.ts:261`). `ticketRow` / `columnView` rows are mounted once per (variant, lifecycle); their handlers close over the `ticket` captured at first render. Visuals stay fresh via the HS-8335 per-row signal, and the *user-action* handlers stay fresh via HS-8367's `Object.assign(ticket, updated)` — but the **poll / external-update path** (`reconcilePerTicketSignals` / `applyServerUpdate` *replace* the signal value with a new object, never touching the closure) leaves the closure `ticket` stale. A subsequent status-cycle / star / category / priority / context-menu click then acts on stale data. Confirmed real (narrow) risk, especially under the channel workflow → filed must-fix **HS-8652**.
- **HS-8652 (done):** fixed the HS-8616 stale-closure bug with a surgical `liveTicket(id, fallback)` re-fetch (exported from `ticketRow.tsx`; reads the per-ticket signal value, falling back to `state.tickets`, then the closure) at every mutable-field-reading handler in `createTicketRow` (status / star / category / priority / context-menu / title-edit / keydown shortcuts) and `createColumnCard` (category / priority / star / context-menu). Chose the targeted re-fetch over the full container-`delegate()` restructure the HS-8616 note sketched: the ticket list is the highest-blast-radius surface (drag, multi-select, title-input focus, the WebKit drag-isolation concerns from `terminalsSettings`), and re-fetch fully closes the correctness bug with a one-line-per-handler change that's unit-testable (`ticketRowStaleClosure.test.tsx` simulates an external `applyServerUpdate` then fires each handler). id-only handlers (selection, drag, keyboard nav) keep their closure — `ticket.id` is immutable. The broader move-rows-to-container-`delegate()` (for `morph()`-safety) remains an opportunistic §60 cleanup, not a correctness need.
