...
Subsequently widened (still one swappable surface) to also re-export `defineStore` / `resetAllStores` / `Store` (§61), `morph` (HS-8365), and — under **HS-8613 / HS-8614** — kerf's event-delegation helpers **`delegate(root, type, sel, handler)`** and **`delegateCapture(...)`**. `delegate` installs ONE listener at a stable container that dispatches via `closest(selector)` from `event.target` (auto-promoting the well-known non-bubblers — `focus`/`blur`/`scroll`/`load`/`error`/`mouseenter`/`mouseleave` — to the capture phase); `delegateCapture` is the explicit capture-phase escape hatch with strict `matches()` matching. Both return a disposer — capture it whenever the registration's scope is shorter than the page (kerf hard rule #5); for a genuinely page-lifetime root, opt out of the `kerfjs/require-delegate-disposer` lint with `void delegate(...)`. These replace per-element `addEventListener` that has to be re-attached on every full-container rebuild and goes stale under `morph()` (reading row identity from a `data-*` attribute instead of a closure-captured index).
