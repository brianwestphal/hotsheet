...
- **HS-8613 (done):** `noteRenderer.tsx` per-note + per-draft + add-button listeners delegated once at `#detail-notes` (the explicitly-deferred HS-8365 follow-up). Identity comes from `data-note-id` / `data-draft-id` + a module-level render context (the container is reused across tickets, so the ticketId/notes can't be closure-captured). The `morph()` flip the delegation was meant to unblock is **deferred to a focused follow-up**: `renderNotes` still does post-build imperative DOM mutation (`proxyGitHubImages` rewrites `img.src`, `appendImageDownloadLinks` appends nodes) and injects the inline-edit `<textarea>` on demand — both of which `morph()` would revert when reconciling against a freshly-built template, so the flip needs those handled (template-side proxying + `data-morph-preserve` on the edit textarea) and real-browser verification of the notes UX.
