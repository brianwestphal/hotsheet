...
Watching `index` catches stages / commits / unstages. Watching `HEAD` catches branch switches and detached-HEAD transitions. Watching `.git/refs/remotes/` would catch background fetches done by other tooling (IDE, terminal `git fetch`), but `fs.watch` plus `git`'s ref-packing can be flaky here — not implemented. **Known gap:** working-tree-only edits (a tracked file modified, or a new untracked file) don't touch `.git/index`/`HEAD`, so they don't bump the version — the chip's unstaged/untracked counts refresh on the next `window.focus` / project switch rather than instantly. §48.2 lists working-tree mutations as in scope; closing the gap (without watching `node_modules`/build dirs) is a follow-up.
