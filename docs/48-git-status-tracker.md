...
> **HS-9111 — working-tree edits refresh via a foreground poll (closes the §48.2 gap).** Working-tree-only edits (a tracked file modified, or a new untracked file) don't touch `.git/index`/`HEAD`, so the directory watcher never fires for them. Rather than recursively watch the working tree — `fs.watch(..., {recursive:true})` isn't supported on Linux, and on macOS it would fan out a huge `node_modules` / `dist` event volume — `src/git/watcher.ts` runs a **low-frequency `git status` poll** (`WORKING_TREE_POLL_MS = 4000`, shared unref'd interval) for the **foreground** project(s) only (the same `isProjectActive` gate the watcher's proactive path uses), bumping the version when the working-tree *signature* (`branch|detached|ahead|behind|staged|unstaged|untracked|conflicted`) changes. `git status` already honors `.gitignore`, so ignored churn never registers — no central exclusion list to maintain. The `.git`-watcher's pre-warm refreshes the same signature baseline, so a stage/commit it already fanned out isn't re-notified by the poll. Cost is bounded: one cached/coalesced `git status` per foreground project every 4 s. Tests: `watcher.test.ts` (state machine — baseline, signature change, unchanged, background-skip) + `watcherWorkingTree.realrepo.test.ts` (real git repo — tracked edit / untracked add bump; a `.gitignore`d file does not). The `.git`-dir `fs.watch` also gained an `error` listener so a removed repo dir / `EMFILE` can't throw unhandled.
