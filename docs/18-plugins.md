...
- **Enable/Disable**: Per-project. Stored in the project's settings table as `plugin_enabled:{id}`. **Default: DISABLED — opt-in per project (HS-8284).** A fresh project's settings table contains no `plugin_enabled:*` rows; `isPluginEnabledForProject` returns `false` for that case so the user must explicitly turn each plugin on for each project from Settings → Plugins. The historical default was "enabled unless explicitly false," which silently activated every installed plugin against every newly-added project folder. Disabling cleans up `ticket_sync` and `sync_outbox` records for that project. Context menu includes "Enable on All Projects" and "Disable on All Projects" (affects open projects only).
...
- **Incremental vs. full pull (HS-8931):** scheduled/auto syncs pass `since = max(last_synced_at)` across this plugin's sync records (the incremental watermark). A **user-initiated sync** (the "Sync" toolbar button → `POST /api/plugins/:id/sync` → `runSync(id, { fullPull: true })`) passes `since = null` (full pull). This matters because an incremental `since` can never surface a remote item that has **no sync record AND was updated before the watermark** — e.g. an issue whose local ticket was deleted (its record went with it), or one missed by an earlier transient apply failure. Once the watermark advances past such an item it would be stranded forever under incremental sync; the full pull on a manual sync reconciles it. (Scheduled syncs stay incremental for efficiency.)
