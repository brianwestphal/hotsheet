...
- This **supersedes** the `otel_rollup_daily.prompt_count` / `session_count` columns as the source of truth for distinct counts (those columns, populated by the HS-9234 backfill under the table's original design, are now vestigial — a cleanup can drop them). **Remaining HS-9243 items** (do NOT block the dashboard-aggregate repoint, since `getPerTicketRollup` is a cheap single-ticket read that can stay on raw until Phase 3): per-ticket `duration_seconds` ingest maintenance (a span side-table, or recompute-on-read) and the marker path — the latter **resolved by HS-9248** (retiring the selection→marker coupling, so ongoing per-ticket attribution is time-window-only; historical markers stay covered by the HS-9234 backfill). Tests: `src/db/otelRollupIngest.test.ts` (`markDailySeen` dedup/day/central), `src/db/otelRollupBackfill.test.ts` (`backfillDailySeenForDir` distinct + idempotent + cumulative-exclusion), `src/db/otelWriters.test.ts` (end-to-end: ingest populates the set).
