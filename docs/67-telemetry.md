...
- **HS-9279 (P3b) — in progress.** New daily rollup table `otel_rollup_activity` (one flexible grain — `kind` ∈ {tool, hour, tool_latency} + `dim1`/`dim2` + `count`/`sum_val`/`sum_n`) for the aggregate reads that still scanned raw. **`getToolRollup` + `getToolLatencyHistogram` DONE:** ingest (`recordToolActivity`) maintains `kind='tool'` (count + duration sum/n) **and** `kind='tool_latency'` (per-bucket counts, via the shared `otelHistogram.ts::latencyBucketIndex`); `backfillActivityToolForDir` (own done-flag, cli-wired) backfills both from raw. `getToolRollup` reads the rollup (count = SUM(count), avg = SUM(sum_val)/SUM(sum_n)); `getToolLatencyHistogram` reads count/totalMs from `kind='tool'` + per-bucket counts from `kind='tool_latency'`, with **p50/p90/p99 approximated from the bucket histogram** (`percentileFromBuckets`; maintainer-accepted — the beta spans-fidelity path was retired). **Remaining:** `getHourlyActivityHeatmap` (kind='hour' cost sum + a new `otel_hourly_seen` (project,day,hour,prompt) dedup for the distinct-prompt count; server-local bucketing, weekday reconstructed client-side). Non-destructive.
