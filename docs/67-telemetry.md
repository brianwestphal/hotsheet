...
- **Still on raw (→ Phase 3 JSONL, non-regressing):** `getPerTicketRollup` (per-ticket cost — has a circular coupling with the HS-9234 backfill that needs an extract-helper refactor first) + the hour-of-week heatmap, recent-prompts, tool-latency, and §68 inspectors. The vestigial `otel_rollup_daily.prompt_count`/`session_count` + `otel_rollup_ticket.duration_seconds` columns are dropped WITH the per-ticket repoint (follow-up).
