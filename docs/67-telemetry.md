...
- **HS-9286 — pending (P3c read-gate, part 2).** The Announcer's `collectTelemetrySignals` (§82 mid-task narration) still reads *recent* `user_prompt`/`tool_result` from raw `otel_events`; it must repoint to the JSONL store before HS-9280 stops the writer INSERTs (it reads recent rows, so it breaks the moment raw writes stop, not just on drop).
