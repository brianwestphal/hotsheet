...
**HS-9233 — ingest-time rollup maintenance + raw slimming (epic HS-9226 Phase 2).** `src/db/otelRollupIngest.ts`, called from `otelWriters.ts`, **dual-writes** the rollups alongside the raw rows (raw → un-snapshotted cluster, rollups → snapshotted main db) — nothing reads the rollups until the HS-9235 repoint, so this is invisible to the UI and safe to land incrementally.
- **`otel_rollup_daily`** is upserted per cost/token data point: `cost_usd` + the four split token sums + `datapoint_count`, keyed by `(project_secret, day, model, query_source)`. Cumulative-monotonic counters are skipped (mirrors the reads' `EXCLUDE_CUMULATIVE_MONOTONIC_SQL` so they can't re-inflate). Because the dashboards SUM raw rows with no per-row dedup, summing the same values at ingest is **behaviorally identical** — these sums are exact relative to what the dashboards show today.
- **`otel_rollup_ticket`** is maintained via the **time-window path**: an `api_request` whose `ts` falls inside an open `ticket_work_intervals` window (read from the cluster db, where intervals live) increments that ticket's `cost_usd` / `total_tokens` / `model_breakdown`; a `user_prompt` in the window bumps `prompt_count`.
- **Raw slimming:** the redundant nested `attributes` array is stripped from the stored `value_json` / `body_json` (the flattened `attributes_json` already holds it, and the per-ticket marker lives in the record *body*, not attributes) — ~40-50% smaller raw rows.
- **Documented gaps** (reconciled by the HS-9234 backfill / decided at the HS-9235 repoint, tracked by follow-ups): `otel_rollup_daily.prompt_count`/`session_count` (distinct counts that can't be maintained exactly at ingest) and `otel_rollup_ticket.duration_seconds` are left at their defaults at ingest; the per-ticket **marker path** (`<!-- hotsheet:ticket=… -->`) is not attributed at ingest. Tests: `src/db/otelRollupIngest.test.ts` + end-to-end assertions in `otelWriters.test.ts`.

**HS-9234 — backfill rollups from existing raw (epic HS-9226 Phase 2).** `src/db/otelRollupBackfill.ts` (`backfillTelemetryRollups`), run once at startup from `cli.ts` right after the HS-9231 relocation, derives the rollup rows from the **existing** raw `otel_*` history (HS-9233 only rolls up rows that arrive after it shipped), so the §70/§71 dashboards keep their full history once HS-9235 repoints them at the rollups (and Phase 3 moves raw to disposable JSONL). For each project it reads raw from its un-snapshotted `<dataDir>/telemetry/db` cluster and writes rollups into its snapshotted main `<dataDir>/db`; the central store (`~/.hotsheet/telemetry`, both handles resolve to the same db) gets only the daily rollup (it has no tickets).
- **Recompute-from-scratch, so idempotent:** unlike the ingest path (which increments), the backfill computes the COMPLETE aggregate from ALL raw rows and OVERWRITES — it `DELETE`s the rollup tables in a project's main db, then bulk-inserts the recomputed rows. Re-running yields the same result and subsumes whatever the dual-write ingest already wrote (the per-ticket UNION is a superset of ingest's window-only attribution). It takes a `daily`-tier backup of the launched project first.
- **Parity with the canonical reads:** the per-ticket scalars (cost / tokens / prompt count / duration) come from `getPerTicketRollup` itself (run against the cluster via `runWithTelemetryDb`), so the backfilled `otel_rollup_ticket` numbers are byte-for-byte what the dashboard shows today (HS-9235 is then a pure source swap), plus a dedicated per-model `model_breakdown` query mirroring the same marker + time-window UNION. The daily aggregate mirrors `getWindowTotals` / `getCostByModel` / `getQuerySourceRollup` (same metric names, the same `EXCLUDE_CUMULATIVE_MONOTONIC_SQL`, the same token-`type` → column mapping, the same `(unknown)` fallbacks) and buckets by the server-local day (matching `serverLocalDay` at ingest).
- **Distinct counts (the HS-9243 gap, computed here):** `prompt_count` / `session_count` can't be split across the `(model, query_source)` grain, so the backfill computes them exactly per `(project, day)` via `COUNT(DISTINCT …)` and stamps the whole day's count onto a single representative grain row (the day's highest-`datapoint_count` row, ties broken by model then source) so `SUM(prompt_count) … GROUP BY day` is exact. A day with counts but no metric grain row gets a synthesized `(unknown)/(unknown)` carrier row. How ONGOING (post-backfill) days source these stays the HS-9243 / HS-9235 decision.
- Guarded by the `telemetryRollupBackfilledV1` config flag, resumable per dir via `telemetryRollupBackfillV1DoneDirs` (same pattern as the HS-8874/9231 migrations). Tests: `src/db/otelRollupBackfill.test.ts` (pure `assembleDailyRows`, the per-dir recompute + parity + idempotence, and the run-once orchestrator).

**HS-9243 — daily distinct-count dedup set (`otel_daily_seen`; epic HS-9226 Phase 2 follow-up).** The daily rollup's `prompt_count` / `session_count` can't be maintained on the `(model, query_source)` grain (a prompt/session spans multiple grain rows), so distinct counts get their own **dedup set** instead: `otel_daily_seen (project_secret, day, kind, id)` in the SNAPSHOTTED main db (`SCHEMA_VERSION` 7→8; added to the §41 co-save). `kind` is `prompt` (distinct `prompt_id` over ALL log events) or `session` (distinct `session.id` over the cost/token metrics) — mirroring the getWindowTotals / getCostByProject distinct-count sources.
- **Ingest** (`markDailySeen` in `otelRollupIngest.ts`, called from `otelWriters.ts`): every event with a `prompt_id` and every rollup-metric datapoint with a `session.id` is `INSERT … ON CONFLICT DO NOTHING`-ed by `(project_secret, server-local day, kind, id)`, so re-seeing an id that day is a no-op. The **read** (HS-9235) derives `prompt_count` / `session_count` as `COUNT(*)` over this set for the window — exact, and with **no dependency on the raw `otel_*` tables**, which is what lets Phase 3 drop them.
- **Backfill** (`backfillTelemetryDailySeen` / `backfillDailySeenForDir` in `otelRollupBackfill.ts`, run once from `cli.ts` after the rollup backfill): derives the distinct prompt/session ids per day from each cluster's raw and inserts them into that project's main-db set (idempotent via `ON CONFLICT`). Guarded by `telemetryDailySeenBackfilledV1`, resumable via `telemetryDailySeenBackfillV1DoneDirs`.
- This **supersedes** the `otel_rollup_daily.prompt_count` / `session_count` columns as the source of truth for distinct counts. **HS-9259 dropped those columns** (and `otel_rollup_ticket.duration_seconds`, superseded by the span table below) — `SCHEMA_VERSION` 9→10, an `ALTER TABLE … DROP COLUMN IF EXISTS` migration in `initSchema`; the backfill's `assembleDailyRows` is now a plain 1:1 grain map. Tests: `src/db/otelRollupIngest.test.ts` (`markDailySeen` dedup/day/central), `src/db/otelRollupBackfill.test.ts` (`backfillDailySeenForDir` distinct + idempotent + cumulative-exclusion), `src/db/otelWriters.test.ts` (end-to-end: ingest populates the set).

**HS-9243 part 2 — per-ticket duration spans (`otel_ticket_prompt_span`).** Per-ticket `duration_seconds` = sum-per-prompt of (max−min ts), which isn't additive at ingest, so it gets a span side-table too: `otel_ticket_prompt_span (project_secret, ticket_number, prompt_id, first_ts, last_ts)` in the snapshotted main db (`SCHEMA_VERSION` 8→9; in the co-save; kept for the life of the ticket like `otel_rollup_ticket`).
- **Ingest:** `attributeApiRequestToTicket` now also takes the event's `promptId` and calls `widenTicketPromptSpan` (`otelRollupIngest.ts`) — `INSERT … ON CONFLICT DO UPDATE` with `first_ts = LEAST(…)`, `last_ts = GREATEST(…)`, widening the prompt's span for the ticket the time-window path attributed. The HS-9235 read recomputes `duration = SUM(last_ts − first_ts)` over the ticket's prompts (recompute-on-read; **supersedes** the `otel_rollup_ticket.duration_seconds` column).
- **Backfill:** `backfillTelemetryTicketSpans` / `backfillTicketPromptSpansForDir` derive the spans from raw — the same **marker UNION time-window** matched set `getPerTicketRollup` uses, grouped per (ticket, prompt) → MIN/MAX ts (so historical marker-attributed tickets keep their duration even though HS-9248 stopped producing new markers). Guarded by `telemetryTicketSpanBackfilledV1`. Tests: `src/db/otelRollupIngest.test.ts` (`widenTicketPromptSpan` LEAST/GREATEST + `attributeApiRequestToTicket` span), `src/db/otelRollupBackfill.test.ts` (`backfillTicketPromptSpansForDir` time-window + marker + idempotent).
- **Marker path (#3) — resolved by HS-9248** (the selection→marker coupling is retired, so ongoing per-ticket attribution is time-window-only; historical markers stay covered by the backfills). All three HS-9243 ingest gaps are now closed; `getPerTicketRollup` can be repointed off raw in HS-9235.

**HS-9235 — repoint the dashboard AGGREGATE reads to the rollups (epic HS-9226 Phase 2, part 1).** The cross-project (§70) + per-project (§71) dashboard aggregate reads now read the compact rollup tables in the snapshotted main db instead of scanning raw `otel_*` in the un-snapshotted cluster.
- New **`getRollupDb()`** (`connection.ts`) — mirrors `getTelemetryDb`'s context resolution (`runWithTelemetryDb` → request → `defaultDbPath` → central) but WITHOUT the `telemetryClusterDataDir()` wrap, so it returns the project's MAIN `<dataDir>/db` where the rollups live (central maps to itself, so the central fallback is identical to `getTelemetryDb`).
- Repointed (`otelRollups.ts`): **getWindowTotals** (cost/token sums from `otel_rollup_daily`; distinct prompt/session counts as `COUNT(DISTINCT id)` from `otel_daily_seen`, prompt-kind → session-kind fallback), **getCostByModel** (GROUP BY the rollup's pre-bucketed `model`; `promptCount` dropped to 0 — not displayed), **getCostByProject** (cost/tokens from the daily rollup + counts from the seen set; `lastActivityTs` originally stayed on raw for sub-day precision, **now reads the JSONL store — HS-9285**), **getCostOverTime** / **getIngestedDates** (read the rollup `day` column; day grain is server-local — locally viewer tz == server tz), **getTodayCost** (+`getTodayCostByProject`). New `buildRollupDayClauses` / `seenDistinctCount` helpers filter on the server-local `day` via `serverLocalDay`. Windows become day-granular (dashboard windows are day-aligned midnights, so exact for them).
- **`clearProjectTelemetry`** now also deletes the four rollup tables for the secret (main db) — else a cleared project would keep showing rollup-sourced cost/counts.
- Public payload shapes are unchanged; the client is untouched. **Parity note:** real-work `tokens` = `SUM(input_tokens + output_tokens)` from the rollup — an UNTYPED token (unknown/legacy `type`) is NOT counted (it lands in `datapoint_count` only), a deliberate consequence of the rollup shape (real Claude Code token.usage always carries a type). Tests: the `otelQueries.test.ts` seed helpers dual-write the rollups (mirroring ingest) so seeding populates raw AND rollup in the same db context the read uses, preserving per-project isolation + clear behavior.
- **Still on raw (→ Phase 3 JSONL, non-regressing):** the hour-of-week heatmap, recent-prompts, tool-latency, and §68 inspectors.

**HS-9257 — repoint `getPerTicketRollup` (epic HS-9226 Phase 2, part 2).** The last dashboard read still scanning raw. `getPerTicketRollup` (`otelDashboard.ts`) now reads cost / tokens / prompt_count from `otel_rollup_ticket` and duration = `SUM(EXTRACT(EPOCH FROM (last_ts - first_ts)))` from `otel_ticket_prompt_span`, both in the main db via `getRollupDb()` — no raw `otel_*` scan.
- **Broke the circular coupling:** `backfillTicketsForDir` used to call `getPerTicketRollup` to read raw for the canonical scalars it writes into `otel_rollup_ticket`; now that `getPerTicketRollup` READS that table, the raw computation was extracted into `computeTicketRollupFromRaw(clusterDb, ticket, secret)` (same file) and the backfill points at THAT.
- The rollup is keyed by `project_secret`; production always passes the request's `projectSecret` (`routes/telemetry.ts`), so a no-secret read finds nothing (rollup-model successor to the old marker-only back-compat path). Marker-tagged HISTORICAL prompts are still attributed by the backfill (per-project) → the ticket's rollup row.
- Tests: the per-ticket tests recompute the rollup + duration spans from raw (production backfill path) before reading. All otel suites green.
- **Vestigial columns** (`otel_rollup_daily.prompt_count`/`session_count`, `otel_rollup_ticket.duration_seconds`) are now written-but-unread; dropping them is a separate cleanup (**HS-9259**) — non-regressing, not a Phase 3 blocker.

**HS-9236 — rotating JSONL raw store (epic HS-9226 Phase 3, part 1).** With the dashboards off raw (above), raw telemetry is needed only by the deep §68 inspectors — so it moves out of the PGLite cluster into day-partitioned JSONL files, letting Phase 3 (HS-9237) drop the raw tables and keep the snapshotted DB small (the epic's goal). `src/db/otelJsonlStore.ts` (mirrors `diagnostics/freezeLogger.ts`):
- **Files:** `<telemetryDir>/otel-<events|metrics|spans>-YYYY-MM-DD.jsonl`, where `<telemetryDir>` = `telemetryClusterDataDir(dataDir)` (a project's `<dataDir>/telemetry`, or the central store). They sit OUTSIDE `<telemetryDir>/db`, so `dumpDataDir` (the snapshot §73 + backup §7 path) never sees them — same reason HS-9230 relocated the raw cluster off the main db. The filename IS the time index (server-local day, matching the rollup grain), so an inspector opens only the relevant day's file.
- **Append** (`appendOtelJsonl`) is a per-FILE single-flight Promise chain (no mid-line interleave); serialization happens inside the chain so a `JSON.stringify` failure can't throw into ingest. **Read** (`readOtelJsonlDay`) is crash-tolerant — a torn/unparseable last line (unclean shutdown mid-append) is skipped. Best-effort throughout (write failures swallowed), the freeze.log contract.
- **Ingest dual-writes** it: `otelWriters.ts`'s three `persist*Payload` writers append the raw row to JSONL alongside each cluster INSERT (the de-risk phase — raw still lives in the cluster too; HS-9237 makes the inspectors read JSONL and drops the raw tables).
- **Sweeper** (`sweepOtelJsonl`, `kinds` filter) age-deletes files, wired into `cleanup.ts::cleanupTelemetryRows` with the SAME windows as the raw-table retention: events/metrics at `telemetry_retention_days` (30), spans at `telemetry_span_retention_days` (7); `0` = keep forever. Tests: `src/db/otelJsonlStore.test.ts` (append/read round-trip, day+kind partitioning, torn-line tolerance, concurrent-append non-interleave, sweeper windows) + dual-write assertions in `otelWriters.test.ts`.

**HS-9237 — Phase 3 execution plan (maintainer chose approach (b), 2026-07-02).** Auditing the remaining raw readers showed dropping `otel_events` needs MORE than the 3 named deep inspectors off raw: four dashboard-frequency reads still hit raw (`getHourlyActivityHeatmap`, `getToolRollup`, `getToolLatencyHistogram`, `getRecentPrompts`), and scanning unindexed JSONL for those on every dashboard open is a perf risk. So (b): **aggregates → new rollups, raw-detail → JSONL, then drop the tables**, decomposed into gated subtickets:
- **HS-9278 (P3a) — DONE.** The raw-detail reads now scan the JSONL store, not raw `otel_*`: `getPromptTimeline` (via `getPromptTimelineFromJsonl`, fanning out over `getAllProjects()` + central + ambient), `getRecentPrompts`, and `getTelemetryDebugInfo` (events + the `metrics` JSONL, incl. the GLOBAL cross-project `dailyMetricCounts` fan-out). Store helpers: `readOtelJsonlRange` / `readAllOtelJsonl` / `listOtelJsonlDays` (`otelJsonlStore.ts`) + `currentTelemetryClusterDir()` (`connection.ts`, resolves the current async-context cluster dir like `getTelemetryDb`). Non-destructive — raw tables stay (still dual-written) until P3c. (The "event list" is the drilldown's event stream, covered by `getPromptTimeline`.)
- **HS-9279 (P3b) — DONE.** All three aggregate reads now read rollups, not raw. New daily rollup table `otel_rollup_activity` (one flexible grain — `kind` ∈ {tool, hour, tool_latency} + `dim1`/`dim2` + `count`/`sum_val`/`sum_n`) + a small `otel_hourly_seen` (project, day, hour, prompt) dedup. Ingest (`recordToolActivity` / `recordHourCost` / `markHourlySeenPrompt`) maintains them; `backfillActivityToolForDir` + `backfillActivityHourForDir` (own done-flag, cli-wired) backfill from raw.
  - `getToolRollup` — reads `kind='tool'` (count = SUM(count), avg = SUM(sum_val)/SUM(sum_n)).
  - `getToolLatencyHistogram` — count/totalMs from `kind='tool'`, per-bucket counts from `kind='tool_latency'`, **p50/p90/p99 approximated** from the bucket histogram (`otelHistogram.ts::percentileFromBuckets`; the beta spans-fidelity path was retired — maintainer-accepted).
  - `getHourlyActivityHeatmap` — cost from `kind='hour'` (`sum_val`), distinct-prompt count from `otel_hourly_seen`; **server-local bucketing** (the weekday is reconstructed from the rollup `day`, so the read-time `timezone` param is now unused — maintainer-accepted, same-machine-tz).
  P3a + P3b moved the §68 inspectors + the three activity aggregates off raw — but a pre-drop audit (2026-07-02) found the gate was **not** fully met: four live reads still hit raw. Those became the real gate for HS-9280 (below).
- **HS-9285 — DONE (P3c read-gate, part 1).** The last two raw `otel_metrics` reads in `otelRollups.ts` moved off raw: **`getQuerySourceRollup`** (§71 "cost by query source") now reads `otel_rollup_daily` GROUP BY `query_source` (the rollup already carried the column — HS-9235 just never switched this fn; `promptCount`→0, like `getCostByModel`), and **`getCostByProject.lastActivityTs`** (§70 "last active") now reads the exact last `claude_code.cost.usage` ts from the day-partitioned metrics JSONL (`latestActivityByProjectFromJsonl`, `currentTelemetryClusterDir()` path) — sub-day precision preserved without the raw table. The now-dead raw-metric helpers (`buildProjectAndWindowClauses`, `REAL_WORK_TOKEN_TYPE_SQL`, `EXCLUDE_CUMULATIVE_MONOTONIC_SQL`) were removed. Tests seed ONLY rollup + JSONL (no raw) so a regression back onto raw fails (`otelQueries.test.ts` "HS-9285 — cross-project reads off raw otel_metrics").
- **HS-9286 — DONE (P3c read-gate, part 2).** The Announcer's `collectTelemetrySignals` (§82 mid-task narration) now reads *recent* `user_prompt`/`tool_result` from the day-partitioned events JSONL (`readOtelJsonlRange` over `[serverLocalDay(since), today]` via `currentTelemetryClusterDir()`) instead of raw `otel_events`, doing the old SQL WHERE + GROUP BY in JS (`isClaudeCodeEvent` name-match, `extractPromptBody` = the `->>'prompt'/'message'/'body'` COALESCE analogue). Same narration output — the existing parity tests (seeded into JSONL now) all pass. It reads *recent* rows, so this had to land before HS-9280 stops the writer INSERTs.
- **HS-9280 (P3c, destructive) — DONE.** The writers' raw INSERTs are gone (`appendOtelJsonl` + the rollup ingest stay); `initSchema` no longer creates the raw tables; a **guarded one-shot `otelRawDrop.ts::dropRawTelemetryTables`** (called from `cli.ts` AFTER the 4 backfills, gated on their flags + a new `telemetryRawDroppedV1`) `DROP TABLE`s `otel_metrics`/`otel_events`/`otel_spans` across the main + cluster DBs for `[launched, ...readProjectList(), central]`. Retention is JSONL-only now (`sweepOtelJsonl`; the raw row-cap/window machinery was removed from `cleanup.ts`); "Clear telemetry data" (§74) wipes the JSONL via `clearOtelJsonl` alongside the rollup clear; `telemetryDiagnostics.ts` (the per-raw-table COUNT diagnostic) was deleted. **JSONL is now the sole raw store.** The relocate/per-project migrations (`telemetryMigration.ts`) were hardened (`telemetryTableExists` skip + `ensureTelemetryTable` create) so a dropped/absent raw table no longer aborts a whole source DB. **HS-9292** carried the ~130-test migration off raw (a shared `test-helpers.ts::createRawOtelTables` re-creates the raw tables for the still-raw-reading backfill tests; writer/decoder tests assert JSONL; retention/schema tests rewritten or removed) + repointed the `--demo:13` seeding to the rollup ingest path.
- **Read foundation shipped in this pass:** `readOtelJsonlRange(dir, kind, fromDay, toDay)` + `dayRange` in `otelJsonlStore.ts` — the inclusive day-range scan the P3a detail reads consume (raw stays untouched; no behavior change yet). Tested in `otelJsonlStore.test.ts`.
