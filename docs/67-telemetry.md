...
- **HS-9280 (P3c, destructive, blocked on 9278+9279 + the 9285/9286 read-gate)** — remove the raw-table INSERTs from the writers (keep `appendOtelJsonl`), drop `otel_events`/`otel_spans`/`otel_metrics` via a **guarded one-shot in `cli.ts` after the backfills** (NOT in `initSchema`, which runs before the backfills read raw → history loss), verify all stats + inspectors.
