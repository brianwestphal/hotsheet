...
- **HS-9279 (P3b) — in progress.** New daily rollup table `otel_rollup_activity` (one flexible grain — `kind` ∈ {tool, hour, tool_latency} + `dim1`/`dim2` + `count`/`sum_val`/`sum_n`) for the aggregate reads that still scanned raw. **`getToolRollup` DONE:** `recordToolActivity` maintains `kind='tool'` at ingest (`otelWriters.ts`), `backfillActivityToolForDir` + `backfillTelemetryActivityRollups` (own done-flag, cli-wired) backfill history from raw, and `getToolRollup` reads the rollup (count = SUM(count), avg duration = SUM(sum_val)/SUM(sum_n)). **Remaining:** `getHourlyActivityHeatmap` (kind='hour', per hour-of-day; weekday reconstructed client-side) + `getToolLatencyHistogram` (kind='tool_latency', per bucket) reuse the same table. Non-destructive.
