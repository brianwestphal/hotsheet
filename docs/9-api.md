...
Migration was **per-domain** (HS-8522 sub-tickets) and is now **complete** (HS-8638 closeout); **git** (`src/api/git.ts` ↔ `routes/git.ts` ↔ `gitStatusChip.tsx` / `gitStatusPopover.tsx`) is the reference implementation. Migrated domains: **git**, **tickets** (`src/api/tickets.ts`, HS-8629 + HS-8642 stragglers — incl. the `GET /tickets/:id` detail response via `TicketDetailSchema`, the `updateTicketField` dynamic-key helper, and routing `undo/actions.ts` through the typed callers), **feedback-drafts** (`src/api/feedbackDrafts.ts`, HS-8642 — the §21 draft endpoints, with `FeedbackDraftSchema` as the wire SSOT consumed by both `src/db/feedbackDrafts.ts` and the client `noteRenderer.tsx`), **terminal** (`src/api/terminal.ts`, HS-8630 — the 10 JSON endpoints; `TerminalConfig` / `TerminalState` / `TerminalStatus` reclaimed here as the SSOT, re-exported by `src/terminals/config.ts` + `registry/types.ts`; the WebSocket attach handler stays bespoke), **telemetry** (`src/api/telemetry.ts`, HS-8632 — the 9 read-only `GET`s + the retention `DELETE`; the cross-project `/dashboard` read forwards `skipProjectScope`; the deeply-nested dashboard payloads are the wire SSOT here while `src/db/otelQueries.ts` keeps its structurally-identical server-internal query types, to bound the blast radius on that module), **backups + db** (`src/api/backups.ts` + `src/api/db.ts`, HS-8636 — backup list/create/now/preview/cleanup/restore + the §42/§73 recovery / snapshot-status / pg_resetwal-repair endpoints; binary tarball download stays bespoke), and **channel** (`src/api/channel.ts`, HS-8631 — claude-check / status / trigger / permission-respond / enable / disable / heartbeat-status; reuses the existing `PendingPermissionSchema` / `PermissionResultBodySchema`; the permission + heartbeat long-polls keep their server-side semantics, and `/channel/{notify,permission/notify,heartbeat}` stay server-to-server), and **projects** (`src/api/projects.ts`, HS-8634 — list / register / delete / reorder / channel-status / feedback-state / reveal + the `/permissions` + `/bell-state` long-polls + the §37 `/quit-summary` aggregator; the permission entry's `request_id` / `tool_name` / `description` use `z.string().catch('')` so a partial entry validates with `''` defaults instead of dropping a real pending request on the long-poll, reconciling the server's all-optional `PendingPermissionEntrySchema` with the popup's required `PermissionData`), and **plugins** (`src/api/plugins.ts`, HS-8637 — list / detail / UI-elements / bundled-catalog / install / enable-disable (per-project + everywhere) / uninstall / reveal / action / validate / config-labels / global-config + the `/backends`, `/sync/tickets`, `/sync/conflicts` reads and conflict-resolve write; this module is the SSOT for the plugin data shapes — `PluginInfo` / `PluginPreference` / `ConfigLayoutItem` / `ConfigLabelColor` / `SyncConflict` / `PluginUIElement` / `BundledPluginInfo` — which `src/client/pluginTypes.tsx` re-exports; one `PluginInfoSchema` covers both the list + detail responses via optional `author` / `configLayout` / `path`), **attachments** (`src/api/attachments.ts`, HS-8633 — `uploadAttachment` / `uploadDraftAttachment` / `deleteAttachment` / `revealAttachment`; the two uploads are multipart so they route through a parallel `apiUploadCall` / `setApiUploadTransport` injection added to `_runner.ts` (wired to the client's `apiUpload` at boot), while the binary serve route `GET /attachments/file/*` stays a bespoke `<img>` / `<iframe>` `src` outside the typed contract), and **settings** (`src/api/settings.ts`, HS-8635 — `getSettings` / `updateSettings` / `getFileSettings` / `updateFileSettings` / `getGlobalConfig` / `updateGlobalConfig`; the enumerated-but-`.loose()` `FileSettingsSchema` is the client SSOT that replaced ~38 inline `api<{ oneKey? }>('/file-settings')` literals — each known key is `.catch(undefined)` so a mistyped legacy value degrades gracefully; the `VisibilityGrouping`/`DashboardConfig`/`GlobalConfig` schemas were de-duplicated to a single definition in `routes/validation.ts` that `src/global-config.ts` now imports), and the **HS-8638 closeout** of the long tail: **shell** (`src/api/shell.ts` — `/shell/exec` / `/shell/kill` / `/shell/running`), **command-log** (`src/api/commandLog.ts` — list / clear / count; `CommandLogEntrySchema` is the wire SSOT, the client adds `isRunningShell` post-fetch), **dashboard** (`src/api/dashboard.ts` — the grab-bag of `routes/dashboard.ts`: `/dashboard` / `/stats` / `/poll` / `/browse` / `/worklist-info` / `/ensure-skills` / `/glassbox/*` / `/print`; SSOT for `DashboardData` + `BrowseResult`), **diagnostics** (`src/api/diagnostics.ts` — `/diagnostics/freeze`), plus `/tags` / `/categories` / `/category-presets` added to `settings.ts` and the plugin `push-ticket` caller added to `plugins.ts`. Cross-project callers take an optional `secret` that forwards to `apiCall`'s `opts.secret`. **The migration is complete: every client call site uses a typed caller, and the raw `api()` / `apiWithSecret()` / `apiUpload()` helpers in `src/client/api.tsx` are now used ONLY as the transport target wired in `app.tsx`.** **When adding a new endpoint:** add its schema + typed caller in `src/api/<resource>.ts`, validate the request server-side against that schema, and call the typed function from the client — do not reintroduce inline `api<{…}>(path)` type literals, and do not call `api()` directly.
