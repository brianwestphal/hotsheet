...
- **HS-9230 — backups exclude telemetry.** The tarball is a single-cluster `dumpDataDir('gzip')` of `<dataDir>/db` only; per-project Claude Code telemetry now lives in the sibling `<dataDir>/telemetry/db` cluster (epic HS-9226 Phase 1), so it is NOT captured in backups. Telemetry is disposable analytics data with its own retention (§85) — keeping it out of backups also keeps the tarball small (it was the bulk of the pre-relocation DB size).
- On startup the scheduler catches up on overdue backups: any tier whose newest backup is older than the tier's interval (5min/hourly/daily) gets a fresh backup. Without the catch-up, daily backups never fire for users who restart Hot Sheet within 24 hours of starting it (HS-7894). **HS-9239 — the catch-up is delayed to 30 s after launch (was 10 s) and its 5-minute tier is skipped under the same backpressure gate as the regular timer** (`shouldDeferFiveMinBackup`), so the heavy CHECKPOINT + `dumpDataDir` + fsync + manifest train doesn't land on top of the startup rush (PGLite init, first ticket load, a kicked-off resume, the first tab-switch) — the freeze a user hit "within a minute of launch." Hourly/daily catch-up still run for durability.
