...
- **Theme-coloured gutter (HS-7960).** `.terminal-body`'s background is set inline (via JS) to the active terminal theme's `background` field whenever the appearance is applied (`reapplyAppearance` + the synchronous prime in `mountXterm`). Pre-fix the gutter painted as `var(--bg)` (white in the default app theme) which produced a visible colour seam between the padded gutter and the xterm canvas under any non-white theme (Dracula, Solarized, etc.). The new helper `resolveAppearanceBackground(appearance)` in `terminalAppearance.ts` resolves the colour; the inner `.terminal-canvas-host > .xterm` block uses `background: inherit` so the cascade carries the theme bg through xterm's own DOM. Pinned by 3 unit tests in `terminalAppearance.test.ts`.
...
| `Opt+Esc` (macOS) | `Alt+Esc` (Linux/Windows) | Routes Esc to Hot Sheet's normal Esc behaviour (HS-8011) — exits dedicated view → centered tile → dashboard / drawer-grid mode, blurs focused inputs, clears ticket selection. Provides an explicit escape hatch from inside a focused terminal without first having to click out. |
