...
**HS-7329 / HS-7459 — Cmd/Ctrl+K clear (platform-specific).** The match logic is a small pure helper (`isClearTerminalShortcut` in `src/client/terminalKeybindings.ts`) shared by every xterm the app mounts — drawer terminal, dashboard tile xterm, dashboard dedicated-view xterm. Each instance's `term.attachCustomKeyEventHandler` calls the helper first and, on match, invokes `term.clear()` and returns `false` to suppress xterm's own handling. The helper is platform-aware: it takes an optional `isMac` boolean which defaults to `navigator.userAgent.includes('Mac')`. On macOS only Cmd+K matches; on Linux/Windows only Ctrl+K matches. This was originally platform-agnostic (HS-7329) but that caused a bug (HS-7459) where Ctrl+K on macOS was hijacked away from readline. The same helper file is where future cross-xterm shortcuts (search, new-tab, etc.) should live so we don't duplicate key-match logic across three call sites. 15 unit tests (`terminalKeybindings.test.ts`) cover the macOS / non-macOS branches, both-modifiers-held rejection, case-insensitivity on the letter, Alt/Shift passthrough, other-key rejection, non-keydown rejection, and the runtime platform-detection default.
...
- **Tickets:** HS-6261 (this doc), HS-6262 (drawer push-up layout prerequisite), HS-6472 (terminal-focused keyboard shortcuts), HS-7329 (Cmd/Ctrl+K clear), HS-7330 (selection highlight colour), HS-7459 (platform-specific Cmd+K only on macOS / Ctrl+K only on Linux/Windows — forwards Ctrl+K on macOS to readline kill-line), HS-7460 (follow-up: audit Cmd/Ctrl+F and Cmd/Ctrl+Up/Down for the same platform-specificity).
