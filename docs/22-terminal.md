...
**Companion: global server-slow banner.** Sibling `src/client/serverBusyChip.tsx` toggles a layout-flow banner (`#server-slow-banner` rendered server-side in `pages.tsx` next to `.update-banner`) whenever ANY non-long-poll HTTP request crosses `SERVER_BUSY_THRESHOLD_MS = 3000`. `api.tsx` wraps every `api()` / `apiWithSecret()` / `apiUpload()` fetch with `trackServerRequest(url)` (returned `done()` is called in the `finally` clause); long-poll endpoints (`/poll?`, `/projects/permissions`, `/projects/bell-state`) are skipped via `LONG_POLL_PATTERNS` so the banner doesn't false-fire on every poll cycle. This addresses the user's note that *"sometimes its not typing into the terminal that's slow, sometimes its when i click a button and nothing happens"* — the banner surfaces stuck button clicks the same way the per-terminal chip surfaces stuck keystrokes. **HS-8226 (2026-05-06)** — replaced the original top-right corner chip (`.server-busy-chip` — translucent amber chip floating at fixed position) with the layout-flow banner described above. Pre-fix the chip was "too transparent and too in the corner" per the user's report; the banner mirrors `.update-banner`'s amber strip pattern (opaque `#fef3c7` background, 2 px solid `#f59e0b` border-bottom, `#92400e` text) but is non-dismissable — it auto-hides when the in-flight set drains. The pulse-dot animation (`terminal-stall-pulse`) survives the redesign so the indicator still has motion to draw the eye.
