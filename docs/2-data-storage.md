...
- **UI (shipped — HS-9004).** A **dialog-wide scope toolbar** sits under the Settings tab strip — an Xcode-build-settings-style segmented control: **Shared** (edit `settings.json`) | **Local overrides** (edit `settings.local.json`) | **Resolved** (the effective view, default). Settings stay in their own tabs; the toolbar re-skins each file-settings field *in place* (mode persists across tab switches, resets to Resolved on each open). Per mode: Resolved shows an origin tag (*from Shared* / *from Local* / *default*) and edits route to each key's default layer (today's behavior); Shared shows the literal `settings.json` value — **even when locally overridden** — plus an "overridden locally" tag; Local shows the override editable with *Reset to shared*, or the inherited value read-only behind *+ Override*. Machine-global settings (the API Keys tab; the Announcer's model/rate/fallback/local-endpoint/speak-permissions) keep their "Global Setting" badge and stay editable in every mode — they write to `~/.hotsheet/config.json`, which is layer-agnostic, so there's nothing to lock. Project-level surfaces that aren't file-settings or aren't yet per-layer-aware (plugin toggles, complex list editors — categories, auto-context, terminals + appearance, allow-rules, custom commands, snapshot/repair, the Announcer's per-project key selector + uninteresting-topics list) are tagged `data-scope-complex` and go read-only ("not locally overridable") outside Resolved — per-layer editing of those is a follow-up (HS-9005). The Announcer's per-project **Enable** toggle (`announcer_enabled`) participates as a normal scoped field (HS-9006 — though [95-settings-sharing-classification.md](95-settings-sharing-classification.md) §95.3 reclassifies the Announcer local-only, pending). **Per-setting sharing classification (personal/Local vs team/Shared vs machine/Global) and element-level per-layer editing for the complex editors are specified in [95-settings-sharing-classification.md](95-settings-sharing-classification.md) (HS-9005) — including the standing rule that complex/ambiguous settings are case-by-case: ask the maintainer, don't guess.** Backed by the layered API (§9): `GET /api/file-settings/layered`, `PATCH /api/file-settings/layer` (`{ layer, settings }`), `POST /api/file-settings/clear-local` (`{ keys }`). Pure layer logic in `src/client/settingsSharing.ts` (`resolveFieldScope`, `scopedDisplayValue`); DOM controller in `settingsScope.tsx`; scalar field writes route through `persistScopedSetting` in `settingsDialog.tsx`.
