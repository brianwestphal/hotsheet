...
1. **Title-change escape sequences** — when the running process pushes a title via the standard OSC 0 / OSC 2 escapes (`\x1b]0;TITLE\x07` or `\x1b]2;TITLE\x07`), the in-pane terminal toolbar label updates to reflect it. The drawer tab keeps its static configured/derived name — shell titles are often long, per-cwd, and noisy, which would make the narrow drawer tab unreadable (HS-6473 follow-up: runtime titles apply to the toolbar only, not the drawer tab). Most modern shells push title updates that include the current working directory or the running command, which is far more informative than a static "Terminal" label.
...
**Toolbar label resolution priority** (in `effectiveHeaderLabel`):
...
**Reset on PTY restart.** When the user clicks the Stop/Start power button to restart the PTY, `onPowerClick` clears `runtimeTitle = ''` so the toolbar falls back to the configured name until the new process pushes its own title.
...
**Empty-string semantics.** If a process explicitly pushes an empty title (`\x1b]0;\x07`), `runtimeTitle` becomes `''` and the toolbar falls back to the configured name. This matches xterm emulator behavior.
...
- Run `printf '\\033]0;custom-title\\007'` in a terminal — the in-pane toolbar label switches to "custom-title". The drawer tab label stays at its static configured name.
- Restart the PTY (Stop, then Start) — the toolbar reverts to the configured name until the new process pushes a title.
