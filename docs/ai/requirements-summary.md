...
**Status:** Shipped — full v1 end-to-end plus HS-6271 (configurable multi-terminal defaults), HS-6306 (dynamic + button), HS-6304 (stop/start power button), HS-6305 (trash icon for clear), HS-6308 (no black strip below the xterm grid), HS-6309 (per-project drawer state + terminal scoping), HS-6310 (eager-spawn for `lazy:false` terminals — spawns at project boot and on settings save), and HS-7275 (closing the active terminal tab selects the nearest remaining terminal tab — walk right from the first-closed position, then left, fall back to Commands Log only when no terminal survives; skipped when the drawer was already showing a different tab). Installed deps: `node-pty` (prod), `ws` (prod + @types/ws dev), `@xterm/xterm` + `@xterm/addon-fit` + `@xterm/addon-web-links` + `@xterm/addon-serialize` (prod). Server side: `src/terminals/{config,ringBuffer,resolveCommand,registry,websocket,eagerSpawn}.ts` + `src/routes/terminal.ts`; client side: `src/client/{terminal,terminalsSettings,terminalTabSelection}.tsx`. HS-7275 isolates the pure selection logic in `src/client/terminalTabSelection.ts` (with 10 unit tests) and routes every close path — single × button, context-menu Close Tab, and bulk Close Others / Left / Right — through a new `selectFallbackAfterClose` helper in `terminal.tsx` that consults `getActiveDrawerTab()` (new export from `commandLog.tsx`) so the fallback only fires when the drawer was actually showing a closed tab. Deferred follow-ups: live Tauri per-platform smoke (manual-test-plan §12), Windows ConPTY verification.
