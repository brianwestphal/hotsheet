...
**Status:** Shipped — `computeLastOutputRange` helper + button + wiring + CSS + 12 unit tests. Follow-up HS-7327 (e2e) open.
...
**Status:** Shipped — keyboard handler + `jumpToPromptMarker` + popover + helpers + setting plumbing + 9 unit tests. Follow-up HS-7328 (Playwright e2e for jumps + popover + settings toggle) open.
...
**Status:** Shipped — helper + click action + popover wiring + CSS + 9 unit tests. All four OSC 133 phases (1a / 1b / 2 / 3) now shipped — HS-7265 spike closed. Follow-up HS-7332 for Playwright e2e + optional shell_analysis event type.
...
| 31 — OSC 133 Phase 1b copy-last-output | Shipped | HS-7268: computeLastOutputRange helper in terminalOsc133.ts + 12 unit tests; copy-output toolbar button with shellIntegration-gated visibility; Lucide clipboard-copy → check flash + shake keyframe CSS; uses navigator.clipboard directly (WKWebView-safe). Follow-up HS-7327 for Playwright e2e |
| 32 — OSC 133 Phase 2 jumps + popover | Shipped | HS-7269: findPromptLine helper + 9 unit tests; Cmd/Ctrl+Up/Down via term.attachCustomKeyEventHandler → jumpToPromptMarker → term.scrollToLine; hover popover anchored to each gutter glyph with Copy command / Copy output / Rerun actions (readRecordCommand / readRecordOutput / term.paste); shell_integration_ui per-project setting (default true) under Settings → Terminal that gates gutter + popover + shortcuts + Phase 1b button without disabling the parser. Follow-up HS-7328 for Playwright e2e |
| 33 — OSC 133 Phase 3 Ask Claude | Shipped | HS-7270: buildAskClaudePrompt helper + 9 unit tests; fourth popover button gated on isChannelAlive(); packs {command, exitCode, cwd (from inst.runtimeCwd), output} into canonical diagnose-and-fix template with tail-truncation at 8 000 chars; dispatches via triggerChannelAndMarkBusy(message). Accent-coloured button styling. Follow-up HS-7332 for e2e + optional shell_analysis command-log event type |
| 34 — terminal find / search | Shipped v1 + e2e; two extensions designed | HS-7331: collapsible `SearchAddon`-backed search widget in the drawer toolbar + dashboard dedicated view (grid tiles deliberately excluded). Cmd/Ctrl+F opens the most recently active widget; Enter / Shift+Enter advance through matches; Esc blurs only (HS-7393); × closes/clears. Amber highlight palette so matches stay distinct from the HS-7330 accent-tinted selection. 14 unit tests in `terminalSearch.test.ts` under happy-dom + 4 Playwright e2e in `terminal-search.spec.ts` (HS-7363). Design complete for two follow-ups: §34.8 regex / case / whole-word toggles (HS-7361 design; HS-7426 implementation pending), §34.9 recent-query history (HS-7362 design; HS-7427 implementation pending) |
