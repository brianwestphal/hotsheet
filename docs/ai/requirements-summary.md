...
| 34 — terminal find / search | Shipped v1 | HS-7331: collapsible `SearchAddon`-backed search widget in the drawer toolbar + dashboard dedicated view (grid tiles deliberately excluded). Cmd/Ctrl+F opens the most recently active widget; Enter / Shift+Enter advance through matches; Esc closes. Amber highlight palette so matches stay distinct from the HS-7330 accent-tinted selection. 14 unit tests in `terminalSearch.test.ts` under happy-dom. Deferred: regex / case-sensitivity / whole-word toggles (HS-7361), search history (HS-7362), Playwright e2e (HS-7363) |
