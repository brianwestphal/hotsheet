...
- `npm run build` → tsup builds three ESM outputs: server (`src/cli.ts` → `dist/cli.js`, externals: pglite, hono, @hono/node-server), channel (`src/channel.ts` → `dist/channel.js`, fully bundled), client (`src/client/app.tsx` → `dist/client/app.global.js` IIFE minified) + SCSS → `dist/client/styles.css` + asset copy.
- `npm run build:client` — client bundle + SCSS + assets only (used by `dev`).
