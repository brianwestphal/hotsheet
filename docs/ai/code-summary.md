...
**Entry + infra:** `app.tsx`, `api.tsx`, `state.tsx`, `dom.ts`, `icons.ts` + `lucide-icons.json`, `animate.ts`, `poll.tsx`, `shortcuts.tsx`, `tauriIntegration.tsx`, `projectTabs.tsx`, `pluginTypes.ts`.
