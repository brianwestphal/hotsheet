...
### 5.2.2 Files with no readable bytes — and unsaved screen captures (HS-9465 / HS-9466)

- Dropped files are **materialized** — read into memory — **before** anything is created or uploaded (`src/client/dropFiles.ts::screenDroppedFiles`), and the upload sends those bytes rather than the original `File`. That makes a truncated body structurally impossible: either the bytes are there or we say so before a request exists. A read that throws, returns **zero bytes**, or returns **fewer bytes than `size`** all count as unreadable (a partial PNG is garbage). Files over 64 MB are streamed as before, after a probe that requires a byte to actually arrive.
  - **HS-9466 correction:** the first version only checked that reading one byte didn't *throw*, and the reported failure got past it — `slice(0,1).arrayBuffer()` can resolve with zero bytes, and one readable byte says nothing about the rest since `fetch` streams a `File` lazily. The symptom moved from the generic crash popup to "Could not attach the file / Malformed upload body", which is how we knew screening had passed it.
...
- **Dragging an unsaved capture now WORKS (HS-9466, maintainer-verified).** Materializing turned out to *fulfill* the promise rather than just detect an unfulfilled one: `fetch` streams a `File` lazily and gets nothing, but a full `arrayBuffer()` is a different request and macOS honors it by delivering the promised bytes. The planned native `NSFilePromiseReceiver` route (docs/130 §130.5) was **never built and is not needed** — and this fix works in the browser too, which the Tauri-only native route could not have. The unreadable-file message above remains the fallback for anything that genuinely doesn't deliver.
